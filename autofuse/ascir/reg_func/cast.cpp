...
std::vector<std::unique_ptr<TmpBufDesc>> CalcCastTmpSize(const AscNode &node) {
  ...
  if (node_inputs[0].attr.dtype == ge::DT_UINT8 &&
      (node_outputs[0].attr.dtype == ge::DT_FLOAT || node_outputs[0].attr.dtype == ge::DT_INT32 ||
       node_outputs[0].attr.dtype == ge::DT_INT16 || node_outputs[0].attr.dtype == ge::DT_INT8 ||
       node_outputs[0].attr.dtype == ge::DT_INT4)) {
    ...
    Expression total_size = sym::Align(input_size * Symbol(2), 32);  // 输入元素个数 * half类型占2个字节
    ...
    GELOGD("node_inputs[0].attr.vectorized_strides[0] is: %s", node_inputs[0].attr.vectorized_strides[0].Str().get());
    GELOGD("input_size is: %s", input_size.Str().get());
    Expression total_size = sym::Align(input_size * Symbol(4), 32);  // 输入元素个数 * float类型占4个字节
    TmpBufDesc desc = {total_size, -1};
    tmpBufDescs.emplace_back(std::make_unique<TmpBufDesc>(desc));
  } else {
