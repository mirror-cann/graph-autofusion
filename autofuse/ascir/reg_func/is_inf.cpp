...
constexpr int32_t IS_INF_ONE_BLK_SIZE = 32;
constexpr int32_t IS_INF_ONE_REPEAT_BYTE_SIZE = 256;
constexpr int32_t IS_INF_MAX_REPEAT_NUM = 255;
constexpr int32_t IS_INF_BASIC_TMP_SIZE = 8192;
...
std::vector<std::unique_ptr<TmpBufDesc>> CalcIsInfTmpSize(const AscNode &node) {
  ...
  Expression min_temp_size = sym::Max(type_size * input_size + Symbol(IS_INF_ONE_BLK_SIZE), Symbol(IS_INF_BASIC_TMP_SIZE));
  min_temp_size = sym::Min(min_temp_size, Symbol(IS_INF_MAX_REPEAT_NUM * IS_INF_ONE_REPEAT_BYTE_SIZE + IS_INF_ONE_BLK_SIZE));
