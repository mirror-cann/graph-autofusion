...
class Scope::ScopeImpl {
 public:
  ...
  const std::unordered_map<std::string, Scope *> &GetSubScopes() const {
    return sub_scopes_;
  }
...
class FusionScopesResult::InnerNodeInfo::InnerNodeInfoImpl {
 public:
  ...
  ge::graphStatus SetDynamicOutputFormat(const std::string &output_name, const uint32_t index,
...
class FusionScopesResult::FusionScopesResultImpl {
 public:
  ...
  const std::map<std::string, std::vector<int32_t>> &GetInputs() const {
    return inputs_;
  }
  const std::map<std::string, std::vector<int32_t>> &GetOutputs() const {
    return outputs_;
  }
...
class ScopeGraph::ScopeGraphImpl {
 public:
  ...
  const std::unordered_map<std::string, FusionScopesResult *> &FusionScopesResults() const {
    return fusion_results_;
  }
  ...
  const std::map<std::string, ge::OperatorPtr> &GetNodesMapNew() const {
    return nodes_map_new_;
  }
  ...
  Status GetInputOrOutputIndex(const ScopeFusionOpInfo &info, const int32_t old_index, const bool input,
                               int32_t &new_index);
