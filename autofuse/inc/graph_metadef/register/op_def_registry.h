...
#define OP_ADD(opType, ...)                                                                                            \
  static int g_##opType##_added = [](const char *name) {                                                               \
    ...
    impl.InferShape(op.GetInferShape()).InferShapeRange(op.GetInferShapeRange()).InferDataType(op.GetInferDataType()); \
...
#define OP_ADD(opType, ...)                                                                         \
  ...
  static ge::graphStatus TilingPrepare##opType(gert::TilingParseContext *context) {                 \
...
#define OP_ADD(opType, ...)                                                                                            \
  static int g_##opType##_added = []() {                                                                               \
    if (ops::OpDefFactory::OpDefRegisterV2 != nullptr) {                                                               \
      return ops::OpDefFactory::OpDefRegisterV2(#opType, [](const char *name) -> ops::OpDef { return opType(name); }); \
    } else {                                                                                                           \
