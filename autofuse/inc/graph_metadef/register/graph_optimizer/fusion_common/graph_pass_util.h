...
class GraphPassUtil {
 public:
  ...
  static ge::OutDataAnchorPtr GetPeerOutAnchorNotInDeleteList(const ge::NodePtr &node, size_t idx);

  static void SetPairTensorIntAttr(const ge::NodePtr &node, size_t idx, const std::map<std::string, int32_t> &attr_val);

  static void SetPairTensorAttr(const ge::NodePtr &node, size_t idx, const std::map<std::string, int32_t> &attr_val,
                                bool is_input = true);
};
