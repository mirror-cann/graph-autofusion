...
class GE_FUNC_DEV_VISIBILITY GE_FUNC_HOST_VISIBILITY Graph {
 ...
 public:
  ...
  graphStatus AddOp(const af::Operator &op);

  ATTRIBUTED_DEPRECATED(graphStatus FindOpByName(const char_t *, af::Operator &))
  graphStatus FindOpByName(const std::string &name, af::Operator &op) const;

  graphStatus FindOpByName(const char_t *name, af::Operator &op) const;

  ATTRIBUTED_DEPRECATED(graphStatus FindOpByType(const char_t *, std::vector<af::Operator> &))
  graphStatus FindOpByType(const std::string &type, std::vector<af::Operator> &ops) const;

  graphStatus FindOpByType(const char_t *type, std::vector<af::Operator> &ops) const;
