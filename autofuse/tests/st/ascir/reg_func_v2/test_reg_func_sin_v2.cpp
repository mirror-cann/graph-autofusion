...
TEST_F(CalcSinTmpSizeV2Test, CalcSinTmpSizeV2_WhenInputsIsFLOAT) {
  ...
  *y.y.strides = {s1, Symbol(1)};
  ...
  // FLOAT: 256 * 3 = 768 bytes
  ASSERT_EQ(result.size(), 1);
  ASSERT_EQ(result[0]->size, Symbol(768));
  ASSERT_EQ(result[0]->life_time_axis_id, -1);
}

TEST_F(CalcSinTmpSizeV2Test, CalcSinTmpSizeV2_ShouldReturn2048_WhenInputsIsFLOAT16) {
  af::AscGraph graph("test");
  auto s0 = graph.CreateSizeVar("s0");
  auto s1 = graph.CreateSizeVar("s1");
  auto s2 = graph.CreateSizeVar("s2");
