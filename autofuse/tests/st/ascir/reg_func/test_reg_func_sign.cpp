...
namespace af {
namespace ascir {
extern std::vector<std::unique_ptr<af::TmpBufDesc>> CalcSignTmpSize(const af::AscNode &node);
...
TEST_F(CalcSignTmpSizeTest, CalcSignTmpSize_ShouldReturnCorrectSize_WhenNodelsValid) {
  ...
  std::vector<std::unique_ptr<af::TmpBufDesc>> result = CalcSignTmpSize(*node);
