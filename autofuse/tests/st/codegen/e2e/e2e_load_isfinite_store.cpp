...
void LoadIsFiniteStore_BeforeAutofuse(af::AscGraph &graph) {
  ...
  *isFinite.y.strides = {s1*s2, s2, One};
  isFinite.attr.tmp_buffers = {{{af::Symbol(8192), -1}, MemAttr(), 0}};
...
void LoadIsFiniteStore_AfterInferOutput(af::AscGraph &graph) {
...
void LoadIsFiniteStore_AfterGetApiInfo(af::AscGraph &graph) {
  auto x = graph.FindNode("x");
...
void LoadIsFiniteStore_AfterScheduler(af::AscGraph &graph) {
...
void LoadIsFiniteStore_AfterQueBufAlloc(af::AscGraph &graph) {
  auto x = graph.FindNode("x");
  ...
  isFinite->outputs[0].attr.buf.id = af::kIdNone;
  ...
  isFinite->outputs[0].attr.opt.ref_tensor = af::kIdNone;
  isFinite->outputs[0].attr.opt.merge_scope = af::kIdNone;
