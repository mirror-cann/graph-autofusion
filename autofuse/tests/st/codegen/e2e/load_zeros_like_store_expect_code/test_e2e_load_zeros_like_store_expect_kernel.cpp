...
extern "C" __global__ __aicore__ void load_zeros_like_store(GM_ADDR x, GM_ADDR y, GM_ADDR workspace, GM_ADDR tiling);
extern "C" void GetTiling(AutofuseTilingData &tiling_data);

class E2E_LoadZerosLikeStore_Code : public testing::Test,
                                    public testing::WithParamInterface<std::pair<std::vector<int>, std::vector<int>>> {
};

TEST_P(E2E_LoadZerosLikeStore_Code, CalculateCorrect) {
  ...
  ICPU_RUN_KF(load_zeros_like_store, tiling_data.block_dim, (uint8_t *)x, (uint8_t *)y, nullptr,
              (uint8_t *)&tiling_data);
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2E_LoadZerosLikeStore_Code,
