...
extern "C" __global__ __aicore__ void concat_same_tail_dim(GM_ADDR x1, GM_ADDR x2, GM_ADDR y, GM_ADDR workspace,
                                                           GM_ADDR tiling);
extern "C" void GetTiling(AutofuseTilingData &tiling_data);

class E2E_ConcatSameTailDim_Code : public testing::Test,
...
TEST_P(E2E_ConcatSameTailDim_Code, ConcatSameTailDim) {
  ...
  ICPU_RUN_KF(concat_same_tail_dim, tiling_data.block_dim, (uint8_t *)x1, (uint8_t *)x2, (uint8_t *)y, nullptr,
              (uint8_t *)&tiling_data);
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2E_ConcatSameTailDim_Code,
                         ::testing::Values(std::pair<std::vector<int>, std::vector<int>>{{65, 1, 65, 1}, {3, 1, 31}}));
