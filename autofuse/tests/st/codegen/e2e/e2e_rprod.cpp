...
void LoadRprodStore_BeforeAutofuse(af::AscGraph &graph, ge::DataType data_type) {
  ...
  *rprod.y.strides = {One, Zero};
  rprod.attr.tmp_buffers = {{{af::Symbol(8192), -1}, MemAttr(), 0}, {{af::Symbol(8192), 0}, MemAttr(), 1}};
...
void LoadRprodStore_AfterAutofuse(af::AscGraph &graph, ge::DataType data_type) {
  ...
  rprod->outputs[0].attr.buf.id = af::kIdNone;
  ...
  rprod->outputs[0].attr.opt.ref_tensor = af::kIdNone;
  rprod->outputs[0].attr.opt.merge_scope = af::kIdNone;

  auto impl = ascgen_utils::GetAscIrCodegenImpl(rprod->GetType());
  std::vector<std::unique_ptr<af::TmpBufDesc>> buffers = impl->CalcTmpBufSize(*rprod);
