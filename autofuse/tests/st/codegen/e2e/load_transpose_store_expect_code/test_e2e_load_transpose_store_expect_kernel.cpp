...
void GetConfusionTranspose2103TilingInfo(const ShapeInfo srcShape, const uint32_t stackBufferSize,
  ...
  uint32_t srcStride = 0;
  uint32_t dstStride =
      (srcShape.originalShape[0] * srcShape.originalShape[1] * srcShape.originalShape[3] - srcShape.originalShape[3]) /
      blockSize;
...
__aicore__ inline void GetConfusionTransposeTilingInfo(const ShapeInfo srcShape, const uint32_t stackBufferSize,
                                                       const uint32_t typeSize, const uint32_t transposeTypeIn,
...
class E2E_LoadTransposeStore_Code : public testing::Test,
...
TEST_P(E2E_LoadTransposeStore_Code, CalculateCorrect) {
  ...
  for (int i = 0; i < test_shape[0]; i++) {
    for (int j = 0; j < test_shape[1]; j++) {
      for (int k = 0; k < test_shape[2]; k++) {
        x[i * test_shape[1] * test_shape[2] + j * test_shape[2] + k] =
            i * test_shape[1] * test_shape[2] + j * test_shape[2] + k;
        expect[i * test_shape[1] * test_shape[2] + k * test_shape[1] + j] =
            i * test_shape[1] * test_shape[2] + j * test_shape[2] + k;
      }
    }
  }
  ...
  GetConfusionTransposeTilingInfo(srcShapeInfo, stackBufferSize, sizeof(uint32_t), (uint32_t)transposeType,
                                  tiling_data.transpose_tilingData_0);
  ...
  ICPU_RUN_KF(load_transpose_store, tiling_data.block_dim, (uint8_t *)x, (uint8_t *)y, nullptr,
              (uint8_t *)&tiling_data);
