...
extern "C" bool CodegenTiling(const std::string &op_name, const ascir::FusedScheduledResult &fused_schedule_result,
  ...
  for (size_t i = 0; i < 10; ++i) {
    auto tiling_data = "tiling_data.graph0_result0_g" + std::to_string(i) + "_tiling_data";
    ss << "  " << tiling_data << ".tiling_key = 0;" << std::endl;
    ss << "  " << tiling_data << ".set_block_dim(48);" << std::endl;
    ss << "  " << tiling_data << ".set_z0z1Tb_size(" << tiling_data << ".get_s0() * " << tiling_data
       << ".get_s1() / 4 / 48 + 1);" << std::endl;
