...
extern "C" __global__ __aicore__ void dynamic_inputs_and_outputs(GM_ADDR x0, GM_ADDR x1, GM_ADDR x2, GM_ADDR x3,
                                                                 GM_ADDR x4, GM_ADDR x5, GM_ADDR x6, GM_ADDR x7,
                                                                 GM_ADDR x8, GM_ADDR x9, GM_ADDR y0, GM_ADDR y1,
                                                                 GM_ADDR y2, GM_ADDR y3, GM_ADDR y4, GM_ADDR y5,
                                                                 GM_ADDR y6, GM_ADDR y7, GM_ADDR y8, GM_ADDR y9,
                                                                 GM_ADDR workspace, GM_ADDR tiling);
extern "C" void GetTiling(AutofuseTilingData &tiling_data);

class E2E_MultiLoadAbsStore_Code : public testing::Test,
...
TEST_P(E2E_MultiLoadAbsStore_Code, packing_func_with_10_io) {
  ...
  ICPU_RUN_KF(dynamic_inputs_and_outputs, tiling_data.block_dim, (uint8_t *)inputs[0], (uint8_t *)inputs[1],
              (uint8_t *)inputs[2], (uint8_t *)inputs[3], (uint8_t *)inputs[4], (uint8_t *)inputs[5],
              (uint8_t *)inputs[6], (uint8_t *)inputs[7], (uint8_t *)inputs[8], (uint8_t *)inputs[9],
              (uint8_t *)outputs[0], (uint8_t *)outputs[1], (uint8_t *)outputs[2], (uint8_t *)outputs[3],
              (uint8_t *)outputs[4], (uint8_t *)outputs[5], (uint8_t *)outputs[6], (uint8_t *)outputs[7],
              (uint8_t *)outputs[8], (uint8_t *)outputs[9], nullptr, (uint8_t *)&tiling_data);
