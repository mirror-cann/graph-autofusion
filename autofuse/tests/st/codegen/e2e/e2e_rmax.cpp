...
void LoadRmaxStore_BeforeAutofuse(af::AscGraph &graph, ge::DataType data_type) {
  ...
  *rmax.y.strides = {One, Zero};
  rmax.attr.tmp_buffers = {{{af::Symbol(8192), -1}, MemAttr(), 0}, {{af::Symbol(8192), 0}, MemAttr(), 1}};
...
void LoadRmaxStore_AfterAutofuse(af::AscGraph &graph, ge::DataType data_type) {
  ...
  rmax->outputs[0].attr.buf.id = af::kIdNone;
  ...
  rmax->outputs[0].attr.opt.ref_tensor = af::kIdNone;
  rmax->outputs[0].attr.opt.merge_scope = af::kIdNone;

  auto impl = ascgen_utils::GetAscIrCodegenImpl(rmax->GetType());
  std::vector<std::unique_ptr<af::TmpBufDesc>> buffers = impl->CalcTmpBufSize(*rmax);
