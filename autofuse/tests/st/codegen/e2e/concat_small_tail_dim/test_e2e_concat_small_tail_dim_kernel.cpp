...
extern "C" __global__ __aicore__ void concat_small_tail_dim(GM_ADDR x1, GM_ADDR x2, GM_ADDR y, GM_ADDR workspace,
                                                            GM_ADDR tiling);
extern "C" void GetTiling(AutofuseTilingData &tiling_data);

class E2E_ConcatSmallTailDim_Code : public testing::Test,
                                    public testing::WithParamInterface<std::pair<std::vector<int>, std::vector<int>>> {
};

TEST_P(E2E_ConcatSmallTailDim_Code, ConcatSmallTailDim) {
  ...
  memset_s(y, test_size_y * sizeof(float), 1, test_size_y * sizeof(float));
  ...
  ICPU_RUN_KF(concat_small_tail_dim, tiling_data.block_dim, (uint8_t *)x1, (uint8_t *)x2, (uint8_t *)y, nullptr,
              (uint8_t *)&tiling_data);
...
// INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2E_ConcatSmallTailDim_Code,
//     ::testing::Values(
//       std::pair<std::vector<int>, std::vector<int>>{{31, 1, 31, 2}, {1 , 1, 31}},
//       std::pair<std::vector<int>, std::vector<int>>{{192, 2, 192, 1}, {6 , 1, 32}}));

INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2E_ConcatSmallTailDim_Code,
                         ::testing::Values(std::pair<std::vector<int>, std::vector<int>>{{65, 1, 65, 2}, {3, 1, 31}}));
