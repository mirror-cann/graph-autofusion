...
void PowInt32_BeforeAutofuse(af::AscGraph &graph, ge::DataType data_type) {
  ...
  *load2.y.strides = {One};
  ...
  pow.attr.tmp_buffers = {{{af::Symbol(8192), -1}, MemAttr(), 0}};
...
void PowInt32_AfterInferOutput(af::AscGraph &graph, ge::DataType data_type) {
...
void PowInt32_AfterGetApiInfo(af::AscGraph &graph) {
  auto x1 = graph.FindNode("x1");
...
void PowInt32_AfterScheduler(af::AscGraph &graph) {
  ...
  pow->outputs[0].attr.vectorized_axis = {
      z0t->id,
  };
  pow->outputs[0].attr.vectorized_strides = {
      One,
  };
...
void PowInt32_AfterQueBufAlloc(af::AscGraph &graph) {
  int tensor_id = 0;
  ...
  pow->outputs[0].attr.buf.id = af::kIdNone;
  ...
  pow->outputs[0].attr.opt.ref_tensor = af::kIdNone;
...
void PowInt32_AfterAutofuse(af::AscGraph &graph, std::vector<af::AscGraph> &impl_graphs, ge::DataType data_type) {
  PowInt32_BeforeAutofuse(graph, data_type);
  PowInt32_AfterInferOutput(graph, data_type);

  impl_graphs.push_back(af::AscGraph("pow_int32_general_0_nil_0_nil"));
