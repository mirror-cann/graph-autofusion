...
void GatherAbsStore_BeforeAutofuse(af::AscGraph &graph) {
  ...
  *gather.y.strides = {s2, One};
  gather.attr.tmp_buffers = {{{af::Symbol(81920), -1}, MemAttr(), 0}};
...
void GatherAbsStore_AfterQueBufAlloc(af::AscGraph &graph) {
  ...
  gather->outputs[0].attr.que.id = af::kIdNone;
  gather->outputs[0].attr.mem.reuse_id = 0;
  gather->outputs[0].attr.que.depth = af::kIdNone;
  gather->outputs[0].attr.que.buf_num = af::kIdNone;
