...
void LoadLogicalOrUbScalarStore_BeforeAutofuse(af::AscGraph &graph, ge::DataType in_data_type, ge::DataType out_data_type) {
  ...
  *logical_or_op.y.strides = {s1*s2, s2, One};
  logical_or_op.attr.tmp_buffers = {{{af::Symbol(8192), -1}, MemAttr(), 0}};
...
void LoadLogicalOrUbScalarStore_AfterInferOutput(af::AscGraph &graph, ge::DataType in_data_type, ge::DataType out_data_type) {
...
void LoadLogicalOrUbScalarStore_AfterGetApiInfo(af::AscGraph &graph) {
    auto x1 = graph.FindNode("x1");
...
void LoadLogicalOrUbScalarDivStore_AfterScheduler(af::AscGraph &graph) {
...
void LoadLogicalOrUbScalarStore_AfterQueBufAlloc(af::AscGraph &graph) {
    auto x1 = graph.FindNode("x1");
    ...
    logical_or->outputs[0].attr.buf.id = af::kIdNone;
    ...
    logical_or->outputs[0].attr.opt.ref_tensor = af::kIdNone;
