...
extern "C" __global__ __aicore__ void load_leakyrelu_store(GM_ADDR x, GM_ADDR y, GM_ADDR workspace, GM_ADDR tiling);
extern "C" void GetTiling(AutofuseTilingData &tiling_data);

class E2E_LoadLeakyreluStore_Code : public testing::Test,
                                    public testing::WithParamInterface<std::pair<std::vector<int>, std::vector<int>>> {
};

TEST_P(E2E_LoadLeakyreluStore_Code, CalculateCorrect) {
  ...
  for (int i = 0; i < test_size; i++) {
    ...
    if (x[i] >= 0.0) {
      ...
      expect[i] = x[i] * 0.8;  // 构图的时候, 给leakyrelu打的属性就是0.8
    }
  }
  ...
  ICPU_RUN_KF(load_leakyrelu_store, tiling_data.block_dim, (uint8_t *)x, (uint8_t *)y, nullptr,
              (uint8_t *)&tiling_data);
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2E_LoadLeakyreluStore_Code,
