...
extern "C" __global__ __aicore__ void broadcast_multi_axes(GM_ADDR x, GM_ADDR y, GM_ADDR workspace,
                                                           GM_ADDR tiling_data);
...
TEST_P(E2E_Broadcast_Multi_Axes, broadcast_multi_axes_a1a1) {
  ...
  ICPU_RUN_KF(broadcast_multi_axes, tiling_data.block_dim, (uint8_t *)x0, (uint8_t *)y, nullptr,
              (uint8_t *)&tiling_data);
  // Count difference
...
INSTANTIATE_TEST_SUITE_P(CalcBroadCastMultiAxes, E2E_Broadcast_Multi_Axes,
                         ::testing::Values(std::vector<int>{1, 2, 8, 2, 8}));

extern "C" __global__ __aicore__ void broadcast_multi_axes_bab(GM_ADDR x, GM_ADDR y, GM_ADDR workspace,
                                                               GM_ADDR tiling_data);
...
TEST_P(E2E_Broadcast_Multi_Axes_BAB, broadcast_multi_axes_11a1) {
  ...
  AscendC::SetKernelMode(KernelMode::AIV_MODE);
  ICPU_RUN_KF(broadcast_multi_axes_bab, tiling_data.block_dim, (uint8_t *)x0, (uint8_t *)y, nullptr,
              (uint8_t *)&tiling_data);
  // Count difference
  uint32_t diff_count = 0;
  for (int i = 0; i < s0 * s1 * s2 * s3 * s4; i++) {
