...
extern "C" __global__ __aicore__ void discrete_store_merge_axis(GM_ADDR x, GM_ADDR y, GM_ADDR workspace,
                                                                GM_ADDR tiling);
extern "C" void GetTiling(AutofuseTilingData &tiling_data);

class E2E_DiscreteStoreMergeAxis_Code
    : public testing::Test,
...
TEST_P(E2E_DiscreteStoreMergeAxis_Code, CalculateCorrect) {
        ...
        for (int m = 0; m < test_shape[3]; m++) {
          for (int n = 0; n < test_shape[4]; n++) {
            auto index = i * (test_shape[1] * test_shape[2] * test_shape[3] * test_shape[4]) +
                         j * (test_shape[2] * test_shape[3] * test_shape[4]) + k * (test_shape[3] * test_shape[4]) +
                         m * (test_shape[4]) + n;
            x[index] = n;
          }
        }
  ...
  ICPU_RUN_KF(discrete_store_merge_axis, tiling_data.block_dim, (uint8_t *)x, (uint8_t *)y, nullptr,
              (uint8_t *)&tiling_data);
...
INSTANTIATE_TEST_SUITE_P(
    CalcWithDifferentShape, E2E_DiscreteStoreMergeAxis_Code,
    ::testing::Values(std::pair<std::vector<int>, std::vector<int>>{{32 * 5, 2, 2, 4, 9}, {32, 5}},
