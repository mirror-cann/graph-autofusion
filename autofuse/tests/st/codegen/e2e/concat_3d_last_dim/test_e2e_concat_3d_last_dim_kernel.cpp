...
extern "C" __global__ __aicore__ void concat_3d_last_dim(GM_ADDR x1, GM_ADDR x2, GM_ADDR y, GM_ADDR workspace,
                                                         GM_ADDR tiling);
extern "C" void GetTiling(AutofuseTilingData &tiling_data);

class E2E_Concat3DLastDim_Code : public testing::Test,
                                 public testing::WithParamInterface<std::pair<std::vector<int>, std::vector<int>>> {};

TEST_P(E2E_Concat3DLastDim_Code, Concat3DLastDim) {
  auto [test_shape, test_tiling] = GetParam();
  uint32_t block_dim = 1;
  int test_shape_x1[3] = {test_shape[0], test_shape[1], test_shape[2]};
  int test_shape_x2[3] = {test_shape[0], test_shape[1], test_shape[2]};
  ...
  int test_size_y = test_shape_x1[0] * test_shape_x1[1] * (test_shape_x1[2] + test_shape_x2[2]);
  ...
  for (int i = 0; i < test_shape_x2[0]; i++) {
    for (int j = 0; j < test_shape_x2[1]; j++) {
      for (int k = 0; k < test_shape_x2[2]; k++) {
        x2[i * test_shape_x2[1] * test_shape_x2[2] + j * test_shape_x2[2] + k] = k + test_shape_x1[2];
      }
    }
  }
  ...
  for (int i = 0; i < test_shape_x1[0]; i++) {
    for (int j = 0; j < test_shape_x1[1]; j++) {
      for (int k = 0; k < test_shape_x1[2] + test_shape_x2[2]; k++) {
        expect[i * (test_shape_x1[1]) * (test_shape_x1[2] + test_shape_x2[2]) +
               j * (test_shape_x1[2] + test_shape_x2[2]) + k] = k;
      }
    }
  }
  ...
  tiling_data.s2 = test_shape_x1[2];
  ...
  ICPU_RUN_KF(concat_3d_last_dim, tiling_data.block_dim, (uint8_t *)x1, (uint8_t *)x2, (uint8_t *)y, nullptr,
              (uint8_t *)&tiling_data);
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2E_Concat3DLastDim_Code,
                         ::testing::Values(std::pair<std::vector<int>, std::vector<int>>{{3, 4, 3}, {1, 1, 3}},
                                           std::pair<std::vector<int>, std::vector<int>>{{128, 17, 35}, {16, 1, 8}}));
