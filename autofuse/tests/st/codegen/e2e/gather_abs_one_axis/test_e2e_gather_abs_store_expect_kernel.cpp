...
extern "C" __global__ __aicore__ void gather_abs_store(GM_ADDR x1, GM_ADDR x2, GM_ADDR y, GM_ADDR workspace,
                                                       GM_ADDR tiling);
extern "C" void GetTiling(AutofuseTilingData &tiling_data);

class E2E_GatherAbsStore_Code : public testing::Test,
...
TEST_P(E2E_GatherAbsStore_Code, CalculateCorrect) {
  ...
  ICPU_RUN_KF(gather_abs_store, tiling_data.block_dim, (uint8_t *)x1, (uint8_t *)x2, (uint8_t *)y, nullptr,
              (uint8_t *)&tiling_data);
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2E_GatherAbsStore_Code,
                         ::testing::Values(std::pair<std::vector<int>, std::vector<int>>{{4000, 100, 100}, {}},
                                           // tile tail
                                           std::pair<std::vector<int>, std::vector<int>>{{4000, 100, 100},
                                                                                         {20, 100, 100}}));
