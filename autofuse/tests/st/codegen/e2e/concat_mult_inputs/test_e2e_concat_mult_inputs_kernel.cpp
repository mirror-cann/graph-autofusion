...
extern "C" __global__ __aicore__ void concat_mult_inputs(GM_ADDR x1, GM_ADDR x2, GM_ADDR x3, GM_ADDR x4, GM_ADDR x5,
                                                         GM_ADDR x6, GM_ADDR x7, GM_ADDR y, GM_ADDR workspace,
                                                         GM_ADDR gm_tiling_data);
extern "C" void GetTiling(AutofuseTilingData &tiling_data);

class E2E_ConcatMultInputs_Code : public testing::Test,
...
TEST_P(E2E_ConcatMultInputs_Code, ConcatMultInputs) {
  ...
  int test_size_y = test_shape[0] * (test_shape[1] + test_shape[2] + test_shape[3] + test_shape[4] + test_shape[5] +
                                     test_shape[6] + test_shape[7]);
  ...
  for (int i = 0; i < test_shape[0]; i++) {
    for (int j = 0; j < test_shape[4]; j++) {
      x4[i * test_shape[4] + j] = j + test_shape[1] + test_shape[2] + test_shape[3];
    }
  }
  for (int i = 0; i < test_shape[0]; i++) {
    for (int j = 0; j < test_shape[5]; j++) {
      x5[i * test_shape[5] + j] = j + test_shape[1] + test_shape[2] + test_shape[3] + test_shape[4];
    }
  }
  for (int i = 0; i < test_shape[0]; i++) {
    for (int j = 0; j < test_shape[6]; j++) {
      x6[i * test_shape[6] + j] = j + test_shape[1] + test_shape[2] + test_shape[3] + test_shape[4] + test_shape[5];
    }
  }
  for (int i = 0; i < test_shape[0]; i++) {
    for (int j = 0; j < test_shape[7]; j++) {
      x7[i * test_shape[7] + j] =
          j + test_shape[1] + test_shape[2] + test_shape[3] + test_shape[4] + test_shape[5] + test_shape[6];
    }
  }

  auto y_shape1 =
      test_shape[1] + test_shape[2] + test_shape[3] + test_shape[4] + test_shape[5] + test_shape[6] + test_shape[7];
  for (int i = 0; i < test_shape[0]; i++) {
  ...
  ICPU_RUN_KF(concat_mult_inputs, tiling_data.block_dim, (uint8_t *)x1, (uint8_t *)x2, (uint8_t *)x3, (uint8_t *)x4,
              (uint8_t *)x5, (uint8_t *)x6, (uint8_t *)x7, (uint8_t *)y, nullptr, (uint8_t *)&tiling_data);
...
INSTANTIATE_TEST_SUITE_P(
    CalcWithDifferentShape, E2E_ConcatMultInputs_Code,
    ::testing::Values(std::pair<std::vector<int>, std::vector<int>>{{16, 17, 31, 6, 2, 2, 1, 7}, {1, 1, 16}},
                      ...
                      std::pair<std::vector<int>, std::vector<int>>{{12 * 1 * 31, 1, 3, 65, 77, 10, 1, 2},
                                                                    {12, 1, 31}}));
