...
void LoadLeakyreluStore_BeforeAutofuse(af::AscGraph &graph, ge::DataType in_data_type, ge::DataType out_data_type) {
  ...
  *load.y.strides = {s1 * s2, s2, One};
  ...
  af::ascir_op::LeakyRelu leakyrelu("leakyrelu");
  ...
  *leakyrelu.y.strides = {s1 * s2, s2, One};
  leakyrelu.attr.tmp_buffers = {{{af::Symbol(8192), -1}, MemAttr(), 0}};
...
void LoadLeakyreluStore_AfterInferOutput(af::AscGraph &graph, ge::DataType in_data_type, ge::DataType out_data_type) {
...
void LoadLeakyreluStore_AfterGetApiInfo(af::AscGraph &graph) {
  auto x = graph.FindNode("x");
...
void LoadLeakyreluStore_AfterScheduler(af::AscGraph &graph) {
...
void LoadLeakyreluStore_AfterQueBufAlloc(af::AscGraph &graph) {
  auto x = graph.FindNode("x");
  ...
  leakyrelu->outputs[0].attr.buf.id = af::kIdNone;
  ...
  leakyrelu->outputs[0].attr.opt.ref_tensor = af::kIdNone;
