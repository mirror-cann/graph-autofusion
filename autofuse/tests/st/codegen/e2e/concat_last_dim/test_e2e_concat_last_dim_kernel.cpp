...
extern "C" __global__ __aicore__ void concat_last_dim(GM_ADDR x1, GM_ADDR x2, GM_ADDR y, GM_ADDR workspace,
                                                      GM_ADDR tiling);
extern "C" void GetTiling(AutofuseTilingData &tiling_data);

class E2E_ConcatLastDim_Code : public testing::Test,
                               public testing::WithParamInterface<std::pair<std::vector<int>, std::vector<int>>> {};

TEST_P(E2E_ConcatLastDim_Code, ConcatLastDim) {
  ...
  ICPU_RUN_KF(concat_last_dim, tiling_data.block_dim, (uint8_t *)x1, (uint8_t *)x2, (uint8_t *)y, nullptr,
              (uint8_t *)&tiling_data);
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2E_ConcatLastDim_Code,
                         ::testing::Values(std::pair<std::vector<int>, std::vector<int>>{{31, 17, 31, 6}, {1, 1, 31}},
                                           std::pair<std::vector<int>, std::vector<int>>{{128, 7, 128, 37}, {8, 1, 16}},
                                           std::pair<std::vector<int>, std::vector<int>>{{6146, 2, 6146, 246},
                                                                                         {40, 77, 2}}));
