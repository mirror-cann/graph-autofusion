...
class E2E_Load_Broadcast_Axis_S1_Store : public testing::Test, public testing::WithParamInterface<std::vector<int>> {};
...
TEST_P(E2E_Load_Broadcast_Axis_S1_Store, BroadCast_Axis_s1_fp32) {
  ...
  ICPU_RUN_KF(load_broadcast_axis_s1_store, tiling_data.block_dim, (uint8_t *)x, (uint8_t *)y, nullptr,
              (uint8_t *)&tiling_data);
...
INSTANTIATE_TEST_SUITE_P(M32_K_BlockAlign, E2E_Load_Broadcast_Axis_S1_Store,
                         ::testing::Values(std::vector<int>{16 * 4 * 8, 16}, std::vector<int>{16 * 4 * 16, 16},
                                           std::vector<int>{16 * 4 * 32, 16}, std::vector<int>{16 * 4 * 8, 64},
