...
class E2E_LoadAbsStore_Code : public testing::Test,
...
TEST_P(E2E_LoadAbsStore_Code, CalculateCorrect) {
  ...
  if (test_tiling.size() == 0U) {  // tiling data 来源于tiling函数GetTiling
    ...
    tiling_data.graph0_result0_g0_tiling_data.z0Tb_size = test_tiling[1];
    tiling_data.graph0_result0_g0_tiling_data.z0t_size = test_tiling[2];
  }
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2E_LoadAbsStore_Code,
                         ::testing::Values(
                             std::pair<std::vector<int>, std::vector<int>>{
                                 {(48 * 64 * 4 - 4), 16}, {48, 64, 4}},  // block尾块 尾轴32对齐padding为size32
                             std::pair<std::vector<int>, std::vector<int>>{
                                 {(48 * 64 * 4 - 4), 36}, {48, 64, 4}},  // block尾块 尾轴32对齐padding为size64
                             std::pair<std::vector<int>, std::vector<int>>{{4096, 32}, {48, 1, 86}}  // tile尾块
                             ));
