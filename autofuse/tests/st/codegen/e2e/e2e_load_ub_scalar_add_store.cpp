...
void LoadUbScalarAddStore_BeforeAutofuse(af::AscGraph &graph, ge::DataType in_data_type, ge::DataType out_data_type) {
...
//   *load2.y.strides = {s1*s2, s2, One};
  ...
  *add.y.strides = {s1*s2, s2, One};
  add.attr.tmp_buffers = {{{af::Symbol(8192), -1}, MemAttr(), 0}};
...
void LoadUbScalarAddStore_AfterInferOutput(af::AscGraph &graph, ge::DataType in_data_type, ge::DataType out_data_type) {
...
void LoadUbScalarAddStore_AfterGetApiInfo(af::AscGraph &graph) {
    auto x1 = graph.FindNode("x1");
...
void LoadUbScalarAddStore_AfterScheduler(af::AscGraph &graph) {
...
void LoadUbScalarAddStore_AfterQueBufAlloc(af::AscGraph &graph) {
    auto x1 = graph.FindNode("x1");
