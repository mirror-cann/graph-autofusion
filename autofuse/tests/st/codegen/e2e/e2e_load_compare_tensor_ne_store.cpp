...
void LoadCompareTensorNeStore_BeforeAutofuse(af::AscGraph &graph, ge::DataType data_type) {
  ...
  *load2.y.strides = {One};
  ...
  ne.attr.tmp_buffers = {{{af::Symbol(8192), -1}, MemAttr(), 0}};
...
void LoadCompareTensorNeStore_AfterInferOutput(af::AscGraph &graph, ge::DataType data_type) {
...
void LoadCompareTensorNeStore_AfterGetApiInfo(af::AscGraph &graph) {
  auto x1 = graph.FindNode("x1");
...
void LoadCompareTensorNeStore_AfterScheduler(af::AscGraph &graph) {
...
void LoadCompareTensorNeStore_AfterQueBufAlloc(af::AscGraph &graph) {
  int tensor_id = 0;
