...
void LoadRemovePadStore_BeforeAutofuse(af::AscGraph &graph, ge::DataType in_data_type) {
  ...
  *load.y.strides = {s1*s2, s2, One};
  ...
  af::ascir_op::RemovePad removepad("removepad");
  ...
  *removepad.y.strides = {s1*s2, s2, One};
  removepad.attr.tmp_buffers = {{{af::Symbol(8192), -1}, MemAttr(), 0}};
...
void LoadRemovePadStore_AfterQueBufAlloc(af::AscGraph &graph) {
    ...
    removepad->outputs[0].attr.buf.id = af::kIdNone;
    ...
    removepad->outputs[0].attr.opt.ref_tensor = af::kIdNone;
    removepad->outputs[0].attr.opt.merge_scope = af::kIdNone;
