...
int main() {
  ...
  ICPU_RUN_KF(autofuse_pointwise_0__abs__add, tiling_data.block_dim, (uint8_t *)x1, (uint8_t *)x2, (uint8_t *)y,
  ...
  if (diff_count == 0) {
    ...
    std::cout << "test kernel failed, test_size: " << test_size << ", diff_count: " << diff_count << std::endl;
    return -1;
  }
}
