...
void ApplySchedulerTransform(ge::AscGraph &graph, const char *name, int64_t z1T, int64_t z1t, int64_t z0z1T,
  ...
  graph.ApplySplit(node, z0z1TB, z0z1Tb);
...
void AutoFuseAfterScheduler(ge::AscGraph &graph) {
  ...
  auto repeats = std::initializer_list<Expr>{axis_0_size, axis_1_size / z1t_size, z1t_size};
  auto init_strides = std::initializer_list<Expr>{ge::Symbol(0), z1t_size, af::sym::kSymbolOne};

  for (const auto *name : {"data", "load"}) {
    auto node = graph.FindNode(name);
    graph.ApplySplit(node, z1T->id, z1t->id);
    graph.ApplyMerge(node, z0z1T.id);
    graph.ApplySplit(node, z0z1TB->id, z0z1Tb->id);
    node->attr.sched.loop_axis = z0z1Tb->id;
    node->outputs[0].attr.axis = axis_list;
    node->outputs[0].attr.repeats = repeats;
    node->outputs[0].attr.strides = init_strides;
    node->outputs[0].attr.vectorized_axis = {z1t->id};
  }

  for (const auto *name : {"data1", "load1", "add", "store"}) {
    ApplySchedulerTransform(graph, name, z1T->id, z1t->id, z0z1T.id, z0z1TB->id, z0z1Tb->id, z0z1Tb->id, {z1t->id});
  }
}
...
TEST_F(TestAutoFuse, casev2) {
  ...
  EXPECT_EQ(res, true);
  ret = std::system(std::string("cp ").append(TILING_DATA_DIR).append("/tiling_func_main_auto_fuse.cpp ./ -f").c_str());
