...
std::string expect_codes = R"(#include <cstdint>
#include <iostream>
...
Node *VisitedNode::GetVarVal(uint64_t *vars)
{
    ...
    for (int32_t i = 0; i < depth; i++)
    {
        if (!cur_node->next_var)
        {
            ...
            if (new_node == nullptr)
            {
                OP_LOG(op_name, "Create new_node failed!");
                return nullptr;
            }
            if (new_node != nullptr) {
...
bool Result::AddVarVal(uint64_t *vars, double obj, double cons)
{
    ...
    if (new_vars == nullptr)
    {
        OP_LOG(op_name, "Create new_vars failed!");
        return false;
    }
    if (rec_num == 0)
    {
    ...
    if (temp == nullptr)
    {
        OP_LOG(op_name, "Create temp failed!");
        return false;
    }
...
bool GeneralSolver::SetSolverInput(const SolverInput &input)
{
    ...
    {
        OP_LOG(op_name, "Create visited_node_ failed!");
        return false;
    }
...
bool GeneralSolver::Init(const SolverInput &input)
{
    ...
    if (result_ == nullptr)
    {
        OP_LOG(op_name, "Create result_ failed!");
        return false;
    }
    return true;
}
...
bool GeneralSolver::TryLocate(int32_t idx, double init_obj, Locality &best_locality)
{
    ...
    if (update_direction != UpdateDirection::NONE)
    {
        ...
        {
            OP_LOG(op_name, "Create update_info failed!");
            return false;
        }
