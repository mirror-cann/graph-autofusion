...
class Result {
 ...
 private:
  bool InsertSorted(VarVal **temp, uint64_t rec_num, VarVal *new_vars, double obj, double cons);
  uint32_t top_n_{0};
  uint32_t var_num_{0};
...
class GeneralSolver {
 ...
 private:
  ...
  bool TryTune(int32_t idx, UpdateDirection update_direction, double init_obj, double init_cons,
               TunePriority &best_priority);
...
inline uint64_t Bound(uint64_t upper_bound, uint64_t lower_bound, uint64_t val, uint64_t step,
