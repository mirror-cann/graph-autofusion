...
#define OP_LOG(log)
  ...
  class L2TileSolver {
   ...
   private:
    ...
    bool CheckAllSolved();
    void UpdateBestSolution(uint32_t *best_val, uint64_t &max_l2_use);
    void ReduceToFitL2(uint32_t l2_size);
    void InitTileArrays(uint32_t *best_val, uint32_t *ori_val);
    void ApplyBestResult(uint32_t *best_val);
    void AdjustSingleTile();
  };
   ...
   * false，表示输入无效。 最后遍历 l2_vars 指针数组中的所有 L2Var 结构。对于每个结构，检查 align、base_val 和 max_value
  ...
  bool L2TileSolver::CheckSolvable() {
    ...
    if (GetL2Use() > input_.l2_size) {
      OP_LOG("No solution, l2 size is too small");
      return false;
    }
    return true;
  }
  ...
  bool L2TileSolver::CheckAllSolved() {
  ...
  void L2TileSolver::UpdateBestSolution(uint32_t * best_val, uint64_t & max_l2_use) {
  ...
  void L2TileSolver::HandleClash(uint32_t loop_id, uint32_t * ori_val, uint32_t * best_val, uint64_t & max_l2_use) {
    ...
    for (uint32_t i = max_blocknum; i >= 1u; i--) {
      ...
      if (loop_id == input_.size - 1) {
        UpdateBestSolution(best_val, max_l2_use);
      } else {
   ...
   */
  void L2TileSolver::ReduceToFitL2(uint32_t l2_size) {
  ...
  void L2TileSolver::InitTileArrays(uint32_t * best_val, uint32_t * ori_val) {
    for (uint32_t i = 0; i < input_.size; i++) {
      auto &var = input_.l2_vars[i];
  ...
  void L2TileSolver::ApplyBestResult(uint32_t * best_val) {
    for (uint32_t i = 0; i < input_.size; i++) {
      auto &var = input_.l2_vars[i];
  ...
  void L2TileSolver::AdjustSingleTile() {
    for (uint32_t i = 0; i < input_.size; i++) {
      auto &var = input_.l2_vars[i];
  ...
  bool L2TileSolver::Run() {
    if (!CheckInput()) {
      OP_LOG("Check input failed");
      return false;
    }
    if (!CheckSolvable()) {
      OP_LOG("Check Solvable failed");
      return false;
    }
    InitInput();
