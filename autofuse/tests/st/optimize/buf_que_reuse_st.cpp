...
af::AscGraph CreatSomeInputFusedConcatGraph(const std::string &name) {
  auto s0 = Sym("s0");
  auto s1 = Sym("s1");
...
TEST_F(BufQueReuseSt, TestVecCanReuseTque) {
  ...
  EXPECT_EQ(sigmoid->outputs[0].attr.mem.alloc_type, af::AllocType::kAllocTypeQueue);
  EXPECT_EQ(sigmoid->outputs[0].attr.que.id, que_id);                               // sigmoid can reuse load1
  EXPECT_EQ(max->outputs[0].attr.mem.alloc_type, af::AllocType::kAllocTypeBuffer);  // different loop cannot reuse que
}
...
TEST_F(BufQueReuseSt, TestInplaceChainVecCanReuseTque) {
  ...
  EXPECT_EQ(sigmoid0->outputs[0].attr.mem.alloc_type, af::AllocType::kAllocTypeQueue);
  EXPECT_EQ(sigmoid0->outputs[0].attr.que.id, que_id);
  EXPECT_NE(sigmoid0->outputs[0].attr.mem.reuse_id, af::kIdNone);

  int64_t vecout_que_id = abs1->outputs[0].attr.que.id;
  EXPECT_EQ(add->outputs[0].attr.mem.alloc_type, af::AllocType::kAllocTypeQueue);  // inplace reuse
  EXPECT_EQ(add->outputs[0].attr.que.id, vecout_que_id);
  EXPECT_NE(add->outputs[0].attr.mem.reuse_id, af::kIdNone);

  EXPECT_EQ(mul->outputs[0].attr.mem.alloc_type, af::AllocType::kAllocTypeQueue);  // inplace reuse
  EXPECT_EQ(mul->outputs[0].attr.que.id, vecout_que_id);
  EXPECT_NE(mul->outputs[0].attr.mem.reuse_id, af::kIdNone);

  EXPECT_EQ(abs1->outputs[0].attr.mem.alloc_type, af::AllocType::kAllocTypeQueue);  // inplace reuse
  EXPECT_EQ(abs1->outputs[0].attr.que.id, vecout_que_id);
  EXPECT_NE(abs1->outputs[0].attr.mem.reuse_id, af::kIdNone);
}
...
TEST_F(BufQueReuseSt, TestVecoutCanInplaceReuseCalc) {
  ...
  // tque share
  EXPECT_EQ(load0_node->outputs[0].attr.mem.alloc_type, af::AllocType::kAllocTypeQueue);
  ...
  EXPECT_EQ(load2_node->outputs[0].attr.mem.alloc_type, af::AllocType::kAllocTypeQueue);
  ...
  EXPECT_EQ(add0_node->outputs[0].attr.mem.alloc_type, af::AllocType::kAllocTypeQueue);
  EXPECT_EQ(pow0_node->outputs[0].attr.mem.alloc_type, af::AllocType::kAllocTypeQueue);
