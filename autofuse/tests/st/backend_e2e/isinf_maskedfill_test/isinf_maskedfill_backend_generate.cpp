...
class TestBackendIsInfMaskedFillE2e : public testing::Test {};

TEST_F(TestBackendIsInfMaskedFillE2e, IsInfMaskedFillE2eCodegen) {
...
#define GET_TILING_DATA(t, tiling)  AutofuseTilingData t = *(AutofuseTilingData*)tiling;
  ...
  auto graph = ascir::ShareGraph::IsInfMaskedFillFusedGraph(3);
  ASSERT_NE(graph, nullptr) << "Failed to create IsInfMaskedFillFusedGraph";
  std::cout << "KERNEL_SRC_LIST=" << KERNEL_SRC_LIST << std::endl;
  std::vector<std::string> parts = splitString(KERNEL_SRC_LIST, ':');
  ASSERT_GE(parts.size(), 3U) << "KERNEL_SRC_LIST must contain at least 2 ':' separators";
