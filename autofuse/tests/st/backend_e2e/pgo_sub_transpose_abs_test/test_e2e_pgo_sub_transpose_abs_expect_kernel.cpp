...
extern "C" int64_t StubPgoGetProfiling(PgoTensorArgs *tensor_args, void *stream, uint32_t workspaceSize,
  ...
  *cost_time = prof_time;
  ...
  ICPU_RUN_KF(sub_transpose_abs_test, tiling_data->block_dim, (uint8_t *)input0, (uint8_t *)input1, (uint8_t *)output0,
...
TEST_P(E2E_BackendSubTransposeAbs_Code, PgoByCoreNum) {
  ...
  int result = PgoTilingSearch("./tests/st/backend_e2e/pgo_sub_transpose_abs_test/search.txt",
                               "./tests/st/backend_e2e/pgo_sub_transpose_abs_test/config.txt", &tiling_data, &ws_size,
                               &block_dim, &g_no_limit_res, &tensor_args, nullptr, StubPgoGetProfiling,
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2E_BackendSubTransposeAbs_Code,
                         ::testing::Values(std::vector<int>{32, 64, 128}
                                           // 用例输入的维度需要与构图接口的dims_size及hint值匹配
                                           ));
