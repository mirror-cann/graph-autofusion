...
namespace att {
ModelInfo CreateModelInfo(const uint32_t m_align = 1u, const ge::ExprType expr_type = ge::ExprType::kExprVariable);
...
class TestTilingScheduleConfigTable : public TilingScheduleConfigTable {
 public:
  ...
  [[nodiscard]] bool IsEnableCacheLineCheck() const override {
    return enable_cache_line_check_;
  }
...
const TilingScheduleConfigTable *RegisterTestScheduleTable(uint32_t cache_line_size,
                                                           bool enable_cache_line_check = true);

ModelInfo CreateGroupParallelCacheLineModelInfo(size_t group_id, uint32_t tiling_case_id, const Expr &cache_line_expr,
