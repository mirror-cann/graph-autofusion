...
namespace af{
namespace ascir{
extern std::vector<std::unique_ptr<af::TmpBufDesc>> CalcZerosLikeTmpSize(const af::AscNode &node);
...
TEST_F(CalcZerosLikeTmpSizeTest, CalcZerosLikeTmpSize_ShouldReturnCorrectSize_WhenNodelsValid)
{
    ...
    af::ascir_op::ZerosLike zeroslike("zeroslike");
    ...
    *y.y.strides = {s1, Symbol(1)};
    ...
    std::shared_ptr<af::AscNode> node = graph.FindNode("zeroslike");
    std::vector<std::unique_ptr<af::TmpBufDesc>> result = CalcZerosLikeTmpSize(*node);
    ASSERT_EQ(result.size(), 1);
    ASSERT_EQ(result[0]->size, sym::Max(af::Symbol(4)*s1*s0, af::Symbol(8192)));
