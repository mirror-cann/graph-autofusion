...
namespace af {
namespace ascir {
extern std::vector<std::unique_ptr<af::TmpBufDesc>> CalcLogicalOrTmpSize(const af::AscNode &node);
...
TEST_F(CalcLogicalOrTmpSizeTest, CalcLogicalOrTmpSize_ShouldReturnCorrectSize_WhenNodelsValid) {
  ...
  std::vector<std::unique_ptr<af::TmpBufDesc>> result = CalcLogicalOrTmpSize(*node);
