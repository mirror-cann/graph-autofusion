...
namespace af{
namespace ascir{
extern std::vector<std::unique_ptr<af::TmpBufDesc>> CalcDivTmpSize(const af::AscNode &node);
...
TEST_F(CalcDivTmpSizeTest, CalcDivTmpSize_ShouldReturnCorrectSize_WhenNodeIsUbScalar)
{
  ...
  *load2.y.vectorized_axis = {z0.id, z1.id};
  ...
  div.y.dtype = af::DT_FLOAT;
  *div.y.axis = {z0.id, z1.id};
  *div.y.repeats = {s0, s1};
  *div.y.strides = {s1, Symbol(1)};
  std::shared_ptr<af::AscNode> node = graph.FindNode("div");
  std::vector<std::unique_ptr<af::TmpBufDesc>> result = CalcDivTmpSize(*node);
