...
void ClipByValueFloat_BeforeAutofuse(af::AscGraph &graph, codegen::Tiler &tiler, ge::DataType data_type) {
...
void ClipByValueFloat_AfterAutofuse(af::AscGraph &graph, std::vector<af::AscGraph> &impl_graphs, codegen::Tiler &tiler,
...
TEST(CodegenKernel, ClipByValueWithSecondInputAndThirdInputAreScalar) {
  ...
  *load_op.y.strides = {s1, One};
  ...
  call.Generate(tpipe, vector<af::AxisId>{}, result);
  EXPECT_EQ(result,
            std::string{"ClipByValue(local_3[0], local_0[0], scalar_1, scalar_2, tmp_buf_0, local_0_actual_size);\n"});
}
