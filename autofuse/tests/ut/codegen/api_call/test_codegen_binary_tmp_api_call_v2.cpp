...
TEST(CodegenKernel, BinaryTmpApicallV2) {
  ...
  *mod_op.y.strides = {s1, One};
  ...
  mod->attr.api.compute_type = af::ComputeType::kComputeElewise;
  mod->attr.api.type = af::ApiType::kAPITypeCompute;
  mod->attr.api.unit = af::ComputeUnit::kUnitVector;
  mod->attr.sched.loop_axis = z0.id;
  mod->attr.tmp_buffers = {{{af::Symbol(8192), -1}, af::MemAttr(), 0}};
  mod->outputs[0].attr.vectorized_axis = {z1.id};
  mod->outputs[0].attr.vectorized_strides = {One};
  mod->outputs[0].attr.dtype = ge::DT_INT16;
  mod->outputs[0].attr.mem.position = af::Position::kPositionVecOut;
  mod->outputs[0].attr.mem.tensor_id = 3;
  mod->outputs[0].attr.mem.alloc_type = af::AllocType::kAllocTypeQueue;
  mod->outputs[0].attr.que.id = 2;
  mod->outputs[0].attr.opt.merge_scope = af::kIdNone;
