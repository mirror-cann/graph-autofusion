...
inline uint32_t ALIGN_UP(uint32_t origin_size, uint32_t align_num) {
  return (0 == (origin_size & (align_num - 1))) ? origin_size
                                                : (origin_size + align_num - (origin_size & (align_num - 1)));
}
...
void GetConfusionTranspose2103TilingInfo(const ShapeInfo srcShape, const uint32_t stackBufferSize,
  ...
  uint32_t srcStride = 0;
  uint32_t dstStride =
      (srcShape.originalShape[0] * srcShape.originalShape[1] * lastDimAlign - lastDimAlign) / blockSize;
...
__aicore__ inline void GetConfusionTransposeTilingInfo(ShapeInfo srcShape, uint32_t stackBufferSize, uint32_t typeSize,
...
static auto CreateTransposeKernel() {
  return [](T *x, T *y, const ShapeInfo &srcShape, AutoFuseTransposeType transposetypeIn) {
    ...
    if (static_cast<AutoFuseTransposeType>(transposetypeIn) == AutoFuseTransposeType::TRANSPOSE_ND2ND_ONLY) {
      ...
      dstShapeSize =
          srcShape.shape[0] * srcShape.shape[3] * srcShape.shape[2] * ALIGN_UP(srcShape.shape[1], BLOCK_CUBE);
    } else {
    ...
    if (static_cast<AutoFuseTransposeType>(transposetypeIn) == AutoFuseTransposeType::TRANSPOSE_ND2ND_ONLY) {
      ...
      GmToUbGeneral(inPutTensor, x, 1, srcShape.shape[0], srcShape.shape[1], srcShape.shape[2],
                    ALIGN_UP(srcShape.shape[2], 16));
    } else if (static_cast<AutoFuseTransposeType>(transposetypeIn) == AutoFuseTransposeType::TRANSPOSE_ND2ND_210) {
      GmToUbGeneral(inPutTensor, x, 1, srcShape.shape[0], srcShape.shape[1], srcShape.shape[2],
                    ALIGN_UP(srcShape.shape[2], 16));
    } else if (static_cast<AutoFuseTransposeType>(transposetypeIn) == AutoFuseTransposeType::TRANSPOSE_ND2ND_0321) {
      GmToUbGeneral(inPutTensor, x, srcShape.shape[0], srcShape.shape[1], srcShape.shape[2], srcShape.shape[3],
                    ALIGN_UP(srcShape.shape[3], 16));
    } else {
    ...
    if (static_cast<AutoFuseTransposeType>(transposetypeIn) == AutoFuseTransposeType::TRANSPOSE_ND2ND_ONLY) {
      UbToGmGeneral(y, outPutTensor, 1, 1, srcShape.shape[1], srcShape.shape[0],
                    ALIGN_UP(srcShape.shape[0], BLOCK_CUBE));
    } else if (static_cast<AutoFuseTransposeType>(transposetypeIn) == AutoFuseTransposeType::TRANSPOSE_ND2ND_021) {
      UbToGmGeneral(y, outPutTensor, 1, srcShape.shape[0], srcShape.shape[2], srcShape.shape[1],
                    ALIGN_UP(srcShape.shape[1], BLOCK_CUBE));
    } else if (static_cast<AutoFuseTransposeType>(transposetypeIn) == AutoFuseTransposeType::TRANSPOSE_ND2ND_210) {
      UbToGmGeneral(y, outPutTensor, 1, srcShape.shape[2], srcShape.shape[1], srcShape.shape[0],
                    ALIGN_UP(srcShape.shape[0], BLOCK_CUBE));
    } else if (static_cast<AutoFuseTransposeType>(transposetypeIn) == AutoFuseTransposeType::TRANSPOSE_ND2ND_0321) {
      UbToGmGeneral(y, outPutTensor, srcShape.shape[0], srcShape.shape[3], srcShape.shape[2], srcShape.shape[1],
                    ALIGN_UP(srcShape.shape[1], BLOCK_CUBE));
    } else {
...
INSTANTIATE_TEST_SUITE_P(
    CalcWithDifferentShapeHalf, TestApiTranspose102Half,
...
INSTANTIATE_TEST_SUITE_P(
    CalcWithDifferentShapeHalf, TestApiTranspose102Float,
...
// 实例化测试用例
INSTANTIATE_TEST_SUITE_P(
    CalcWithDifferentShape, TestApiTranspose0213Half,
...
INSTANTIATE_TEST_SUITE_P(
    CalcWithDifferentShape, TestApiTranspose0213Float,
...
class TestApiTranspose2103Base : public testing::Test {
 protected:
  void RunTest(const std::vector<int> &param) {
    ...
    AscendC::SetKernelMode(KernelMode::AIV_MODE);
    if constexpr (std::is_same_v<T, half>) {
      ICPU_RUN_KF(kernel_perm2103_half, 1, output_gm, input_gm, srcShapeInfo,
                  AutoFuseTransposeType::TRANSPOSE_ND2ND_2103);
    } else {
      ICPU_RUN_KF(kernel_perm2103_float, 1, output_gm, input_gm, srcShapeInfo,
                  AutoFuseTransposeType::TRANSPOSE_ND2ND_2103);
    }
...
INSTANTIATE_TEST_SUITE_P(
    CalcWithDifferentShape, TestApiTranspose2103Half,
...
INSTANTIATE_TEST_SUITE_P(
    CalcWithDifferentShape, TestApiTranspose2103Float,
