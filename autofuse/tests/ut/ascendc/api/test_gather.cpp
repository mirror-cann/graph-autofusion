...
void TestGatherExtendLastAxis(std::vector<uint32_t> &input1_shape, std::vector<uint32_t> &input2_shape,
  ...
  // 构造Api调用函数
  auto kernel = [](uint32_t param_last_axis_size, uint32_t vectorized_axis_size, T1 *x1, T2 *x2, T1 *y) {
    TPipe pipe;
