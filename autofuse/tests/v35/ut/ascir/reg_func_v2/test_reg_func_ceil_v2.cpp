...
namespace af{
namespace ascir{
extern std::vector<std::unique_ptr<af::TmpBufDesc>> CalcCeilTmpSizeV2(const af::AscNode &node);
...
class CalcCeilTmpSizeV2Test:public::testing::Test{
protected:
    void SetUp() override{}
    void TearDown() override{}
};

TEST_F(CalcCeilTmpSizeV2Test, CalcCeilTmpSize_ShouldReturnCorrectSize_WhenInputsIsFLOAT)
{
    ...
    *load1.y.vectorized_axis = {z0.id, zo_s_0.id};
    ...
    ceil.x = load1.y;
    ceil.attr.sched.axis = {z0.id, zo_s_0.id};
    ceil.y.dtype = af::DT_FLOAT;
    *ceil.y.axis = {z0.id, zo_s_0.id};
    *ceil.y.repeats = {s0, s1};
    *ceil.y.strides = {s1, Symbol(1)};
    ...
    *y.y.strides = {s1, Symbol(1)};
    ...
    std::shared_ptr<af::AscNode> node = graph.FindNode("ceil");
    node->inputs[0].attr.vectorized_strides = {s1, Symbol(1)};
    std::vector<std::unique_ptr<af::TmpBufDesc>> result = CalcCeilTmpSizeV2(*node);
    ASSERT_EQ(result.size(), 1);
    ASSERT_EQ(result[0]->size, Symbol(256));
    ASSERT_EQ(result[0]->life_time_axis_id, -1);
}

TEST_F(CalcCeilTmpSizeV2Test, CalcCeilTmpSize_ShouldReturnCorrectSize_WhenInputsIsFLOAT16)
{
    af::AscGraph graph("test");
    auto s0 = graph.CreateSizeVar("s0");
    auto s1 = graph.CreateSizeVar("s1");
    ...
    *y.y.strides = {s1, Symbol(1)};
    ...
    std::vector<std::unique_ptr<af::TmpBufDesc>> result = CalcCeilTmpSizeV2(*node);
    ASSERT_EQ(result.size(), 1);
    ASSERT_EQ(result[0]->size, Symbol(512));
    ASSERT_EQ(result[0]->life_time_axis_id, -1);
}
} // namespace ascir
} // namespace ge
