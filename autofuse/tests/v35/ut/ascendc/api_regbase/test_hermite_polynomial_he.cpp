...
template <typename T>
struct HermitePolynomialHeInputParam : public UnaryInputParam<T> {
  int32_t n{};
};

// 概率学家埃尔米特多项式 He_n(x) 递推参考实现：
// He_0 = 1, He_1 = x, He_m = x·He_{m-1} - (m-1)·He_{m-2}
static double CalcHermiteHeExpectedValue(double x, int32_t n) {
  if (n < 0) {
    return 0.0;
  }
  if (n > 128) {
...
class TestRegbaseApiHermitePolynomialHeUT : public testing::Test {
 protected:
  static constexpr double kDefaultTolerance = 1e-4;
  static constexpr double kRecursiveTolerance = 5e-3;

  template <typename T>
  static void InvokeTensorTensorKernel(HermitePolynomialHeInputParam<T> &param) {
    TPipe tpipe;
    ...
    HermitePolynomialHeExtend(l_y, l_x, param.n, l_tmp, param.size);
    UbToGm(param.y, l_y, param.size);
  }

  template <typename T>
  static void InvokeTensorTensorTensorKernel(HermitePolynomialHeInputParam<T> &param) {
    TPipe tpipe;
    TBuf<TPosition::VECCALC> xbuf;
    TBuf<TPosition::VECCALC> ybuf;
    ...
    HermitePolynomialHeExtend(l_y, l_x, l_n, l_tmp, param.size);
    UbToGm(param.y, l_y, param.size);
  }
  ...
  template <typename T>
  static void CreateTensorInput(HermitePolynomialHeInputParam<T> &param) {
    ...
    for (uint32_t i = 0; i < param.size; ++i) {
      ...
      param.exp[i] = static_cast<T>(CalcHermiteHeExpectedValue(static_cast<double>(param.x1[i]), param.n));
    }
  }

  template <typename T>
  static uint32_t Valid(HermitePolynomialHeInputParam<T> &param) {
  ...
  template <typename T>
  static void HermitePolynomialHeTest(uint32_t size, int32_t n) {
    HermitePolynomialHeInputParam<T> param{};
    param.size = size;
    param.n = n;
  ...
  template <typename T>
  static void HermitePolynomialHeTensorNTest(uint32_t size, int32_t n) {
    HermitePolynomialHeInputParam<T> param{};
    param.size = size;
    param.n = n;
    CreateTensorInput(param);
...
TEST_F(TestRegbaseApiHermitePolynomialHeUT, HermitePolynomialHeNegativeOrder_TensorTensor_Test) {
  HermitePolynomialHeTest<float>(32U, -1);
}

TEST_F(TestRegbaseApiHermitePolynomialHeUT, HermitePolynomialHeZeroOrder_TensorTensor_Test) {
  HermitePolynomialHeTest<float>(32U, 0);
}

TEST_F(TestRegbaseApiHermitePolynomialHeUT, HermitePolynomialHeLinear_TensorTensor_Test) {
  HermitePolynomialHeTest<float>(9U, 1);
}

TEST_F(TestRegbaseApiHermitePolynomialHeUT, HermitePolynomialHeOrder2_TensorTensor_Test) {
  HermitePolynomialHeTest<float>(32U, 2);
}

TEST_F(TestRegbaseApiHermitePolynomialHeUT, HermitePolynomialHeOrder4_TensorTensor_Test) {
  HermitePolynomialHeTest<float>(65U, 4);
}

TEST_F(TestRegbaseApiHermitePolynomialHeUT, HermitePolynomialHeOrder6_TensorTensor_Test) {
  HermitePolynomialHeTest<float>(6U, 6);
}

TEST_F(TestRegbaseApiHermitePolynomialHeUT, HermitePolynomialHeLimit_TensorTensor_Test) {
  HermitePolynomialHeTest<float>(32U, 128);
}

TEST_F(TestRegbaseApiHermitePolynomialHeUT, HermitePolynomialHeOverLimit_TensorTensor_Test) {
  HermitePolynomialHeTest<float>(32U, 129);
}

TEST_F(TestRegbaseApiHermitePolynomialHeUT, HermitePolynomialHeOrder4_TensorTensorTensor_Test) {
  HermitePolynomialHeTensorNTest<float>(65U, 4);
}
