...
class TestRegbaseApiBesselY0UT : public testing::Test {
 protected:
  template <typename T>
  static void InvokeTensorTensorKernel(UnaryInputParam<T> &param) {
    ...
    BesselY0Extend(l_y, l_x, l_tmp, param.size);
  ...
  static void CreateTensorInput(UnaryInputParam<T> &param) {
    ...
    for (uint32_t i = 0; i < param.size; i++) {
      ...
      } else if (param.x1[i] == 0.0f) {
        param.exp[i] = -std::numeric_limits<T>::infinity();
      } else {
        param.exp[i] = static_cast<T>(std::cyl_neumann(0, param.x1[i]));
      }
    }
  }
  ...
  template <typename T>
  static void BesselY0Test(uint32_t size) {
...
TEST_F(TestRegbaseApiBesselY0UT, BesselY0_TensorTensor_Test) {
  BesselY0Test<float>(ONE_BLK_SIZE / sizeof(float));
  BesselY0Test<float>(ONE_REPEAT_BYTE_SIZE / sizeof(float));
  BesselY0Test<float>(MAX_REPEAT_NUM * ONE_REPEAT_BYTE_SIZE / 2 / sizeof(float));
  BesselY0Test<float>((ONE_BLK_SIZE - sizeof(float)) / sizeof(float));
  BesselY0Test<float>((ONE_REPEAT_BYTE_SIZE - ONE_BLK_SIZE) / sizeof(float));
  BesselY0Test<float>((MAX_REPEAT_NUM - 1) * ONE_REPEAT_BYTE_SIZE / 2 / sizeof(float));
  BesselY0Test<float>(((MAX_REPEAT_NUM - 1) * ONE_REPEAT_BYTE_SIZE + (ONE_REPEAT_BYTE_SIZE - ONE_BLK_SIZE) +
