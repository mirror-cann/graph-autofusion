...
class TestRegbaseApiBesselY1UT : public testing::Test {
 protected:
  template <typename T>
  static void InvokeTensorTensorKernel(UnaryInputParam<T> &param) {
    ...
    BesselY1Extend(l_y, l_x, l_tmp, param.size);
  ...
  static void CreateTensorInput(UnaryInputParam<T> &param) {
    ...
    for (uint32_t i = 0; i < param.size; i++) {
      ...
      if (param.x1[i] < 0.0f) {
        ...
        param.exp[i] = static_cast<T>(std::cyl_neumann(1, param.x1[i]));
      }
    }
  }
  ...
  template <typename T>
  static void BesselY1Test(uint32_t size) {
...
TEST_F(TestRegbaseApiBesselY1UT, BesselY1_TensorTensor_Test) {
  BesselY1Test<float>(ONE_BLK_SIZE / sizeof(float));
  BesselY1Test<float>(ONE_REPEAT_BYTE_SIZE / sizeof(float));
  BesselY1Test<float>(MAX_REPEAT_NUM * ONE_REPEAT_BYTE_SIZE / 2 / sizeof(float));
  BesselY1Test<float>((ONE_BLK_SIZE - sizeof(float)) / sizeof(float));
  BesselY1Test<float>((ONE_REPEAT_BYTE_SIZE - ONE_BLK_SIZE) / sizeof(float));
  BesselY1Test<float>((MAX_REPEAT_NUM - 1) * ONE_REPEAT_BYTE_SIZE / 2 / sizeof(float));
  BesselY1Test<float>(((MAX_REPEAT_NUM - 1) * ONE_REPEAT_BYTE_SIZE + (ONE_REPEAT_BYTE_SIZE - ONE_BLK_SIZE) +
