...
#include "scaled_modified_bessel_k1.h"

using namespace AscendC;

namespace af {

class TestRegbaseApiScaledModifiedBesselK1UT : public testing::Test {
 protected:
  template <typename T>
  static void InvokeTensorTensorKernel(UnaryInputParam<T> &param) {
    ...
    ScaledModifiedBesselK1Extend(l_y, l_x, l_tmp, param.size);
  ...
  static void CreateTensorInput(UnaryInputParam<T> &param) {
    ...
    for (uint32_t i = 0; i < param.size; i++) {
      // Scaled K1 only defined for x > 0, generate range [0.0, 20.0]
      ...
      if (param.x1[i] == 0.0f) {
        ...
        // scaled_K1(x) = K1(x) * exp(x)
        param.exp[i] = static_cast<T>(std::cyl_bessel_k(1, param.x1[i]) * std::exp(param.x1[i]));
      }
    }
  }
  ...
  template <typename T>
  static void ScaledModifiedBesselK1Test(uint32_t size) {
...
TEST_F(TestRegbaseApiScaledModifiedBesselK1UT, ScaledModifiedBesselK1_TensorTensor_Test) {
  ScaledModifiedBesselK1Test<float>(ONE_BLK_SIZE / sizeof(float));
  ScaledModifiedBesselK1Test<float>(ONE_REPEAT_BYTE_SIZE / sizeof(float));
  ScaledModifiedBesselK1Test<float>(MAX_REPEAT_NUM * ONE_REPEAT_BYTE_SIZE / 2 / sizeof(float));
  ScaledModifiedBesselK1Test<float>((ONE_BLK_SIZE - sizeof(float)) / sizeof(float));
  ScaledModifiedBesselK1Test<float>((ONE_REPEAT_BYTE_SIZE - ONE_BLK_SIZE) / sizeof(float));
  ScaledModifiedBesselK1Test<float>((MAX_REPEAT_NUM - 1) * ONE_REPEAT_BYTE_SIZE / 2 / sizeof(float));
  ScaledModifiedBesselK1Test<float>(((MAX_REPEAT_NUM - 1) * ONE_REPEAT_BYTE_SIZE + (ONE_REPEAT_BYTE_SIZE - ONE_BLK_SIZE) +
