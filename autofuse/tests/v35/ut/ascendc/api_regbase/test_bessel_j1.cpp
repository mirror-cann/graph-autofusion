...
class TestRegbaseApiBesselJ1UT : public testing::Test {
 protected:
  template <typename T>
  static void InvokeTensorTensorKernel(UnaryInputParam<T> &param) {
    ...
    BesselJ1Extend(l_y, l_x, l_tmp, param.size);
  ...
  static void CreateTensorInput(UnaryInputParam<T> &param) {
    ...
    for (uint32_t i = 0; i < param.size; i++) {
      ...
      param.exp[i] = static_cast<T>(std::cyl_bessel_j(1, std::fabs(param.x1[i])) * std::copysign(1.0, param.x1[i]));
    }
  }
  ...
  template <typename T>
  static void BesselJ1Test(uint32_t size) {
...
TEST_F(TestRegbaseApiBesselJ1UT, BesselJ1_TensorTensor_Test) {
  BesselJ1Test<float>(ONE_BLK_SIZE / sizeof(float));
  BesselJ1Test<float>(ONE_REPEAT_BYTE_SIZE / sizeof(float));
  BesselJ1Test<float>(MAX_REPEAT_NUM * ONE_REPEAT_BYTE_SIZE / 2 / sizeof(float));
  BesselJ1Test<float>((ONE_BLK_SIZE - sizeof(float)) / sizeof(float));
  BesselJ1Test<float>((ONE_REPEAT_BYTE_SIZE - ONE_BLK_SIZE) / sizeof(float));
  BesselJ1Test<float>((MAX_REPEAT_NUM - 1) * ONE_REPEAT_BYTE_SIZE / 2 / sizeof(float));
  BesselJ1Test<float>(((MAX_REPEAT_NUM - 1) * ONE_REPEAT_BYTE_SIZE + (ONE_REPEAT_BYTE_SIZE - ONE_BLK_SIZE) +
