...
TEST(CodegenKernel, MicroNegApiCall_Load_Neg_INT8_Store) {
  af::AscGraph graph("test_neg_int8_graph");
  ...
  *store_op.y.strides = {s1 * s2 * s3 * Four, s2 * s3 * Three, s3 * Two, One};
  ...
  neg_node->attr.api.compute_type = af::ComputeType::kComputeLoad;
  neg_node->attr.api.type = af::ApiType::kAPITypeCompute;
  neg_node->attr.api.unit = af::ComputeUnit::kUnitMTE2;
  neg_node->attr.sched.loop_axis = z0.id;
  neg_node->outputs[0].attr.vectorized_axis = {z1.id, z2.id, z3.id};
  neg_node->outputs[0].attr.vectorized_strides = {af::Symbol(8), af::Symbol(2), One};
  neg_node->outputs[0].attr.dtype = af::DT_INT8;
  neg_node->outputs[0].attr.mem.position = af::Position::kPositionVecIn;
...
TEST(CodegenKernel, MicroNegApiCall_Load_Neg_INT64_Store) {
  af::AscGraph graph("test_neg_int64_graph");
  ...
  *store_op.y.strides = {s1 * s2 * s3 * Four, s2 * s3 * Three, s3 * Two, One};
  ...
  neg_node->outputs[0].attr.vectorized_axis = {z1.id, z2.id, z3.id};
  neg_node->outputs[0].attr.vectorized_strides = {af::Symbol(8), af::Symbol(2), One};
  neg_node->outputs[0].attr.dtype = af::DT_INT64;
  neg_node->outputs[0].attr.mem.position = af::Position::kPositionVecIn;
  neg_node->outputs[0].attr.mem.tensor_id = 1;
  neg_node->outputs[0].attr.mem.position = af::Position::kPositionVecIn;
  neg_node->outputs[0].attr.mem.alloc_type = af::AllocType::kAllocTypeQueue;
