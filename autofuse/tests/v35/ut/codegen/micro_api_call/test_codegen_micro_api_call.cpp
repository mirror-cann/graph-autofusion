...
TEST(CodegenKernel, MicroApiCall_Load_Abs_Store) {
  ...
  *store_op.y.strides = {s1 * s2 * s3 * Four, s2 * s3 * Three, s3 * Two, One};
  ...
  abs_node->attr.api.compute_type = af::ComputeType::kComputeLoad;
  abs_node->attr.api.type = af::ApiType::kAPITypeCompute;
  abs_node->attr.api.unit = af::ComputeUnit::kUnitMTE2;
  abs_node->attr.sched.loop_axis = z0.id;
  abs_node->outputs[0].attr.vectorized_axis = {z1.id, z2.id, z3.id};
  abs_node->outputs[0].attr.vectorized_strides = {af::Symbol(8), af::Symbol(2), One};
  abs_node->outputs[0].attr.dtype = af::DT_FLOAT;
  abs_node->outputs[0].attr.mem.position = af::Position::kPositionVecIn;
  abs_node->outputs[0].attr.mem.tensor_id = 1;
  abs_node->outputs[0].attr.mem.position = af::Position::kPositionVecIn;
  abs_node->outputs[0].attr.mem.alloc_type = af::AllocType::kAllocTypeQueue;
  abs_node->outputs[0].attr.que.id = 2;
  abs_node->outputs[0].attr.opt.merge_scope = af::kIdNone;
