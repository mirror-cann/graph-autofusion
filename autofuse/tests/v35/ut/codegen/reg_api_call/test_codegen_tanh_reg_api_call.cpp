...
TEST(CodegenKernel, UnaryApiTmpV2TanhCall) {
  ...
  Data x_op("x", graph);
  Load load_op("load");
  af::ascir_op::Tanh tanh_op("TanhExtend");
  ...
  *tanh_op.y.strides = {s1, One};
  ...
  codegen::UnaryApiTmpCall call("TanhExtend");
  ...
  EXPECT_EQ(result, std::string{"TanhExtend(local_1[0], local_0[0], tmp_buf_0, local_0_actual_size);\n"});
}
