...
void LoadDivStore_BeforeAutofuse(af::AscGraph &graph) {
...
void LoadDivStore_AfterInferOutput(af::AscGraph &graph) {
  ...
  div->outputs[0].attr.dtype = (af::DataType)load1->outputs[0].attr.dtype;
  ...
  store->outputs[0].attr.dtype = (af::DataType)div->outputs[0].attr.dtype;
...
void LoadDivStore_AfterGetApiInfo(af::AscGraph &graph) {
  auto x1 = graph.FindNode("x1");
...
void LoadDivStore_AfterScheduler(af::AscGraph &graph) {
...
void LoadDivStore_AfterQueBufAlloc(af::AscGraph &graph) {
  auto x1 = graph.FindNode("x1");
