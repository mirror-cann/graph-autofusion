...
void LoadAbsStore_AfterInferOutput(af::AscGraph &graph) {
  ...
  abs->outputs[0].attr.dtype =(af::DataType)load->outputs[0].attr.dtype;
  ...
  store->outputs[0].attr.dtype = (af::DataType)abs->outputs[0].attr.dtype;
...
void LoadAbsStore_AfterScheduler_store_scalar(af::AscGraph &graph) {
  ...
  auto [z0B, z0b] = graph.BlockSplit(z0);
  vector<AxisId> vectorized_axis{z1, z2};
  vector<af::Expression> vectorized_strides{Zero, Zero};
  auto size = af::GetSizeByDataType(af::DT_FLOAT16);
