...
class TestBackendFmaInt8E2e : public testing::Test {
 protected:
  void SetUp() override {
...
TEST_F(TestBackendFmaInt8E2e, FmaInt8E2eCodegen) {
...
#define GET_TILING_DATA(t, tiling)  AutofuseTilingData t = *(AutofuseTilingData*)tiling;
  ...
  // shape_info 和 FmaInt8FusedGraph入参dims_size匹配（个数相同，命名规则为s开头、编号从0开始）
  std::map<std::string, std::string> shape_info(
        { {"s0", "stub_s0"}, {"s1", "stub_s1"}, {"s2", "stub_s2"} }
    );
  auto graph = ascir::ShareGraph::FmaInt8FusedGraph(3);
  std::cout<<"KERNEL_SRC_LIST="<<KERNEL_SRC_LIST<<std::endl;
  std::vector<std::string> parts = splitString(KERNEL_SRC_LIST, ':');
  std::string kernel_src_file_name = parts[0];      // fma_int8_test_tiling.cpp
  std::string tiling_src_file_name = parts[1];      // fma_int8_test_kernel.cpp
