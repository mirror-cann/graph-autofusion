...
extern "C" __global__ __aicore__ void fma_int8_test(GM_ADDR x1, GM_ADDR x2, GM_ADDR x3, GM_ADDR y, GM_ADDR workspace,
                                                    GM_ADDR tiling);
...
TEST_P(E2E_BackendFmaInt8_Code, CalculateCorrect) {
  ...
  for (int i = 0; i < test_size; i++) {
    ...
    expect[i] = static_cast<int8_t>(static_cast<float>(input1[i]) * static_cast<float>(input2[i]) +
                                    static_cast<float>(input3[i]));
  }
  ...
  ICPU_RUN_KF(fma_int8_test, tiling_data.block_dim, (uint8_t *)input1, (uint8_t *)input2, (uint8_t *)input3,
