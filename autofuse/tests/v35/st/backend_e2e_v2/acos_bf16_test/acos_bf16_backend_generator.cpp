...
TEST_F(TestBackendAcosBf16E2e, AcosBf16E2eCodegen) {
...
#define GET_TILING_DATA(t, tiling)  AutofuseTilingData t = *(AutofuseTilingData*)tiling;
  ...
  // shape_info 和 AcosBf16FusedGraph入参dims_size匹配（个数相同，命名规则为s开头、编号从0开始）
  std::map<std::string, std::string> shape_info(
        { {"s0", "stub_s0"}, {"s1", "stub_s1"}, {"s2", "stub_s2"} }
    );
  auto graph = ascir::ShareGraph::AcosBf16FusedGraph(3);
  std::cout<<"KERNEL_SRC_LIST="<<KERNEL_SRC_LIST<<std::endl;
  std::vector<std::string> parts = splitString(KERNEL_SRC_LIST, ':');
  std::string kernel_src_file_name = parts[0];      // acos_bf16_test_tiling.cpp
  std::string tiling_src_file_name = parts[1];      // acos_bf16_test_kernel.cpp
