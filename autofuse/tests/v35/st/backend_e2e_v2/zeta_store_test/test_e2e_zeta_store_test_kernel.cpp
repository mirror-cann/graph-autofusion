...
extern "C" __global__ __aicore__ void zeta_store_test(GM_ADDR x1, GM_ADDR x2, GM_ADDR y1, GM_ADDR workspace,
                                                      GM_ADDR tiling);
...
class E2EBackendZetaStoreCode : public testing::Test, public testing::WithParamInterface<std::vector<int>> {};

static float ZetaReference(float s, float q) {
  float sum = 0.0F;
  for (int n = 0; n < 2000; ++n) {
    sum += 1.0F / std::pow(q + static_cast<float>(n), s);
  }
  return sum;
}

TEST_P(E2EBackendZetaStoreCode, CalculateCorrect) {
  ...
  for (int i = 0; i < test_size; i++) {
    x1[i] = static_cast<float>((i % 5) + 2);
    x2[i] = static_cast<float>((i % 5) + 1) * 0.5F;
    expect[i] = ZetaReference(x1[i], x2[i]);
  }
  ...
  ICPU_RUN_KF(zeta_store_test, tiling_data.block_dim, reinterpret_cast<uint8_t *>(x1), reinterpret_cast<uint8_t *>(x2),
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2EBackendZetaStoreCode,
