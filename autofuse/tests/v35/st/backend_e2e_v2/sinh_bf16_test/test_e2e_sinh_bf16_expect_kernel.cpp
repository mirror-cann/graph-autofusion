...
extern "C" __global__ __aicore__ void sinh_bf16_test(GM_ADDR x0, GM_ADDR output, GM_ADDR workspace,
...
namespace {
class E2E_SinhBf16_Code : public testing::Test, public testing::WithParamInterface<std::vector<int>> {};

TEST_P(E2E_SinhBf16_Code, CalculateCorrect) {
  ...
  for (int i = 0; i < test_size; i++) {
    x[i] = static_cast<bfloat16_t>(static_cast<bfloat16_t>(i) / test_size * 10.0f -
                                   5.0f);  // Normalize to [-5, 5] range for sinh
    expect[i] = static_cast<bfloat16_t>(std::sinh(static_cast<bfloat16_t>(x[i])));
  }
  ...
  ICPU_RUN_KF(sinh_bf16_test, tiling_data.block_dim, (uint8_t *)x, (uint8_t *)y, nullptr, (uint8_t *)&tiling_data);
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2E_SinhBf16_Code, ::testing::Values(std::vector<int>{2, 8, 16}));
