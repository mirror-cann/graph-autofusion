...
extern "C" __global__ __aicore__ void atan2_bf16_test(GM_ADDR x1, GM_ADDR x2, GM_ADDR y, GM_ADDR workspace,
                                                      GM_ADDR tiling);
...
class E2E_BackendAtan2Bf16_Code : public testing::Test, public testing::WithParamInterface<std::vector<int>> {};

TEST_P(E2E_BackendAtan2Bf16_Code, CalculateCorrect) {
  ...
  for (int i = 0; i < test_size; i++) {
    float val1 = rand() / (double)RAND_MAX;
    float val2 = rand() / (double)RAND_MAX;
    input1[i] = static_cast<bfloat16_t>(val1);
    input2[i] = static_cast<bfloat16_t>(val2);

    // atan2(y, x) - 注意参数顺序
    expect[i] = std::atan2(val2, val1);
  }
  ...
  ICPU_RUN_KF(atan2_bf16_test, tiling_data.block_dim, (uint8_t *)input1, (uint8_t *)input2, (uint8_t *)y, nullptr,
              (uint8_t *)&tiling_data);
  ...
  for (int i = 0; i < test_size; i++) {
    ...
    if (std::fabs(y_val - expect[i]) > 1e-2) {
      printf("diff at index %d, expect %f, got %f\n", i, expect[i], y_val);
      diff_count++;
    }
  }
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2E_BackendAtan2Bf16_Code,
                         ::testing::Values(std::vector<int>{2, 4, 8}));
