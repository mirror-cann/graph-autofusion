...
extern "C" __global__ __aicore__ void load_gather_abs_store_store_test(GM_ADDR param, GM_ADDR indices, GM_ADDR y,
                                                                       GM_ADDR workspace, GM_ADDR tiling);
extern "C" int64_t AutofuseTiling(uint32_t s0, uint32_t s1, uint32_t s2, uint32_t s3, uint32_t s4, uint32_t s5,
                                  uint32_t s6, AutofuseTilingData *tiling, uint32_t *workspaceSize, uint32_t *blockDim,
                                  uint32_t aiv_num, uint32_t ub_size);
class E2E_Load_Gather_B_T_Abs_Store
    : public testing::Test,
...
TEST_P(E2E_Load_Gather_B_T_Abs_Store, Gather_B_T_AbsTest) {
  ...
  // AutofuseTiling(uint32_t s0, uint32_t s1, uint32_t s2, uint32_t s3, uint32_t s4, uint32_t s5, uint32_t s6,
  // AutofuseTilingData* tiling, uint32_t* workspaceSize, uint32_t *blockDim, uint32_t aiv_num, uint32_t ub_size);
  AutofuseTiling(s0, s1, s2, s3, s4, s5, s6, &tiling_data, &ws_size, &block_dim, 48, 192 * 1024);
  AscendC::SetKernelMode(KernelMode::AIV_MODE);
  ICPU_RUN_KF(load_gather_abs_store_store_test, tiling_data.block_dim, (uint8_t *)param, (uint8_t *)indices,
              (uint8_t *)output, nullptr, (uint8_t *)&tiling_data);
  // Count difference
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2E_Load_Gather_B_T_Abs_Store,
                         ::testing::Values(std::pair<std::vector<int>, std::vector<int>>{{2, 2, 4, 2, 7}, {2, 4}}));
