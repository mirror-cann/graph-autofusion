...
// 输入输出数量与{graph}中描述一致
extern "C" __global__ __aicore__ void rshift_uint8_test(GM_ADDR x1, GM_ADDR x2, GM_ADDR y, GM_ADDR workspace,
                                                        GM_ADDR tiling);
...
TEST_P(E2E_BackendRshiftUint8_Code, CalculateCorrect) {
  ...
  ICPU_RUN_KF(rshift_uint8_test, tiling_data.block_dim, (uint8_t *)input1, (uint8_t *)input2, (uint8_t *)y, nullptr,
              (uint8_t *)&tiling_data);
  ...
  for (int i = 0; i < test_size; i++) {
    if (y[i] != expect[i]) {
      // 打印相关数据，方便问题处理
      printf("Index %d: input1=%d, input2=%d, y=%d, expect=%d, diff=%f\n", i, input1[i], input2[i], y[i], expect[i],
             std::fabs(y[i] - expect[i]));
      diff_count++;
    }
  }
