...
extern "C" __global__ __aicore__ void load_gather_tail_abs_store_store_test(GM_ADDR param, GM_ADDR indices, GM_ADDR y,
                                                                            GM_ADDR workspace, GM_ADDR tiling);
extern "C" void AutofuseTiling(uint32_t s0, uint32_t s1, uint32_t s2, uint32_t s3, uint32_t s4,
                               AutofuseTilingData *tiling, uint32_t *workspaceSize, uint32_t *blockDim,
                               uint32_t aiv_num, uint32_t ub_size);

class E2E_Load_Gather_Tail_B_T_Abs_Store
    : public testing::Test,
...
TEST_P(E2E_Load_Gather_Tail_B_T_Abs_Store, Gather_Tail_B_T_AbsTest) {
  ...
  ICPU_RUN_KF(load_gather_tail_abs_store_store_test, tiling_data.block_dim, (uint8_t *)param, (uint8_t *)indices,
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2E_Load_Gather_Tail_B_T_Abs_Store,
                         ::testing::Values(std::pair<std::vector<int>, std::vector<int>>{{2, 5, 5}, {2, 2}}));
