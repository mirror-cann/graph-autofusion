...
extern "C" __global__ __aicore__ void slice_concat_v2_test(GM_ADDR data0, GM_ADDR data1, GM_ADDR output,
...
TEST_P(E2E_BackendConcat_Code, CalculateCorrect) {
  ...
  ICPU_RUN_KF(slice_concat_v2_test, tiling_data.block_dim, (uint8_t *)input1, (uint8_t *)input2, (uint8_t *)y, nullptr,
              (uint8_t *)&tiling_data);
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2E_BackendConcat_Code,
                         ::testing::Values(std::vector<int>{15, 7}, std::vector<int>{16, 7}, std::vector<int>{17, 7},
                                           // std::vector<int>{15, 9}, cpu nddma bug，临时注释
                                           // std::vector<int>{17, 9}, cpu nddma bug，临时注释
                                           std::vector<int>{31, 15}, std::vector<int>{31, 17},
                                           // std::vector<int>{31, 31}, cpu nddma bug，临时注释
                                           std::vector<int>{31, 33}, std::vector<int>{16, 63},
