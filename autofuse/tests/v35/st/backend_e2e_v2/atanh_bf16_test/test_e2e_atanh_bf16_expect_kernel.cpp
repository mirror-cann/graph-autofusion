...
extern "C" __global__ __aicore__ void atanh_bf16_test(GM_ADDR x, GM_ADDR y, GM_ADDR workspace, GM_ADDR tiling);
...
class E2E_BackendAtanh_bf16_Code : public testing::Test, public testing::WithParamInterface<std::vector<int>> {};

TEST_P(E2E_BackendAtanh_bf16_Code, CalculateCorrect) {
  ...
  for (int i = 0; i < test_size; i++) {
    float val = rand() / (double)RAND_MAX * 0.99f;  // [0, 0.99) to avoid domain error
    input[i] = static_cast<bfloat16_t>(val);
    expect[i] = std::atanh(val);
  }
  ...
  ICPU_RUN_KF(atanh_bf16_test, tiling_data.block_dim, (uint8_t *)input, (uint8_t *)y, nullptr, (uint8_t *)&tiling_data);
  ...
  for (int i = 0; i < test_size; i++) {
    ...
    if (std::fabs(y_val - expect[i]) > EPS) {
      printf("diff: %f, expect: %f, i: %d, input: %f\n", y_val - expect[i], expect[i], i, static_cast<float>(input[i]));
      diff_count++;
    }
  }
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2E_BackendAtanh_bf16_Code,
