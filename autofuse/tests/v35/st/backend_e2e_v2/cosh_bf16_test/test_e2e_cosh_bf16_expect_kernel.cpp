...
extern "C" __global__ __aicore__ void cosh_bf16_test(GM_ADDR x, GM_ADDR y, GM_ADDR workspace, GM_ADDR tiling);
...
class E2E_BackendCosh_bf16_Code : public testing::Test, public testing::WithParamInterface<std::vector<int>> {};

TEST_P(E2E_BackendCosh_bf16_Code, CalculateCorrect) {
  ...
  for (int i = 0; i < test_size; i++) {
    float val = rand() / (double)RAND_MAX * 2.0f - 1.0f;  // [-1, 1]
    input[i] = static_cast<bfloat16_t>(val);
    expect[i] = std::cosh(val);
  }
  ...
  ICPU_RUN_KF(cosh_bf16_test, tiling_data.block_dim, (uint8_t *)input, (uint8_t *)y, nullptr, (uint8_t *)&tiling_data);
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2E_BackendCosh_bf16_Code,
