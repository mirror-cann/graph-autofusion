...
extern "C" __global__ __aicore__ void shifted_chebyshev_polynomial_v_store_test(GM_ADDR x1, GM_ADDR y1,
...
class E2EBackendShiftedChebyshevPolynomialVStoreCode : public testing::Test,
                                                       public testing::WithParamInterface<std::vector<int>> {};

static float ShiftedChebyshevPolynomialVReference(float x) {
  float t = 2.0F * x - 1.0F;
  float v0 = 1.0F;
  float v1 = 2.0F * t - 1.0F;
  float v2 = 2.0F * t * v1 - v0;
  return 2.0F * t * v2 - v1;
}

TEST_P(E2EBackendShiftedChebyshevPolynomialVStoreCode, CalculateCorrect) {
  ...
  for (int i = 0; i < test_size; i++) {
    ...
    expect[i] = ShiftedChebyshevPolynomialVReference(x[i]);
  }
  ...
  ICPU_RUN_KF(shifted_chebyshev_polynomial_v_store_test, tiling_data.block_dim, reinterpret_cast<uint8_t *>(x),
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2EBackendShiftedChebyshevPolynomialVStoreCode,
