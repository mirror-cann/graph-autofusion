...
extern "C" __global__ __aicore__ void shifted_chebyshev_polynomial_t_store_test(GM_ADDR x1, GM_ADDR y1,
...
class E2EBackendShiftedChebyshevPolynomialTStoreCode : public testing::Test,
                                                       public testing::WithParamInterface<std::vector<int>> {};

TEST_P(E2EBackendShiftedChebyshevPolynomialTStoreCode, CalculateCorrect) {
  ...
  ICPU_RUN_KF(shifted_chebyshev_polynomial_t_store_test, tiling_data.block_dim, reinterpret_cast<uint8_t *>(x),
              reinterpret_cast<uint8_t *>(y), nullptr, reinterpret_cast<uint8_t *>(&tiling_data));

  // Basic sanity: output should not be NaN for valid inputs
  ...
  EXPECT_EQ(invalid_count, 0U) << " of " << test_size;

  AscendC::GmFree(x);
  AscendC::GmFree(y);
}

INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2EBackendShiftedChebyshevPolynomialTStoreCode,
