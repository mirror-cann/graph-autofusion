...
extern "C" __global__ __aicore__ void isfinite_bf16_test(GM_ADDR x, GM_ADDR y, GM_ADDR workspace, GM_ADDR tiling);
...
class E2E_BackendIsfiniteBf16_Code : public testing::Test, public testing::WithParamInterface<std::vector<int>> {};

TEST_P(E2E_BackendIsfiniteBf16_Code, CalculateCorrect) {
  ...
  for (int i = 0; i < test_size; i++) {
    ...
    // IsFinite returns 1 if finite, 0 if not finite (inf, -inf, NaN)
    expect[i] = std::isfinite(val) ? 1 : 0;
  }
  ...
  ICPU_RUN_KF(isfinite_bf16_test, tiling_data.block_dim, (uint8_t *)input, (uint8_t *)y, nullptr,
              (uint8_t *)&tiling_data);
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2E_BackendIsfiniteBf16_Code,
