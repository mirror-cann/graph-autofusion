...
extern "C" __global__ __aicore__ void bool_mask_flow_test(GM_ADDR x1, GM_ADDR x2, GM_ADDR x3, GM_ADDR x4, GM_ADDR y1,
...
class E2EBackendBoolMaskFlowCode : public testing::Test, public testing::WithParamInterface<std::vector<int>> {};

TEST_P(E2EBackendBoolMaskFlowCode, CalculateCorrect) {
  ...
  ICPU_RUN_KF(bool_mask_flow_test, tiling_data.block_dim, reinterpret_cast<uint8_t *>(data.x1),
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2EBackendBoolMaskFlowCode, ::testing::Values(std::vector<int>{8, 8}));
