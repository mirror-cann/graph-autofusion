...
extern "C" __global__ __aicore__ void scaled_modified_bessel_k0_store_test(GM_ADDR x1, GM_ADDR y1, GM_ADDR workspace,
                                                                           GM_ADDR tiling);
...
class E2EBackendScaledModifiedBesselK0StoreCode : public testing::Test,
                                                  public testing::WithParamInterface<std::vector<int>> {};

static float ScaledModifiedBesselK0Reference(float x) {
  return static_cast<float>(std::exp(x) * std::cyl_bessel_k(0.0F, x));
}

TEST_P(E2EBackendScaledModifiedBesselK0StoreCode, CalculateCorrect) {
  ...
  for (int i = 0; i < test_size; i++) {
    x[i] = static_cast<float>((i % 9) + 1) / 4.0F;
    expect[i] = ScaledModifiedBesselK0Reference(x[i]);
  }
  ...
  ICPU_RUN_KF(scaled_modified_bessel_k0_store_test, tiling_data.block_dim, reinterpret_cast<uint8_t *>(x),
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2EBackendScaledModifiedBesselK0StoreCode,
