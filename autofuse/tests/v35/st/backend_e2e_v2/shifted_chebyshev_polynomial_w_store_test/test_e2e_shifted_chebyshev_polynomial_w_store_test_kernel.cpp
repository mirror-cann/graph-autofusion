...
extern "C" __global__ __aicore__ void shifted_chebyshev_polynomial_w_store_test(GM_ADDR x1, GM_ADDR y1,
...
class E2EBackendShiftedChebyshevPolynomialWStoreCode : public testing::Test,
                                                       public testing::WithParamInterface<std::vector<int>> {};

static float ShiftedChebyshevPolynomialWReference(float x) {
  float t = 2.0F * x - 1.0F;
  float w0 = 1.0F;
  float w1 = 2.0F * t + 1.0F;
  float w2 = 2.0F * t * w1 - w0;
  return 2.0F * t * w2 - w1;
}

TEST_P(E2EBackendShiftedChebyshevPolynomialWStoreCode, CalculateCorrect) {
  ...
  for (int i = 0; i < test_size; i++) {
    ...
    expect[i] = ShiftedChebyshevPolynomialWReference(x[i]);
  }
  ...
  ICPU_RUN_KF(shifted_chebyshev_polynomial_w_store_test, tiling_data.block_dim, reinterpret_cast<uint8_t *>(x),
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2EBackendShiftedChebyshevPolynomialWStoreCode,
