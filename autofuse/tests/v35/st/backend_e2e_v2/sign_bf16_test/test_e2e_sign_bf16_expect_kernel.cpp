...
extern "C" __global__ __aicore__ void sign_bf16_test(GM_ADDR x0, GM_ADDR output, GM_ADDR workspace,
...
namespace {
class E2E_SignBf16_Code : public testing::Test, public testing::WithParamInterface<std::vector<int>> {};

TEST_P(E2E_SignBf16_Code, CalculateCorrect) {
  ...
  for (int i = 0; i < test_size; i++) {
    float input = (i % 200) - 100;  // 生成 -100 到 99 的测试值
    x[i] = static_cast<bfloat16_t>(input);

    if (input > 0) {
      expect[i] = static_cast<bfloat16_t>(1.0f);
    } else if (input < 0) {
      expect[i] = static_cast<bfloat16_t>(-1.0f);
    } else {
      expect[i] = static_cast<bfloat16_t>(0.0f);
    }
  }
  ...
  ICPU_RUN_KF(sign_bf16_test, tiling_data.block_dim, (uint8_t *)x, (uint8_t *)y, nullptr, (uint8_t *)&tiling_data);

  // Count difference
  uint32_t diff_count = 0;
  const float eps = 1e-2f;
  for (int i = 0; i < test_size; i++) {
    auto diff = static_cast<float>(y[i] - expect[i]);
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2E_SignBf16_Code, ::testing::Values(std::vector<int>{2, 8, 8}));
