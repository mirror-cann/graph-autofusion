...
extern "C" __global__ __aicore__ void abs_fma_test(GM_ADDR x1, GM_ADDR x2, GM_ADDR x3, GM_ADDR y, GM_ADDR workspace,
                                                   GM_ADDR tiling);
...
class E2E_BackendAbsFma_Code : public testing::Test, public testing::WithParamInterface<std::vector<int>> {};

TEST_P(E2E_BackendAbsFma_Code, CalculateCorrect) {
  auto test_shape = GetParam();

  uint64_t block_dim = 1;

  AutofuseTilingData tiling_data;
  float *input1 = (float *)AscendC::GmAlloc(test_shape[0] * sizeof(float) + 32);
  float *input2 = (float *)AscendC::GmAlloc(test_shape[0] * sizeof(float) + 32);
  float *input3 = (float *)AscendC::GmAlloc(test_shape[0] * sizeof(float) + 32);
  ...
  ICPU_RUN_KF(abs_fma_test, tiling_data.block_dim, (uint8_t *)input1, (uint8_t *)input2, (uint8_t *)input3,
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2E_BackendAbsFma_Code,
