...
extern "C" __global__ __aicore__ void bessel_y0_store_test(GM_ADDR x1, GM_ADDR y1, GM_ADDR workspace, GM_ADDR tiling);
...
class E2EBackendBesselY0StoreCode : public testing::Test, public testing::WithParamInterface<std::vector<int>> {};

static float BesselY0Reference(float x) {
  return static_cast<float>(std::cyl_neumann(0.0F, x));
}

TEST_P(E2EBackendBesselY0StoreCode, CalculateCorrect) {
  ...
  for (int i = 0; i < test_size; i++) {
    ...
    expect[i] = BesselY0Reference(x[i]);
  }
  ...
  ICPU_RUN_KF(bessel_y0_store_test, tiling_data.block_dim, reinterpret_cast<uint8_t *>(x),
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2EBackendBesselY0StoreCode,
