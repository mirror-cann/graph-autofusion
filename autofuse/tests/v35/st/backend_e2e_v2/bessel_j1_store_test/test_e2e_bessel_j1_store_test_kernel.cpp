...
extern "C" __global__ __aicore__ void bessel_j1_store_test(GM_ADDR x1, GM_ADDR y1, GM_ADDR workspace, GM_ADDR tiling);
...
class E2EBackendBesselJ1StoreCode : public testing::Test, public testing::WithParamInterface<std::vector<int>> {};

static float BesselJ1Reference(float x) {
  if (x < 0.0F) {
    return -static_cast<float>(std::cyl_bessel_j(1.0F, -x));
  }
  return static_cast<float>(std::cyl_bessel_j(1.0F, x));
}

TEST_P(E2EBackendBesselJ1StoreCode, CalculateCorrect) {
  ...
  for (int i = 0; i < test_size; i++) {
    ...
    expect[i] = BesselJ1Reference(x[i]);
  }
  ...
  ICPU_RUN_KF(bessel_j1_store_test, tiling_data.block_dim, reinterpret_cast<uint8_t *>(x),
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2EBackendBesselJ1StoreCode,
