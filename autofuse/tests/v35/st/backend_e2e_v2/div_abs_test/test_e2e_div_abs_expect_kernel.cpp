...
extern "C" __global__ __aicore__ void div_abs_test(GM_ADDR x0, GM_ADDR x1, GM_ADDR y, GM_ADDR workspace,
                                                   GM_ADDR tiling);
...
class E2E_BackendDivAbs_Code : public testing::Test, public testing::WithParamInterface<std::vector<int>> {};

TEST_P(E2E_BackendDivAbs_Code, CalculateCorrect) {
  ...
  for (int i = 0; i < test_size; i++) {
    ...
    expect[i] = static_cast<float>(fabs(input0[i] / input1[i]));
  }
  ...
  ICPU_RUN_KF(div_abs_test, tiling_data.block_dim, (uint8_t *)input0, (uint8_t *)input1, (uint8_t *)y, nullptr,
              (uint8_t *)&tiling_data);
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2E_BackendDivAbs_Code,
