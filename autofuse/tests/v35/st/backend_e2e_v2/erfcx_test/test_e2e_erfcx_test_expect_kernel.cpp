...
extern "C" __global__ __aicore__ void erfcx_bf16_test(GM_ADDR x1, GM_ADDR y, GM_ADDR workspace, GM_ADDR tiling);
...
class E2E_BackendErfcxTest_Code : public testing::Test, public testing::WithParamInterface<std::vector<int>> {};

TEST_P(E2E_BackendErfcxTest_Code, CalculateCorrect) {
  ...
  for (int i = 0; i < test_size; i++) {
    float val = rand() / (float)RAND_MAX;  // [0,1)
    input1[i] = static_cast<bfloat16_t>(val);
    // erfcx(x) = exp(x^2) * erfc(x)
    expect[i] = static_cast<bfloat16_t>(std::exp(static_cast<float>(input1[i]) * static_cast<float>(input1[i])) *
                                        std::erfc(static_cast<float>(input1[i])));
  }
  ...
  ICPU_RUN_KF(erfcx_bf16_test, tiling_data.block_dim, (uint8_t *)input1, (uint8_t *)y, nullptr,
              (uint8_t *)&tiling_data);

  // 根据精度校验规则: bfloat16_t 使用容差比较, EPS = 1e-2
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2E_BackendErfcxTest_Code,
