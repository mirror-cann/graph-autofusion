...
extern "C" __global__ __aicore__ void abs_uint8_test(GM_ADDR x0, GM_ADDR output, GM_ADDR workspace,
...
namespace {
class E2E_AbsUint8_Code : public testing::Test, public testing::WithParamInterface<std::vector<int>> {};

TEST_P(E2E_AbsUint8_Code, CalculateCorrect) {
  ...
  for (int i = 0; i < test_size; i++) {
    ...
    expect[i] = static_cast<uint8_t>(fabs(i));
  }
  ...
  ICPU_RUN_KF(abs_uint8_test, tiling_data.block_dim, (uint8_t *)x, (uint8_t *)y, nullptr, (uint8_t *)&tiling_data);
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2E_AbsUint8_Code, ::testing::Values(std::vector<int>{2, 16}));
