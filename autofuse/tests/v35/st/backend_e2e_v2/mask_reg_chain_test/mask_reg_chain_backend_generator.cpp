...
class TestBackendMaskRegChainE2e : public testing::Test {
 protected:
  void SetUp() override {
...
TEST_F(TestBackendMaskRegChainE2e, MaskRegChainE2eCodegen) {
...
#define GET_TILING_DATA(t, tiling)  AutofuseTilingData t = *(AutofuseTilingData*)tiling;
  ...
  // shape_info 和 MaskRegChainFusedGraph入参匹配 (dims_size=2表示两个维度s0,s1)
  std::map<std::string, std::string> shape_info({{"s0", "stub_s0"}, {"s1", "stub_s1"}});
  auto graph = ascir::ShareGraph::MaskRegChainFusedGraph(2);
