...
extern "C" __global__ __aicore__ void continues_brc_test(GM_ADDR data0, GM_ADDR data1, GM_ADDR output, GM_ADDR output2,
...
TEST_P(E2E_ContinuesBrc_Code, CalculateCorrect) {
        ...
        for (int i3 = 0; i3 < test_shape[3]; ++i3) {
          for (int i4 = 0; i4 < test_shape[4]; ++i4) {
            ...
            size_t idx0 = i0 * test_shape[1] * test_shape[2] * test_shape[3] * test_shape[4] +
                          i1 * test_shape[2] * test_shape[3] * test_shape[4] + i2 * test_shape[3] * test_shape[4] +
                          i3 * test_shape[4] + i4;
  ...
  ICPU_RUN_KF(continues_brc_test, tiling_data.block_dim, (uint8_t *)input0, (uint8_t *)input1, (uint8_t *)y,
              (uint8_t *)y1, nullptr, (uint8_t *)&tiling_data);
