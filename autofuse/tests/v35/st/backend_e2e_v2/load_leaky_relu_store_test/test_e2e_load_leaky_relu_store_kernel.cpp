...
extern "C" __global__ __aicore__ void load_leaky_relu_store_test(GM_ADDR x1, GM_ADDR y1, GM_ADDR workspace,
                                                                 GM_ADDR tiling);
...
class E2E_BackendLoadLeakyReluStore_Code : public testing::Test,
...
TEST_P(E2E_BackendLoadLeakyReluStore_Code, CalculateCorrect) {
  ...
  ICPU_RUN_KF(load_leaky_relu_store_test, tiling_data.block_dim, (uint8_t *)x, (uint8_t *)y, nullptr,
              (uint8_t *)&tiling_data);
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2E_BackendLoadLeakyReluStore_Code,
                         ::testing::Values(std::vector<int>{32, 16},  // 用例输入的维度需要与构图接口的dims_size匹配
                                           std::vector<int>{32, 18}, std::vector<int>{512, 15}));
