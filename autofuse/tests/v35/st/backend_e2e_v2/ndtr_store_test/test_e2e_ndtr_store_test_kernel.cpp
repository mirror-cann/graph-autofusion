...
extern "C" __global__ __aicore__ void ndtr_store_test(GM_ADDR x1, GM_ADDR y1, GM_ADDR workspace, GM_ADDR tiling);
...
class E2EBackendNdtrStoreCode : public testing::Test, public testing::WithParamInterface<std::vector<int>> {};

static float NdtrReference(float x) {
  constexpr float sqrt_two = 1.4142135623730951F;
  return 0.5F * std::erfc(-x / sqrt_two);
}

TEST_P(E2EBackendNdtrStoreCode, CalculateCorrect) {
  ...
  for (int i = 0; i < test_size; i++) {
    x[i] = static_cast<float>((i % 21) - 10) / 5.0F;
    expect[i] = NdtrReference(x[i]);
  }
  ...
  ICPU_RUN_KF(ndtr_store_test, tiling_data.block_dim, reinterpret_cast<uint8_t *>(x), reinterpret_cast<uint8_t *>(y),
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2EBackendNdtrStoreCode,
