...
extern "C" __global__ __aicore__ void gather_reduce_store_store_test(GM_ADDR param, GM_ADDR indices, GM_ADDR y,
                                                                     GM_ADDR workspace, GM_ADDR tiling);
extern "C" int64_t AutofuseTiling(uint32_t s0, uint32_t s1, uint32_t s2, uint32_t s3, AutofuseTilingData *tiling,
                                  uint32_t *workspaceSize, uint32_t *blockDim, uint32_t aiv_num, uint32_t ub_size);
...
TEST_P(E2E_Gather_Max_Store, Gather_To_Load_Gather_StoreTest) {
  ...
  ICPU_RUN_KF(gather_reduce_store_store_test, tiling_data.block_dim, (uint8_t *)param, (uint8_t *)indices,
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2E_Gather_Max_Store,
                         ::testing::Values(std::pair<std::vector<int>, std::vector<int>>{{2, 2}, {2, 2}}));
