...
extern "C" __global__ __aicore__ void bessel_j0_store_test(GM_ADDR x1, GM_ADDR y1, GM_ADDR workspace, GM_ADDR tiling);
...
class E2EBackendBesselJ0StoreCode : public testing::Test, public testing::WithParamInterface<std::vector<int>> {};

static float BesselJ0Reference(float x) {
  // Series expansion for J0(x): sum_{k=0}^{inf} (-1)^k (x/2)^{2k} / (k!)^2
  float half_x = x * 0.5F;
  float term = 1.0F;
  float sum = 1.0F;
  for (int k = 1; k <= 20; ++k) {
...
TEST_P(E2EBackendBesselJ0StoreCode, CalculateCorrect) {
  ...
  for (int i = 0; i < test_size; i++) {
    ...
    expect[i] = BesselJ0Reference(x[i]);
  }
  ...
  ICPU_RUN_KF(bessel_j0_store_test, tiling_data.block_dim, reinterpret_cast<uint8_t *>(x),
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2EBackendBesselJ0StoreCode,
