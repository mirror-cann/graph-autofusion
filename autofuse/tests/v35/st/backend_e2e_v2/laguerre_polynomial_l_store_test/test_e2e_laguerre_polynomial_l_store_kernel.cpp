...
static float LaguerrePolynomialLReference(float x, int n) {
  if (n == 0) {
    return 1.0F;
  }
  float prev2 = 1.0F;
...
TEST_P(E2EBackendLaguerrePolynomialLStoreCode, CalculateCorrect) {
  ...
  for (int i = 0; i < test_size; i++) {
    ...
    n[i] = 3;
    expect[i] = LaguerrePolynomialLReference(x[i], n[i]);
  }
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2EBackendLaguerrePolynomialLStoreCode,
