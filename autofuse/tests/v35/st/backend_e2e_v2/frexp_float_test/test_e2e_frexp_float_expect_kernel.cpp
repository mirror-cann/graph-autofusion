...
extern "C" __global__ __aicore__ void frexp_float_test(GM_ADDR x, GM_ADDR y, GM_ADDR y1, GM_ADDR workspace,
                                                       GM_ADDR tiling_data);
...
TEST_P(E2E_FrexpFloat_Code, CalculateCorrect) {
  ...
  ICPU_RUN_KF(frexp_float_test, tiling_data.block_dim, (uint8_t *)x, (uint8_t *)mantissa, (uint8_t *)exponent, nullptr,
              (uint8_t *)&tiling_data);
  ...
  for (int i = 0; i < test_size; i++) {
    ...
    if (diff_mantissa < -1e-5 || diff_mantissa > 1e-5) {
      printf("mantissa diff at index %d: x: %f, mantissa: %f, expect: %f, diff: %f\n", i, static_cast<float>(x[i]),
             static_cast<float>(mantissa[i]), static_cast<float>(expect_mantissa[i]), diff_mantissa);
      diff_count++;
    }
    if (diff_exponent < -1e-5 || diff_exponent > 1e-5) {
      printf("exponent diff at index %d: x: %f, exponent: %f, expect: %f, diff: %f\n", i, static_cast<float>(x[i]),
             static_cast<float>(exponent[i]), static_cast<float>(expect_exponent[i]), diff_exponent);
      diff_count++;
    }
  }
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2E_FrexpFloat_Code, ::testing::Values(std::vector<int>{2, 8, 16}));
