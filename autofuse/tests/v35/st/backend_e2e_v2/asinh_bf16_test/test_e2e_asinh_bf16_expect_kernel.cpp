...
extern "C" __global__ __aicore__ void asinh_bf16_test(GM_ADDR x0, GM_ADDR output, GM_ADDR workspace,
...
namespace {
class E2E_AsinhBf16_Code : public testing::Test, public testing::WithParamInterface<std::vector<int>> {};

TEST_P(E2E_AsinhBf16_Code, CalculateCorrect) {
  ...
  for (int i = 0; i < test_size; i++) {
    float val = static_cast<float>(i) / test_size;  // Normalize to [-1, 1] range for asinh
    x[i] = static_cast<bfloat16_t>(val);
    expect[i] = static_cast<bfloat16_t>(std::asinh(val));
  }
  ...
  ICPU_RUN_KF(asinh_bf16_test, tiling_data.block_dim, (uint8_t *)x, (uint8_t *)y, nullptr, (uint8_t *)&tiling_data);
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2E_AsinhBf16_Code, ::testing::Values(std::vector<int>{2, 8, 16}));
