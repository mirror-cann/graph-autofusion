...
struct ExpressionItem {
  ...
  // 常用构造函数
  ExpressionItem(const ge::Expression &e, ExpressionConvertMethod m) : expr(e), method(m) {}
  ...
  // 带乘数和 cast 的构造函数
  ExpressionItem(const ge::Expression &e, ExpressionConvertMethod m, int64_t multiplier, const std::string &cast)
...
struct CombinedExpression {
  ...
  // 单项表达式构造
  explicit CombinedExpression(const ExpressionItem &item) : combined_expr(item.expr), items{item} {}

  // 两项加减构造
  CombinedExpression(const ExpressionItem &item1, const ExpressionItem &item2, const std::string &op)
  ...
  // 多项构造
  CombinedExpression(const std::vector<ExpressionItem> &its, const std::vector<std::string> &ops)
