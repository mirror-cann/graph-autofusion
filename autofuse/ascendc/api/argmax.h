...
inline __aicore__ void ArgmaxLEOneRepeat(const LocalTensor<int64_t> &dstTensor, const LocalTensor<T> &srcTensor,
                                         const LocalTensor<uint8_t> &sharedTmpBuffer, const uint32_t repeatTimes,
                                         const uint32_t last) {
...
__aicore__ void ArgmaxGTOneRepeat(const LocalTensor<int64_t> &dstTensor, const LocalTensor<T> &srcTensor,
                                  const LocalTensor<uint8_t> &sharedTmpBuffer, const uint32_t first,
                                  const uint32_t last, const uint32_t repeatTimesOneRow) {
  ...
  for (uint32_t i = 0; i < first; ++i) {
    ...
    AscendC::SetFlag<AscendC::HardEvent::V_S>(EVENT_ID0);
    AscendC::WaitFlag<AscendC::HardEvent::V_S>(EVENT_ID0);
    uint64_t minMask[1] = {(static_cast<uint64_t>(tmpMask(0)) << 0) | (static_cast<uint64_t>(tmpMask(1)) << 8) |
                           (static_cast<uint64_t>(tmpMask(2)) << 16) | (static_cast<uint64_t>(tmpMask(3)) << 24) |
                           (static_cast<uint64_t>(tmpMask(4)) << 32) | (static_cast<uint64_t>(tmpMask(5)) << 40) |
                           (static_cast<uint64_t>(tmpMask(6)) << 48) | (static_cast<uint64_t>(tmpMask(7)) << 56)};
    AscendC::SetFlag<AscendC::HardEvent::S_V>(EVENT_ID0);
    AscendC::WaitFlag<AscendC::HardEvent::S_V>(EVENT_ID0);
...
__aicore__ void ArgmaxRA(const LocalTensor<int64_t> &dstTensor, const LocalTensor<T> &srcTensor,
                         const LocalTensor<uint8_t> &sharedTmpBuffer, const uint32_t first, const uint32_t last,
                         const uint32_t repeatTimesOneRow) {
  LocalTensor<float> tmpLocal = sharedTmpBuffer.ReinterpretCast<float>();
  LocalTensor<float> maxLocal = sharedTmpBuffer[256].ReinterpretCast<float>();
  LocalTensor<float> intTmp = sharedTmpBuffer[512].ReinterpretCast<float>();
  LocalTensor<float> dupIndexTensor = sharedTmpBuffer[768].ReinterpretCast<float>();
...
__aicore__ inline void ArgMaxExtend(const LocalTensor<T> &dst, const LocalTensor<U> &src,
  ...
  static_assert(std::is_same_v<U, float> || std::is_same_v<U, int32_t>,
                "ArgMaxExtend src only support float and int32_t");
  ...
  if constexpr (SupportType<pattern, Pattern::Reduce::AR>()) {
    if constexpr (std::is_same_v<U, float>) {
      ...
      if (last <= 64) {
        ...
        uint32_t repeatTimesOneRow = CeilDivision(last, 64);
        ArgmaxGTOneRepeat<int32_t, Pattern::Reduce::AR, false>(dst, src, sharedTmpBuffer, first, last,
                                                               repeatTimesOneRow);
      }
    }
