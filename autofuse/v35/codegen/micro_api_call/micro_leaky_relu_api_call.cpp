...
namespace codegen {
Status MicroLeakyReluApiCall::Generate(const codegen::TensorManager &tensor_mng, [[maybe_unused]] const TPipe &tpipe,
  ...
  for (auto in_arg : this->inputs_) {
    ...
    GE_CHK_STATUS_RET(Tensor::DtypeName(tensor_mng.GetTensor(in_arg.second)->dtype_, x_dtype),
                      "get name of dtype:%d failed", static_cast<int32_t>(tensor_mng.GetTensor(in_arg.second)->dtype_));
