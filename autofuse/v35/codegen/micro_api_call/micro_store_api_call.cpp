...
Status MicroStoreApiCall::Init(const ascir::NodeView &node) {
  ...
  bool is_output_out_dtype_scalar = std::all_of(
      node->outputs[0].attr.vectorized_strides.begin(), node->outputs[0].attr.vectorized_strides.end(),
      [](const ascir::SizeExpr &stride) {
