        ...
        uint64_t tilingdata_offset =                                                                   \
            (sizeof(MatMulV3TilingData) > TILINGDATA_OFFSET) ?                                         \
                ...
                MMV3DivFloor(GetCurrentBlockIdx(), MMV3DivCeil(GetBlockNum(), TILINGDATA_SPLIT_NUM)) * \
                    TILINGDATA_OFFSET;                                                                 \
        const MatMulV3TilingData &tilingData = *reinterpret_cast<const MatMulV3TilingData *>(reinterpret_cast<const uint8_t *>(&(tilingGM.matmul_tiling_data)) + tilingdata_offset); \
...
__aicore__ void mat_mul_v3(
    GM_ADDR aGM, GM_ADDR bGM, GM_ADDR biasGM, GM_ADDR offsetWGM, GM_ADDR cGM, GM_ADDR workspaceGM,
#ifdef INDUCTOR_TILING_DATA
    ...
    )
{
    ...
    if constexpr (
        ...
        TPipe pipe;
        #ifdef INDUCTOR_TILING_DATA
        GET_TILING_DATA_WITH_STRUCT_PTR(MatMulV3KEqZeroBasicTilingData, tilingData, tilingGM);
        #else
...
template <int8_t API_LEVEL, int8_t A_TRANS, int8_t B_TRANS, int8_t BATCH_MODEL, int8_t MODEL, int8_t FULL_LOAD,
    int8_t L0C2OUT_MODEL>
__aicore__ void mat_mul_v3_fusion(
    GM_ADDR aGM, GM_ADDR bGM, GM_ADDR biasGM, GM_ADDR offsetWGM, GM_ADDR cGM, GM_ADDR workspaceGM,
#ifdef INDUCTOR_TILING_DATA
    const CVAutofuseTilingData& tilingGM,
#else
    ...
    AutoFusionVector::Params *param)
{
    ...
    if constexpr (
        ...
        L0C2OUT_MODEL == MAT_MUL_ON_THE_FLY) {
        GET_TILING_DATA_WITH_STRUCT(MatMulV3BasicTilingData, tilingData, tilingGM);
        MatmulV3Advanced::MatMulActKernelFusion<
            DTYPE_X1, DTYPE_X2, DTYPE_Y, DTYPE_BIAS, aLayout, bLayout, layout::RowMajor, B_FULL_LOAD_MODE,
            OP_TYPE_RELU_VALUE>(
            aGM, bGM, biasGM, cGM, nullptr, tilingData, param);
    } else if constexpr (
...
template <int8_t API_LEVEL, int8_t A_TRANS, int8_t B_TRANS, int8_t BATCH_MODEL, int8_t MODEL, int8_t FULL_LOAD,
    int8_t L0C2OUT_MODEL>
__aicore__ void mat_mul_v3_fusion_db(
    GM_ADDR aGM, GM_ADDR bGM, GM_ADDR biasGM, GM_ADDR offsetWGM, GM_ADDR cGM, GM_ADDR workspaceGM,
#ifdef INDUCTOR_TILING_DATA
    const CVAutofuseTilingData& tilingGM,
#else
