...
inline __aicore__ void WhereExtend(const AscendC::LocalTensor<T> &dst, const AscendC::LocalTensor<uint8_t> &mask,
                                   T src0, T src1, const uint32_t size) {
...
template <typename T>
inline __aicore__ void WhereExtend(const AscendC::LocalTensor<T> &dst, const AscendC::LocalTensor<uint8_t> &mask,
                                   T src0, const AscendC::LocalTensor<T> &src1, const uint32_t size) {
  auto mask_tmp = mask.template ReinterpretCast<bool>();
  Where(dst, src0, src1, mask_tmp, size);
}
...
inline __aicore__ void WhereExtend(const LocalTensor<T> &dst, const LocalTensor<uint8_t> &mask,
  ...
  if constexpr (isBcastSrc0 && isBcastSrc1) {
    if constexpr (sizeof(T) != 8) {
      ...
      WhereNormal2DVecImpl<true, true, T>(dstLocal, maskLocal, src0Local, src1Local, dstStride, maskStride, srcStride,
                                          repeat, counterFirst, counterLast, scalar0, scalar1);
    } else {
      uint16_t repeat = static_cast<uint16_t>(CeilDivision(counterLast, 2 * GetVecLen() / sizeof(T)));
      WhereNormal2DVecImpl<true, true, T, MicroAPI::RegTraitNumTwo>(dstLocal, maskLocal, src0Local, src1Local,
                                                                    dstStride, maskStride, srcStride, repeat,
                                                                    counterFirst, counterLast, scalar0, scalar1);
    }
  } else if constexpr (isBcastSrc0 && !isBcastSrc1) {
    if constexpr (sizeof(T) != 8) {
      uint16_t repeat = static_cast<uint16_t>(CeilDivision(counterLast, GetVecLen() / sizeof(T)));
      WhereNormal2DVecImpl<true, false, T>(dstLocal, maskLocal, src0Local, src1Local, dstStride, maskStride, srcStride,
                                           repeat, counterFirst, counterLast, scalar0, scalar1);
    } else {
      uint16_t repeat = static_cast<uint16_t>(CeilDivision(counterLast, 2 * GetVecLen() / sizeof(T)));
      WhereNormal2DVecImpl<true, false, T, MicroAPI::RegTraitNumTwo>(dstLocal, maskLocal, src0Local, src1Local,
                                                                     dstStride, maskStride, srcStride, repeat,
                                                                     counterFirst, counterLast, scalar0, scalar1);
    }
  } else if constexpr (!isBcastSrc0 && isBcastSrc1) {
    if constexpr (sizeof(T) != 8) {
      uint16_t repeat = static_cast<uint16_t>(CeilDivision(counterLast, GetVecLen() / sizeof(T)));
      WhereNormal2DVecImpl<false, true, T>(dstLocal, maskLocal, src0Local, src1Local, dstStride, maskStride, srcStride,
                                           repeat, counterFirst, counterLast, scalar0, scalar1);
    } else {
      uint16_t repeat = static_cast<uint16_t>(CeilDivision(counterLast, 2 * GetVecLen() / sizeof(T)));
      WhereNormal2DVecImpl<false, true, T, MicroAPI::RegTraitNumTwo>(dstLocal, maskLocal, src0Local, src1Local,
                                                                     dstStride, maskStride, srcStride, repeat,
                                                                     counterFirst, counterLast, scalar0, scalar1);
    }
  } else {
    if constexpr (sizeof(T) != 8) {
      uint16_t repeat = static_cast<uint16_t>(CeilDivision(counterLast, GetVecLen() / sizeof(T)));
      WhereNormal2DVecImpl<false, false, T>(dstLocal, maskLocal, src0Local, src1Local, dstStride, maskStride, srcStride,
                                            repeat, counterFirst, counterLast, scalar0, scalar1);
    } else {
      uint16_t repeat = static_cast<uint16_t>(CeilDivision(counterLast, 2 * GetVecLen() / sizeof(T)));
      WhereNormal2DVecImpl<false, false, T, MicroAPI::RegTraitNumTwo>(dstLocal, maskLocal, src0Local, src1Local,
                                                                      dstStride, maskStride, srcStride, repeat,
                                                                      counterFirst, counterLast, scalar0, scalar1);
    }
  }
