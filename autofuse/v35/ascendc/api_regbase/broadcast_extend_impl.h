...
__aicore__ inline void GetBroadcastTilingInfoExtendImpl(
    uint32_t rank, const uint32_t* dstShape, const uint32_t* srcShape, bool srcInnerPad, BroadcastTilingExtend& tiling)
{
    static_assert(
...
__aicore__ inline void BroadcastComputeExtend(
        ...
        if (srcStride[dim - 1] == 0) {
            ...
            if constexpr (constRank == 2) {
                ...
                if constexpr (sizeof(T) == sizeof(uint64_t)) {
                    ...
                    if (dstShape[0] == 1 && oriSrcShape[0] == 1) {
                        BroadcastInternal::BrcNlastWrapperForThreeDim<BrcType, constRank>(dstUb, srcUb, dstShape + 1, srcStride + 1);
                    } else {
...
__aicore__ inline void BroadcastExtendImpl(
    ...
    BroadcastTilingExtend* tiling)
{
    ...
    BroadcastComputeExtend<T, constRank>(dst, src, newDstShape, dstStride, srcStride, dim, srcSize, dstSize, loopNum, tiling->oriSrcShape);
}
