...
constexpr float AN[] = {
    +3.46538101525629032477e-01, +1.20075952739645805542e+01, +7.62796053615234516538e+01, +1.68089224934630576269e+02,
    +1.59756391350164413639e+02, +7.05360906840444183113e+01, +1.40264691163389668864e+01, +9.99999999999999995305e-01,
};

constexpr float AD[] = {
    +5.67594532638770212846e-01, +1.47562562584847203173e+01, +8.45138970141474626562e+01, +1.77318088145400459522e+02,
    +1.64234692871529701831e+02, +7.14778400825575695274e+01, +1.40959135607834029598e+01, +1.00000000000000000470e+00,
};
...
constexpr float AGN[] = {
    +1.97339932091685679179e-02, +3.91103029615688277255e-01, +1.06579897599595591108e+00, +9.39169229816650230044e-01,
    +3.51465656105547619242e-01, +6.33888919628925490927e-02, +5.85804113048388458567e-03, +2.82851600836737019778e-04,
    +6.98793669997260967291e-06, +8.11789239554389293311e-08, +3.41551784765923618484e-10,
};

constexpr float AGD[] = {
    +9.30892908077441974853e+00, +1.98352928718312140417e+01, +1.55646628932864612953e+01, +5.47686069422975497931e+00,
    +9.54293611618961883998e-01, +8.64580826352392193095e-02, +4.12656523824222607191e-03, +1.01259085116509135510e-04,
    +1.17166733214413521882e-06, +4.91834570062930015649e-09,
};
...
__simd_callee__ inline void mainIterForAiryAi(Reg::RegTensor<T> &iterReg, Reg::RegTensor<T> &xReg, Reg::MaskReg &mask) {
...
template <typename T>
__simd_callee__ inline void AiryAiNeg(Reg::RegTensor<T> &srcReg, Reg::RegTensor<T> &constReg,
...
template <typename T>
__simd_callee__ inline void AiryAiMid(Reg::RegTensor<T> &srcReg, Reg::RegTensor<T> &tempRes, Reg::MaskReg &mask) {
...
template <typename T>
__simd_callee__ inline void AiryAiPos(Reg::RegTensor<T> &srcReg, Reg::RegTensor<T> &constReg,
                                      Reg::RegTensor<T> &tempRes, Reg::MaskReg &mask) {
...
template <typename T>
__aicore__ inline void AiryAiExtend(const LocalTensor<T> &dstLocal, const LocalTensor<T> &srcLocal,
                                    const LocalTensor<uint8_t> &sharedTmpBuffer, const uint32_t calCount) {
  static_assert(SupportType<T, float>(), "Current data type is not support on current device!");

  VF_CALL<AiryAiImplVF<T>>((__ubuf__ T *)dstLocal.GetPhyAddr(), (__ubuf__ T *)srcLocal.GetPhyAddr(), calCount);
}
