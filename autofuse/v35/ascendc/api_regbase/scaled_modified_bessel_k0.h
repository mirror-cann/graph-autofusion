...
__simd_callee__ inline void ScaledModifiedBesselK0BigSliceCompute(AscendC::Reg::RegTensor<T> &srcReg,
                                                                  ...
                                                                  __ubuf__ T *tmpBuf, uint32_t offSet,
                                                                  uint32_t tensorLen) {
...
template <typename T>
__simd_vf__ inline void ScaledModifiedBesselK0ImplVF(__ubuf__ T *dst, __ubuf__ T *src, __ubuf__ T *tmpBuf,
                                                     uint32_t calCount) {
  ...
  AscendC::Reg::RegTensor<T> srcReg, smallDstReg, bigDstReg, dstReg, factorReg;
  ...
  for (uint16_t i = 0U; i < repeatTime; ++i) {
    ...
    ScaledModifiedBesselK0BigSliceCompute<T, 1, 12, 0>(srcReg, bigDstReg, branchMask, dst, tmpBuf, i * vlSize,
                                                       tensorLen);
  }
  ...
  for (uint16_t i = 0U; i < repeatTime; ++i) {
    ...
    // ===== Big branch: x > 2.0 =====
    AscendC::Reg::Compares<T, CMPMODE::GT>(branchMask, srcReg, (T)2.0, mask);
    ScaledModifiedBesselK0BigSliceCompute<T, 13, 24, 1>(srcReg, bigDstReg, branchMask, dst, tmpBuf, i * vlSize,
                                                        tensorLen);
...
template <typename T>
__aicore__ inline void ScaledModifiedBesselK0Extend(const LocalTensor<T> &dst, const LocalTensor<T> &src,
  ...
  ScaledModifiedBesselK0ImplVF<T>((__ubuf__ T *)dst.GetPhyAddr(), (__ubuf__ T *)src.GetPhyAddr(),
                                  (__ubuf__ T *)tmpUB.GetPhyAddr(), calCount);
}

#endif  // __ASCENDC_API_REGBASE_SCALED_MODIFIED_BESSEL_K0_H__
