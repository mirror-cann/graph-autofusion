...
__simd_vf__ inline void BrcLastGatherOne(__ubuf__ T *dstUb, __ubuf__ T *srcUb, __ubuf__ IndexT *indexUb, uint16_t size0,
                                         uint16_t size1) {
...
template <typename T, typename IndexT>
__simd_vf__ inline void BrcLastGatherTwo(__ubuf__ T *dstUb, __ubuf__ T *srcUb, __ubuf__ IndexT *indexUb, uint16_t size0,
                                         uint16_t size1) {
  constexpr uint16_t VF_LEN = GetVecLen() / sizeof(T);
  constexpr uint32_t VF_LEN_HALF = GetVecLen() / 2 / sizeof(T);
...
template <typename T, typename IndexT>
__simd_vf__ inline void BrcNlastGatherOne(__ubuf__ T *dstUb, __ubuf__ T *srcUb, __ubuf__ IndexT *indexUb,
                                          uint16_t size0, uint16_t size1) {
  constexpr uint32_t VF_LEN_HALF = GetVecLen() / 2 / sizeof(T);
  uint32_t main = size0 * size1;
...
template <typename T, typename IndexT>
__simd_vf__ inline void BrcNlastGatherTwo(__ubuf__ T *dstUb, __ubuf__ T *srcUb, __ubuf__ IndexT *indexUb,
                                          uint16_t size0, uint16_t size1) {
  constexpr uint16_t VF_LEN = GetVecLen() / sizeof(T);
  constexpr uint32_t VF_LEN_HALF = GetVecLen() / 2 / sizeof(T);
  uint16_t factor = VF_LEN / size1;
...
template <typename T>
__simd_vf__ inline void BrcLastE2BLargerThanVL(__ubuf__ T *dstUb, __ubuf__ T *srcUb, uint16_t size0, uint16_t size1,
                                               uint16_t size2, uint16_t srcStride0) {
  constexpr uint16_t VF_LEN = GetVecLen() / sizeof(T);
...
template <typename T>
__simd_vf__ inline void BrcLastE2BLessThanVL(__ubuf__ T *dstUb, __ubuf__ T *srcUb, uint16_t size0, uint16_t size1,
                                             uint16_t size2, uint16_t srcStride0) {
  constexpr uint16_t VF_LEN = GetVecLen() / sizeof(T);
  uint32_t preg = size1 * size2;
  uint32_t sreg;
...
template <typename T>
__simd_vf__ inline void BrcLastE2B(__ubuf__ T *dstUb, __ubuf__ T *srcUb, uint16_t size0, uint16_t size1, uint16_t size2,
                                   uint16_t size3, uint16_t srcStride0, uint16_t srcStride1) {
  constexpr uint16_t VF_LEN = GetVecLen() / sizeof(T);
...
template <typename T>
__simd_vf__ inline void BrcNlastGatherBOne(__ubuf__ T *dstUb, __ubuf__ T *srcUb, __ubuf__ uint32_t *indexUb,
                                           uint16_t size0, uint16_t size1) {
  uint32_t main = size0 * size1;
...
template <typename T>
__simd_vf__ inline void BrcNlastGatherBTwo(__ubuf__ T *dstUb, __ubuf__ T *srcUb, __ubuf__ uint32_t *indexUb,
                                           uint16_t size0, uint16_t size1) {
  constexpr uint16_t VF_LEN = GetVecLen() / sizeof(T);
  uint16_t factor = VF_LEN / size1;
  uint16_t repeatTimes = CeilDivision(size0, factor) - 1;
...
template <typename T>
__simd_vf__ inline void BrcLastLessThanVLAligned(__ubuf__ T *dstUb, __ubuf__ T *srcUb, uint16_t size0, uint16_t size1,
                                                 uint16_t size2, uint16_t srcStride0, uint16_t srcStride1) {
  Reg::MaskReg pregCnt;
  Reg::RegTensor<T> srcReg;
...
template <typename T>
__simd_vf__ inline void BrcLastLessThanVLAligned(__ubuf__ T *dstUb, __ubuf__ T *srcUb, uint16_t size0, uint16_t size1,
                                                 uint16_t size2, uint16_t size3, uint16_t srcStride0,
                                                 uint16_t srcStride1, uint16_t srcStride2) {
  Reg::MaskReg pregCnt;
  Reg::RegTensor<T> srcReg;
...
template <typename T>
__simd_vf__ inline void BrcNlastLessThanVLAligned(__ubuf__ T *dstUb, __ubuf__ T *srcUb, uint16_t size0, uint16_t size1,
                                                  uint16_t size2, uint16_t size3, uint16_t srcStride0,
                                                  uint16_t srcStride1, uint16_t srcStride2) {
  Reg::MaskReg pregCnt;
  Reg::RegTensor<T> srcReg;
...
template <typename T>
__simd_vf__ inline void BrcNlastLessThanVLUnaligned(__ubuf__ T *dstUb, __ubuf__ T *srcUb, uint16_t size0,
                                                    uint16_t size1, uint16_t size2, uint16_t srcStride0,
                                                    uint16_t srcStride1) {
  Reg::MaskReg pregCnt;
  Reg::RegTensor<T> srcReg;
...
template <typename T>
__simd_vf__ inline void BrcNlastLessThanVLUnaligned(__ubuf__ T *dstUb, __ubuf__ T *srcUb, uint16_t size0,
                                                    uint16_t size1, uint16_t size2, uint16_t size3, uint16_t srcStride0,
                                                    uint16_t srcStride1, uint16_t srcStride2) {
  Reg::MaskReg pregCnt;
  Reg::RegTensor<T> srcReg;
  Reg::UnalignReg ureg0, ureg1;
    ...
    for (uint16_t j = 0; j < size2; ++j) {
      ...
      for (uint16_t k = 0; k < factor; ++k) {
        ...
        for (uint16_t t = 0; t < size1; ++t) {
          ...
          Reg::StoreAlign(dstUb + i * size1 * size2 * size3 + t * size2 * size3 + j * size3 + k * VF_LEN, srcReg,
                          pregCnt);
        }
      }
    }
...
template <typename T>
__simd_vf__ inline void BrcNlastLargerThanVLAlignedWithBlock(__ubuf__ T *dstUb, __ubuf__ T *srcUb, uint16_t size0,
                                                             uint16_t size1) {
  constexpr uint16_t VF_LEN = GetVecLen() / sizeof(T);
  uint16_t factor = CeilDivision(size1, VF_LEN);
...
template <typename T>
__simd_vf__ inline void BrcNlastLargerThanVLAlignedWithBlock(__ubuf__ T *dstUb, __ubuf__ T *srcUb, uint16_t size0,
                                                             uint16_t size1, uint16_t size2, uint16_t size3,
    ...
    for (uint16_t j = 0; j < factor; ++j) {
      ...
      for (uint16_t k = 0; k < size0; ++k) {
        for (uint16_t t = 0; t < size2; ++t) {
          ...
          Reg::StoreAlign(dstUb + k * size1 * size2 * size3 + i * size2 * size3 + t * size3 + j * VF_LEN, srcReg,
                          pregCnt);
        }
      }
    }
    ...
    for (uint16_t j = 0; j < factor; ++j) {
      for (uint16_t k = 0; k < size0; ++k) {
        for (uint16_t t = 0; t < size2; ++t) {
          ...
          Reg::StoreAlign(dstUb + j * VF_LEN + t * size3 + i * size2 * size3 + k * size1 * size2 * size3, srcReg,
                          pregFull);
        }
      }
    }
...
template <typename T, int32_t constRank = -1>
__aicore__ inline bool BrcLastWrapperForThreeDim(__ubuf__ T *dstUb, __ubuf__ T *srcUb, const uint32_t *dstShape,
...
template <typename T, int32_t constRank = -1>
__aicore__ inline bool BrcLastWrapperForFourDim(__ubuf__ T *dstUb, __ubuf__ T *srcUb, const uint32_t *dstShape,
                                                const uint32_t *srcStride) {
  constexpr uint16_t VF_LEN = GetVecLen() / sizeof(T);
  constexpr uint32_t VF_LEN_HALF = GetVecLen() / 2 / sizeof(T);
  constexpr uint32_t oneBlockElementNum = GetDataBlockSizeInBytes() / sizeof(T);
  ...
  if (sizeI[3] == oneBlockElementNum && sizeof(T) != sizeof(uint8_t) && stride[2] != 0 &&
    ...
    if (sizeI[3] % oneBlockElementNum == 0) {
      BrcLastLessThanVLAligned<T>(dstUb, srcUb, sizeI[0], sizeI[1], sizeI[2], sizeI[3], stride[0], stride[1],
                                  stride[2]);
    } else {
      if constexpr (constRank == -1) {
        return true;
      } else {
        BrcLastLessThanVLUnaligned<T>(dstUb, srcUb, sizeI[0], sizeI[1], sizeI[2], sizeI[3], stride[0], stride[1],
                                      stride[2]);
      }
    }
  } else {
    if (sizeI[3] % oneBlockElementNum == 0) {
      BrcLastLargerThanVLAligned<T>(dstUb, srcUb, sizeI[0], sizeI[1], sizeI[2], sizeI[3], stride[0], stride[1],
                                    stride[2]);
    } else {
      if constexpr (constRank == -1) {
        return true;
      } else {
        BrcLastLargerThanVLUnaligned<T>(dstUb, srcUb, sizeI[0], sizeI[1], sizeI[2], sizeI[3], stride[0], stride[1],
                                        stride[2]);
      }
    }
  }
...
template <typename T, int32_t constRank = -1>
__aicore__ inline bool BrcNlastWrapperForThreeDim(__ubuf__ T *dstUb, __ubuf__ T *srcUb, const uint32_t *dstShape,
                                                  const uint32_t *srcStride) {
  constexpr uint16_t VF_LEN = GetVecLen() / sizeof(T);
  constexpr uint32_t VF_LEN_HALF = GetVecLen() / 2 / sizeof(T);
  constexpr uint32_t oneBlockElementNum = GetDataBlockSizeInBytes() / sizeof(T);
  ...
  if (sizeI[2] < VF_LEN_HALF) {
    ...
    if (sizeI[2] % oneBlockElementNum == 0) {
      if (sizeI[2] % VF_LEN == 0) {
        ...
        BrcNlastLargerThanVLAlignedWithBlock<T>(dstUb, srcUb, sizeI[0], sizeI[1], sizeI[2], stride[0], stride[1]);
      }
    } else {
      if constexpr (constRank == -1) {
        return true;
...
template <typename T, int32_t constRank = -1>
__aicore__ inline bool BrcNlastWrapperForFourDim(__ubuf__ T *dstUb, __ubuf__ T *srcUb, const uint32_t *dstShape,
                                                 const uint32_t *srcStride) {
  constexpr uint16_t VF_LEN = GetVecLen() / sizeof(T);
  constexpr uint32_t VF_LEN_HALF = GetVecLen() / 2 / sizeof(T);
  constexpr uint32_t oneBlockElementNum = GetDataBlockSizeInBytes() / sizeof(T);
  ...
  if (sizeI[3] < VF_LEN_HALF && sizeof(T) != sizeof(uint8_t)) {
    ...
    if (sizeI[3] % oneBlockElementNum == 0) {
      BrcNlastLessThanVLAligned<T>(dstUb, srcUb, sizeI[0], sizeI[1], sizeI[2], sizeI[3], stride[0], stride[1],
                                   stride[2]);
    } else {
      if constexpr (constRank == -1) {
        return true;
      } else {
        BrcNlastLessThanVLUnaligned<T>(dstUb, srcUb, sizeI[0], sizeI[1], sizeI[2], sizeI[3], stride[0], stride[1],
                                       stride[2]);
      }
    }
  } else {
    if (sizeI[3] % oneBlockElementNum == 0) {
      if (sizeI[3] % VF_LEN == 0) {
        BrcNlastLargerThanVLAlignedWithVL<T>(dstUb, srcUb, sizeI[0], sizeI[1], sizeI[2], sizeI[3], stride[0], stride[1],
                                             stride[2]);
      } else {
        BrcNlastLargerThanVLAlignedWithBlock<T>(dstUb, srcUb, sizeI[0], sizeI[1], sizeI[2], sizeI[3], stride[0],
                                                stride[1], stride[2]);
      }
    } else {
      if constexpr (constRank == -1) {
        ...
        BrcNlastLargerThanVLUnaligned<T>(dstUb, srcUb, sizeI[0], sizeI[1], sizeI[2], sizeI[3], stride[0], stride[1],
                                         stride[2]);
      }
    }
  }
...
template <typename T>
__aicore__ inline void BrcNlastWrapperForMoreDim(__ubuf__ T *dstUb, __ubuf__ T *srcUb, const uint32_t *dstShape,
                                                 const uint32_t *dstStride, const uint32_t *srcStride) {
...
template <typename T>
__aicore__ inline void BrcNlastWrapperForMoreDimDynamicShape(__ubuf__ T *dstUb, __ubuf__ T *srcUb, const uint32_t dim,
        ...
        for (uint16_t t = 0; t < loops[3]; ++t) {
          for (uint16_t p = 0; p < loops[4]; ++p) {
            dstUb = dstUbTmp + p * dstStride[(dim - 5 + totalDim) % totalDim] +
                    t * dstStride[(dim - 6 + totalDim) % totalDim] + k * dstStride[(dim - 7 + totalDim) % totalDim] +
                    j * dstStride[(dim - 8 + totalDim) % totalDim] + i * dstStride[(dim - 9 + totalDim) % totalDim];
            srcUb = srcUbTmp + p * srcStride[(dim - 5 + totalDim) % totalDim] +
                    t * srcStride[(dim - 6 + totalDim) % totalDim] + k * srcStride[(dim - 7 + totalDim) % totalDim] +
                    j * srcStride[(dim - 8 + totalDim) % totalDim] + i * srcStride[(dim - 9 + totalDim) % totalDim];
            if (sizeI[3] < VF_LEN_HALF && sizeof(T) != sizeof(uint8_t)) {
              ...
              if (sizeI[3] % oneBlockElementNum == 0) {
                ...
                BrcNlastLessThanVLUnaligned<T>(dstUb, srcUb, sizeI[0], sizeI[1], sizeI[2], sizeI[3], stride[0],
                                               stride[1], stride[2]);
              }
            } else {
              if (sizeI[3] % oneBlockElementNum == 0) {
                ...
                BrcNlastLargerThanVLUnaligned<T>(dstUb, srcUb, sizeI[0], sizeI[1], sizeI[2], sizeI[3], stride[0],
                                                 stride[1], stride[2]);
              }
            }
          }
...
template <typename T>
__aicore__ inline void BrcLastWrapperForMoreDimDynamicShape(__ubuf__ T *dstUb, __ubuf__ T *srcUb, const uint32_t dim,
                                                            const uint32_t *dstShape, const uint32_t *dstStride,
                                                            const uint32_t *srcStride) {
  constexpr uint16_t VF_LEN = GetVecLen() / sizeof(T);
  constexpr uint32_t oneBlockElementNum = GetDataBlockSizeInBytes() / sizeof(T);
        ...
        for (uint16_t t = 0; t < loops[3]; ++t) {
          for (uint16_t p = 0; p < loops[4]; ++p) {
            ...
            if (sizeI[3] <= VF_LEN) {
              ...
              BrcLastLargerThanVLUnaligned<T>(dstUb, srcUb, sizeI[0], sizeI[1], sizeI[2], sizeI[3], stride[0],
                                              stride[1], stride[2]);
            }
          }
        }
