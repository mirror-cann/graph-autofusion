...
template <typename T>
__simd_vf__ inline void ScaledModifiedBesselK1ImplVF(__ubuf__ T *dst, __ubuf__ T *src, __ubuf__ T *tmpBuf,
                                                     uint32_t calCount) {
  ...
  for (uint16_t i = 0U; i < repeatTime; ++i) {
    ...
    ScaledModifiedBesselK1BigSliceCompute<T, 1, 12, 0>(srcReg, bigDstReg, branchMask, dst, tmpBuf, i * vlSize,
                                                       tensorLen);
  }
  ...
  for (uint16_t i = 0U; i < repeatTime; ++i) {
    ...
    // ===== Big branch: x > 2.0 =====
    AscendC::Reg::Compares<T, CMPMODE::GT>(branchMask, srcReg, (T)2.0, mask);
    ScaledModifiedBesselK1BigSliceCompute<T, 13, 24, 1>(srcReg, bigDstReg, branchMask, dst, tmpBuf, i * vlSize,
                                                        tensorLen);
...
__aicore__ inline void ScaledModifiedBesselK1Extend(const LocalTensor<T> &dst, const LocalTensor<T> &src,
  ...
  ScaledModifiedBesselK1ImplVF<T>((__ubuf__ T *)dst.GetPhyAddr(), (__ubuf__ T *)src.GetPhyAddr(),
