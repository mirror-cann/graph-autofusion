 ...
 */
#ifndef __ASCENDC_API_REGBASE_HERMITE_POLYNOMIAL_HE_H__
#define __ASCENDC_API_REGBASE_HERMITE_POLYNOMIAL_HE_H__

constexpr int32_t HERMITE_POLYNOMIAL_HE_LIMIT = 128;

template <typename T>
__simd_vf__ inline void HermitePolynomialHeFillVF(__ubuf__ T *dstUb, uint32_t calCount, T fillValue) {
 ...
 */
template <typename T>
__simd_vf__ inline void HermitePolynomialHeLinearVF(__ubuf__ T *srcUb, __ubuf__ T *dstUb, uint32_t calCount) {
  const uint32_t repeatElem = static_cast<uint32_t>(AscendC::GetVecLen() / sizeof(T));
  const uint16_t repeatTime = static_cast<uint16_t>(AscendC::CeilDivision(calCount, repeatElem));
  ...
  for (uint16_t i = 0U; i < repeatTime; ++i) {
    ...
    AscendC::Reg::StoreAlign(dstUb + i * repeatElem, xReg, activeMask);
  }
}

/**
 * n >= 2 且 n <= 128时使用。
 * 递推公式：He_m = x · He_{m-1} - (m-1) · He_{m-2}
 */
template <typename T>
__simd_vf__ inline void HermitePolynomialHeImplVF(__ubuf__ T *srcUb, __ubuf__ T *dstUb, int32_t n, uint32_t calCount) {
  const uint32_t repeatElem = static_cast<uint32_t>(AscendC::GetVecLen() / sizeof(T));
  const uint16_t repeatTime = static_cast<uint16_t>(AscendC::CeilDivision(calCount, repeatElem));
  ...
  for (uint16_t i = 0U; i < repeatTime; ++i) {
    activeMask = AscendC::Reg::UpdateMask<T>(calCount);
    AscendC::Reg::LoadAlign(xReg, srcUb + i * repeatElem);
    AscendC::Reg::Duplicate(pReg, static_cast<T>(1.0f), activeMask);  // pReg = He_0 = 1
    AscendC::Reg::Mul(qReg, xReg, oneReg, activeMask);                // qReg = He_1 = x
    ...
    for (uint8_t k = 0U; k < static_cast<uint8_t>(n - 1); ++k) {
      AscendC::Reg::Mul(kpReg, negKReg, pReg, nonNanMask);     // kp = -k · He_{m-2}（等价于 Mul+Neg）
      AscendC::Reg::MulAddDst(kpReg, xReg, qReg, nonNanMask);  // kp = -k·He_{m-2} + x·He_{m-1} = He_m
...
/**
 * 对外接口（标量 n 版）：计算概率学中埃尔米特多项式 He_n(x)。
 *
...
__aicore__ inline void HermitePolynomialHeExtend(const AscendC::LocalTensor<T> &dst, const AscendC::LocalTensor<T> &src,
                                                 U n, const AscendC::LocalTensor<uint8_t> &sharedTmpBuffer,
                                                 const uint32_t calCount) {
  if ASCEND_IS_AIC {
    return;
  }
  static_assert(std::is_same<T, float>::value, "HermitePolynomialHe currently only supports float");
  ...
  if (order < 0) {
    HermitePolynomialHeFillVF<T>(dstUb, calCount, static_cast<T>(0.0f));
    return;
  }

  if (order == 0) {
    HermitePolynomialHeFillVF<T>(dstUb, calCount, static_cast<T>(1.0f));
    return;
  }

  if (order > HERMITE_POLYNOMIAL_HE_LIMIT) {
    HermitePolynomialHeFillVF<T>(dstUb, calCount, AscendC::NumericLimits<T>::QuietNaN());
    return;
  }

  __ubuf__ T *srcUb = (__ubuf__ T *)src.GetPhyAddr();
  if (order == 1) {
    HermitePolynomialHeLinearVF<T>(srcUb, dstUb, calCount);
    return;
  }

  HermitePolynomialHeImplVF<T>(srcUb, dstUb, order, calCount);
}
...
__aicore__ inline void HermitePolynomialHeExtend(const AscendC::LocalTensor<T> &dst, const AscendC::LocalTensor<T> &src,
                                                 ...
                                                 const uint32_t calCount) {
  HermitePolynomialHeExtend(dst, src, n.GetValue(0), sharedTmpBuffer, calCount);
}

#endif  // __ASCENDC_API_REGBASE_HERMITE_POLYNOMIAL_HE_H__
