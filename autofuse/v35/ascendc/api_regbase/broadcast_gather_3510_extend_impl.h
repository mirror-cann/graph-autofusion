...
template <typename T>
__simd_vf__ inline void VfGenIndex(__ubuf__ T *indexUb, uint32_t sizeI0, uint32_t sizeI1, uint32_t sizeI2,
                                   uint32_t strideI0, uint32_t strideI1, uint32_t strideI2) {
  Reg::RegTensor<T> v0;
  Reg::RegTensor<T> v1;
  Reg::RegTensor<T> v2;
...
__simd_vf__ inline void VfGenIndexB8(__ubuf__ int16_t *indexUb, uint32_t sizeI0, uint32_t sizeI1, uint32_t sizeI2,
                                     uint32_t strideI0, uint32_t strideI1, uint32_t strideI2) {
...
template <typename T>
__simd_vf__ inline void VfGenIndexForFourDim(__ubuf__ T *indexUb, uint32_t sizeI0, uint32_t sizeI1, uint32_t sizeI2,
                                             uint32_t sizeI3, uint32_t strideI0, uint32_t strideI1, uint32_t strideI2,
                                             uint32_t strideI3) {
  Reg::RegTensor<T> v0;
  Reg::RegTensor<T> v1;
  Reg::RegTensor<T> v2;
...
template <typename T>
__simd_vf__ inline void VfGatherBrc(__ubuf__ T *dstUb, __ubuf__ T *srcUb, __ubuf__ T *indexUb, uint16_t size1,
                                    uint16_t size2, uint16_t srcStride1, uint16_t srcStride2, uint32_t main,
                                    uint32_t tail) {
  Reg::UnalignReg u0;
...
template <typename T>
__simd_vf__ inline void VfGatherBrcB8(__ubuf__ T *dstUb, __ubuf__ T *srcUb, __ubuf__ uint16_t *indexUb, uint16_t size1,
                                      uint16_t size2, uint16_t srcStride1, uint16_t srcStride2, uint32_t main,
                                      uint32_t tail) {
...
template <typename T>
__simd_vf__ inline void VfGatherBrcForFourDim(__ubuf__ T *dstUb, __ubuf__ T *srcUb, __ubuf__ T *indexUb, uint16_t size1,
                                              uint16_t size2, uint16_t size3, uint16_t srcStride1, uint16_t srcStride2,
                                              uint16_t srcStride3, uint32_t main, uint32_t tail) {
  Reg::UnalignReg u0;
  Reg::RegTensor<T> vindex0;
  Reg::RegTensor<T> vindex;
  Reg::RegTensor<T> vstride1;
...
__aicore__ inline void GenGatherIndex(__ubuf__ T *indexUb, const uint32_t *size, const uint32_t *srcStride) {
  ...
  if constexpr (sizeof(T) == sizeof(uint8_t)) {
    VfGenIndexB8((__ubuf__ int16_t *)indexUb, sizeI[0], sizeI[1], sizeI[2], srcStride[0], srcStride[1], srcStride[2]);
  } else {
...
template <typename T>
__aicore__ inline void GenGatherIndexForFourDim(__ubuf__ T *indexUb, const uint32_t *size, const uint32_t *srcStride) {
  constexpr uint32_t VF_LEN = GetVecLen() / sizeof(T);
  uint32_t sizeI[4];
  ...
  VfGenIndexForFourDim<T>(indexUb, sizeI[0], sizeI[1], sizeI[2], sizeI[3], srcStride[0], srcStride[1], srcStride[2],
                          srcStride[3]);
}

template <typename T>
__aicore__ inline void GatherWrapper(__ubuf__ T *dstUb, __ubuf__ T *srcUb, const uint32_t *size,
  ...
  if constexpr (sizeof(T) == sizeof(uint8_t)) {
    ...
    VfGatherBrcB8<T>(dstUb, srcUb, (__ubuf__ uint16_t *)indexUb.GetPhyAddr(), sizeI1, sizeI2, srcStrideI1, srcStrideI2,
                     main, tail);
    ...
    VfGatherBrc<uint32_t>((__ubuf__ uint32_t *)dstUb, (__ubuf__ uint32_t *)srcUb,
                          (__ubuf__ uint32_t *)indexUb.GetPhyAddr(), sizeI1, sizeI2, srcStrideI1, srcStrideI2, main,
                          tail);
  } else {
    ...
    VfGatherBrc<uint16_t>((__ubuf__ uint16_t *)dstUb, (__ubuf__ uint16_t *)srcUb,
                          (__ubuf__ uint16_t *)indexUb.GetPhyAddr(), sizeI1, sizeI2, srcStrideI1, srcStrideI2, main,
                          tail);
  }
}

template <typename T>
__aicore__ inline void GatherWrapperForFourDim(__ubuf__ T *dstUb, __ubuf__ T *srcUb, const uint32_t *size,
                                               const uint32_t *srcStride) {
  constexpr uint32_t VF_LEN = GetVecLen() / sizeof(T);
  ...
  if constexpr (sizeof(T) == sizeof(uint32_t)) {
    ...
    VfGatherBrcForFourDim<uint32_t>((__ubuf__ uint32_t *)dstUb, (__ubuf__ uint32_t *)srcUb,
                                    (__ubuf__ uint32_t *)indexUb.GetPhyAddr(), sizeI1, sizeI2, sizeI3, srcStrideI1,
                                    srcStrideI2, srcStrideI3, main, tail);
  } else {
    ...
    VfGatherBrcForFourDim<uint16_t>((__ubuf__ uint16_t *)dstUb, (__ubuf__ uint16_t *)srcUb,
                                    (__ubuf__ uint16_t *)indexUb.GetPhyAddr(), sizeI1, sizeI2, sizeI3, srcStrideI1,
                                    srcStrideI2, srcStrideI3, main, tail);
  }
}
}  // namespace BroadcastInternal
