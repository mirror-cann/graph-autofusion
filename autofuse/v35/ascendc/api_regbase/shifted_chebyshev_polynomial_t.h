...
#ifndef __ASCENDC_API_REGBASE_SHIFTED_CHEBYSHEV_POLYNOMIAL_T_H__
#define __ASCENDC_API_REGBASE_SHIFTED_CHEBYSHEV_POLYNOMIAL_T_H__

using namespace AscendC;

template <typename T, int64_t N>
__aicore__ inline void ShiftedChebyshevPolynomialTCal(__ubuf__ T *dst, __ubuf__ T *src, uint32_t calCount) {
  ...
  for (uint16_t i = 0U; i < repeatTime; ++i) {
    ...
    Reg::Adds(dstReg, dstReg, (T)(-1.0), mask);  // dstReg = 2x-1

    Reg::Muls(srcReg, dstReg, (T)2.0, mask);  // srcReg = 4x-2

    Reg::Duplicate(temp1Reg, (T)1.0, mask);  // temp1Reg = 1
...
template <typename T, int64_t N>
__aicore__ inline void ShiftedChebyshevPolynomialTExtend(const LocalTensor<T> &dst, const LocalTensor<T> &src,
  ...
  if constexpr (N < 0) {
    ...
    VF_CALL<ShiftedChebyshevPolynomialTCal<T, N>>((__ubuf__ T *)dst.GetPhyAddr(), (__ubuf__ T *)src.GetPhyAddr(),
                                                  calCount);
  }
}
