...
template <typename T>
__simd_vf__ inline void LegendrePolynomialPLinearVF(__ubuf__ T *xAddr, __ubuf__ T *yAddr, uint32_t calCount) {
  const uint32_t repeatElem = static_cast<uint32_t>(AscendC::GetVecLen() / sizeof(T));
  const uint16_t repeatTime = static_cast<uint16_t>(AscendC::CeilDivision(calCount, repeatElem));
  ...
  for (uint16_t i = 0U; i < repeatTime; ++i) {
    ...
    AscendC::Reg::StoreAlign(yAddr + i * repeatElem, xReg, activeMask);
  }
}
...
__simd_vf__ inline void LegendrePolynomialPImplVF(__ubuf__ T *xAddr, __ubuf__ T *yAddr, int32_t n,
  ...
  AscendC::Reg::MaskReg negOneMask;
  AscendC::Reg::RegTensor<T> xReg;
  ...
  for (uint16_t i = 0U; i < repeatTime; ++i) {
    ...
    AscendC::Reg::Select(qReg, xReg, xReg, activeMask);
    ...
    for (int32_t k = 0; k < n - 1; ++k) {
      AscendC::Reg::Mul(nextReg, coefReg, xReg, nonNanMask);
      ...
      AscendC::Reg::Div<T, &kLegendreHighPrecisionDivMode>(nextReg, kpReg, denReg, nonNanMask);
    ...
    AscendC::Reg::Compare<T, AscendC::CMPMODE::EQ>(posOneMask, xReg, oneReg, activeMask);
    AscendC::Reg::Neg(nextReg, oneReg, activeMask);
    AscendC::Reg::Compare<T, AscendC::CMPMODE::EQ>(negOneMask, xReg, nextReg, activeMask);
    AscendC::Reg::Select(qReg, oneReg, qReg, posOneMask);
    if ((n & 1) == 0) {
      AscendC::Reg::Select(qReg, oneReg, qReg, negOneMask);
    } else {
      AscendC::Reg::Select(qReg, nextReg, qReg, negOneMask);
    }
...
__aicore__ inline void LegendrePolynomialPExtend(const AscendC::LocalTensor<T> &dst,
  ...
  if (order < 0) {
    LegendrePolynomialPFillVF<T>(dstAddr, calcount, static_cast<T>(0.0f));
    return;
  }

  if (order == 0) {
    LegendrePolynomialPFillVF<T>(dstAddr, calcount, static_cast<T>(1.0f));
    return;
  }

  __ubuf__ T *srcAddr = (__ubuf__ T *)src.GetPhyAddr();
  if (order == 1) {
    LegendrePolynomialPLinearVF<T>(srcAddr, dstAddr, calcount);
    return;
  }

  LegendrePolynomialPImplVF<T>(srcAddr, dstAddr, order, calcount);
}
...
#endif  // __ASCENDC_API_REGBASE_LEGENDRE_POLYNOMIAL_P_H__