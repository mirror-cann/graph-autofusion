                                     ...
                                     __ubuf__ uint32_t* srcUb, uint32_t calc_cnt)
{
    ...
    for (uint16_t i = 0; i < repeat_time; ++i) {
        ...
        Reg::Adds((Reg::RegTensor<int32_t>&)(expField),
                  (Reg::RegTensor<int32_t>&)(expField), static_cast<int16_t>(-126), mask);
        ...
        for (uint32_t si = 0; si < 23; ++si) {
            Reg::Compares<uint32_t, CMPMODE::LT>(needsShiftMask, mantField,
                          static_cast<uint32_t>(1 << 22), subnormOnlyMask);
        ...
        Reg::Select((Reg::RegTensor<int32_t>&)(expField),
        ...
        Reg::StoreAlign(exponentUb + i * vl_size,
                        (Reg::RegTensor<int32_t>&)(expField), mask);
    }
}

template <typename T>
__aicore__ inline void FrexpExtend(const LocalTensor<T>& mantissa,
                                   const LocalTensor<int32_t>& exponent,
