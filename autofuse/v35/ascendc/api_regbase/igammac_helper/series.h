...
constexpr float kLanczosNum[13] = {
    0.006061842346248906525783753964555936883222f, 0.5098416655656676188125178644804694509993f,
    19.51992788247617482847860966235652136208f,    449.9445569063168119446858607650988409623f,
    6955.999602515376140356310115515198987526f,    75999.29304014542649875303443598909137092f,
    601859.6171681098786670226533699352302507f,    3481712.15498064590882071018964774556468f,
    14605578.08768506808414169982791359218571f,    43338889.32467613834773723740590533316085f,
    86363131.28813859145546927288977868422342f,    103794043.1163445451906271053616070238554f,
    56906521.91347156388090791033559122686859f};

constexpr float kLanczosDen[13] = {1.0f,         66.0f,       1925.0f,     32670.0f,     357423.0f,
                                   2637558.0f,   13339535.0f, 45995730.0f, 105258076.0f, 150917976.0f,
                                   120543840.0f, 39916800.0f, 0.0f};
}  // anonymous namespace

namespace IGammaCInternal {

template <int32_t iterationNum>
__simd_callee__ inline void LanczosNumIter(Reg::RegTensor<float> &numAnsReg, Reg::RegTensor<float> &yReg,
...
template <int32_t iterationNum>
__simd_callee__ inline void LanczosDenIter(Reg::RegTensor<float> &denAnsReg, Reg::RegTensor<float> &yReg,
                                           Reg::MaskReg &mask0, Reg::MaskReg &mask1, Reg::MaskReg &fullMask) {
  Reg::RegTensor<float> tmpReg;
...
__simd_callee__ inline void Igammac_lanczos_sum_expg_scaled_float(Reg::RegTensor<float> &dstReg,
                                                                  Reg::RegTensor<float> &src0Reg, Reg::MaskReg mask) {
  // return igammac_ratevl_float(a, kLanczosNum, (int64_t)(sizeof(kLanczosNum) / sizeof(kLanczosNum[0] -1)),
  // kLanczosDen, (int64_t)(sizeof(kLanczosNum) / sizeof(kLanczosNum[0] -1))) return igammac_ratevl_float(a,
  // kLanczosNum, (int64_t)(12), kLanczosDen, (int64_t)(12))
...
__simd_callee__ inline void Igammac_helper_fac_float(Reg::RegTensor<float> &dstReg, Reg::RegTensor<float> &src0Reg,
                                                     Reg::RegTensor<float> &src1Reg, Reg::RegTensor<float> &powReg,
                                                     Reg::RegTensor<float> &lgammaReg, Reg::MaskReg fullMask) {
...
__simd_callee__ inline void Igammac_helper_series_float(Reg::RegTensor<float> &dstReg, Reg::RegTensor<float> &src0Reg,
                                                        Reg::RegTensor<float> &src1Reg,
                                                        Reg::RegTensor<float> &lgammaReg, Reg::RegTensor<float> &powReg,
                                                        Reg::MaskReg mask) {
