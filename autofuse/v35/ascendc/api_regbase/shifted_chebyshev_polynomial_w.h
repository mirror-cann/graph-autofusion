...
#ifndef __ASCENDC_API_REGBASE_SHIFTED_CHEBYSHEV_POLYNOMIAL_W_H__
#define __ASCENDC_API_REGBASE_SHIFTED_CHEBYSHEV_POLYNOMIAL_W_H__
...
__aicore__ inline void ShiftedChebyshevPolynomialWCal(__ubuf__ T *dst, __ubuf__ T *src, uint32_t calCount) {
  ...
  for (uint16_t i = 0U; i < repeatTime; ++i) {
    ...
    Reg::Adds(dstReg, dstReg, (T)(-1.0), mask);  // dstReg = 4x-1
...
template <typename T, int64_t N>
__aicore__ inline void ShiftedChebyshevPolynomialWExtend(const LocalTensor<T> &dst, const LocalTensor<T> &src,
  ...
  if constexpr (N < 0) {
    ...
    VF_CALL<ShiftedChebyshevPolynomialWCal<T, N>>((__ubuf__ T *)dst.GetPhyAddr(), (__ubuf__ T *)src.GetPhyAddr(),
                                                  calCount);
  }
}
