...
std::vector<std::unique_ptr<TmpBufDesc>> CalcCosTmpSizeV2(const AscNode &node) {
  ...
  constexpr uint32_t COS_FLOAT_CALC_FAC = 3;
  constexpr uint32_t COS_HALF_CALC_FAC = 8;
  constexpr uint32_t COS_ONE_REPEAT_BYTE_SIZE = 256;
  Expression buf_nums =
      ((node_inputs[0].attr.dtype == ge::DT_FLOAT) ? Symbol(COS_FLOAT_CALC_FAC) : Symbol(COS_HALF_CALC_FAC));
  Expression total_size = buf_nums * Symbol(COS_ONE_REPEAT_BYTE_SIZE);
