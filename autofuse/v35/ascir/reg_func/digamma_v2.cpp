...
CalcDigammaTmpSizeV2(const AscNode &node) {
  constexpr uint32_t DIGAMMA_FLOAT_CALC_PROC = 7;
  constexpr uint32_t DIGAMMA_HALF_CALC_PROC = 16;
  ...
  uint32_t calc_tmp_buf = DIGAMMA_ONE_REPEAT_BYTE_SIZE * (node_inputs[0].attr.dtype == ge::DT_FLOAT16
                          ? DIGAMMA_HALF_CALC_PROC : DIGAMMA_FLOAT_CALC_PROC);
