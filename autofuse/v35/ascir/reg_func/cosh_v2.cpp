...
CalcCoshTmpSizeV2(const AscNode &node) {
  ...
  Expression buf_nums =
      ((node_inputs[0].attr.dtype == ge::DT_FLOAT) ? Symbol(COSH_FLOAT_CALC_FAC) : Symbol(COSH_HALF_CALC_FAC));
  Expression total_size = buf_nums * Symbol(COSH_ONE_REPEAT_BYTE_SIZE);
