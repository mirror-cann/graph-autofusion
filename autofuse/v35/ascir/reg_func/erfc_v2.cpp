...
CalcErfcTmpSizeV2(const AscNode &node) {
  constexpr uint32_t ERFC_FLOAT_CALC_PROC = 7;
  constexpr uint32_t ERFC_HALF_CALC_PROC = 16;
  ...
  uint32_t calc_tmp_buf = ERFC_ONE_REPEAT_BYTE_SIZE * (node_inputs[0].attr.dtype == ge::DT_FLOAT16
                          ? ERFC_HALF_CALC_PROC : ERFC_FLOAT_CALC_PROC);
