...
std::vector<std::unique_ptr<TmpBufDesc>> CalcLog2TmpSizeV2(const AscNode &node) {
  std::map<ge::DataType, ge::DataType> dtype_conversion_map = {
      {DT_FLOAT, DT_FLOAT}, {DT_FLOAT16, DT_FLOAT16}, {DT_BF16, DT_FLOAT}};
  ...
  uint32_t calcTmpBuf = ((dtype_conversion_map[node_inputs[0].attr.dtype] == ge::DT_FLOAT16) ? LOG2_TMP_SIZE : 0);
