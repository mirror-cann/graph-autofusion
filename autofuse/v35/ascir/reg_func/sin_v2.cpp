...
std::vector<std::unique_ptr<TmpBufDesc>> CalcSinTmpSizeV2(const AscNode &node) {
  constexpr uint32_t SIN_HALF_CALC_PROC = 8;
  constexpr uint32_t SIN_FLOAT_CALC_PROC = 3;
  ...
  uint32_t calc_tmp_buf = SIN_ONE_REPEAT_BYTE_SIZE *
                          (node_inputs[0].attr.dtype == ge::DT_FLOAT16 ? SIN_HALF_CALC_PROC : SIN_FLOAT_CALC_PROC);
