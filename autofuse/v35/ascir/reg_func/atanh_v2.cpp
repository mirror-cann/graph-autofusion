...
CalcAtanhTmpSizeV2(const AscNode &node) {
  constexpr uint32_t ATANH_FLOAT_CALC_PROC = 1;
  constexpr uint32_t ATANH_HALF_CALC_PROC = 4;
  constexpr uint32_t ATANH_ONE_REPEAT_BYTE_SIZE = 256;
  auto node_inputs = node.inputs;
  GE_ASSERT_TRUE(node_inputs.Size() > 0, "Node %s[%s] inputs size is 0.",
                 node.GetTypePtr(), node.GetNamePtr());
  uint32_t calc_tmp_buf = ATANH_ONE_REPEAT_BYTE_SIZE * (node_inputs[0].attr.dtype == ge::DT_FLOAT16
                          ? ATANH_HALF_CALC_PROC : ATANH_FLOAT_CALC_PROC);
