...
class AttUtils {
 public:
  ...
  static void CollectReduceAxisNames(const NodeInfo &node_info, std::set<std::string> &reduce_axis_orig_names);

  // 收集Broadcast轴的原始轴名称（供 ascend_graph_parser 和 arg_list_reorder 共用）
  static void CollectBroadcastAxisNames(const NodeInfo &node_info, std::set<std::string> &broadcast_axis_orig_names);
};
}  // namespace att
