...
#ifndef ATT_TERNARY_OP_H_
#define ATT_TERNARY_OP_H_
...
class IfCase {
 public:
  ...
  IfCase(const CondType &type_cond, const Expr &left_cond, const Expr &right_cond, std::shared_ptr<IfCase> &&if_case_a,
         std::shared_ptr<IfCase> &&if_case_b)
      : cond_type_(type_cond), cond_left_(left_cond), cond_right_(right_cond) {
  ...
  bool IsLeaf() const {
    return choice_b_ == nullptr;
  }
  const Expr &GetExpr() const {
    return expr_;
  }
  const Expr &GetCondLeft() const {
    return cond_left_;
  }
  const Expr &GetCondRight() const {
    return cond_right_;
  }
  CondType GetCondType() const {
    return cond_type_;
  }
  std::shared_ptr<IfCase> GetChoiceA() const {
    return choice_a_;
  }
  std::shared_ptr<IfCase> GetChoiceB() const {
    return choice_b_;
  }
...
class TernaryOp {
 public:
  TernaryOp() = default;
  explicit TernaryOp(const Expr &expr);
  TernaryOp(const CondType &cond_type, const Expr &cond_left, const Expr &cond_right, const Expr &choice_a,
            const Expr &choice_b);
  TernaryOp(const Expr &var, std::shared_ptr<IfCase> &&op, const std::vector<Expr> &related);
  TernaryOp(const CondType &cond_type, const Expr &cond_left, const Expr &cond_right,
            std::shared_ptr<IfCase> &&if_case_a, std::shared_ptr<IfCase> &&if_case_b);
  TernaryOp(const CondType &cond_type, const Expr &cond_left, const Expr &cond_right, const TernaryOp &ternary_op_a,
            const TernaryOp &ternary_op_b);
 ...
 private:
  ...
  std::shared_ptr<IfCase> ternary_op_;
