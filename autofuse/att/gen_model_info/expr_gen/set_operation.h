...
struct DimRange {
  ...
  bool operator==(const DimRange &v) const {
    return (this->upper_bound == v.upper_bound) && (this->lower_bound == v.lower_bound);
  }
};
...
class SetOperation {
 public:
  ...
  // 计算并集
  static void ProductImplement(std::vector<std::vector<uint32_t>> &seq, std::vector<std::vector<uint32_t>> &res,
