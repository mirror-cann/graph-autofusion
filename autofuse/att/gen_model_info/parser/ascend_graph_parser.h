...
private:
  ...
  // 把ascir 轴转换为 att轴信息
  void ParserSubAxis(const af::AxisPtr &axis, SubAxisPtr &sub_axis_ptr) const;

  // 构建轴之间的关系，轴的父轴和原始轴
  void MakeSubAxisRelation(void);

  // 解析tensor内存信息
  af::Status ConstructQueueContainer(const af::AscTensorAttr &ascir_tensor_info);
  ...
  // 计算tensor大小
  af::Status ParseTensorDims(TensorPtr &tensor, af::AscTensorAttr &tensor_attr);
  ...
  // 获取节点归属于哪些Data节点
  af::Status GetNodeFromData(const af::AscNodePtr &ge_node, NodeInfo &node_info);

  // 转换node信息到tuning space
  af::Status ConvertNodeInfos(const af::AscNodePtr &ge_node, const ScheduleAttr &attrs, const af::AscGraph &graph,
                              const bool use_cache_flag);

  // 把graph信息解析为tuning space
  af::Status ConvertToTuningSpace(const af::AscGraph &graph);

  // 更新tensor信息到tuning space
  void AssembleTensorInfos();

  // 解析ascir graph可选属性信息
  void ParserOptionalInfos(const af::AscGraph &graph) const;

  // 校验轴id是否有效
  af::Status CheckAxisIdValid(const int64_t axis_id);

  af::Status CheckAxisIdValid(std::vector<int64_t> &axis_ids);
  ...
  void SetContinuesStrides(TensorPtr &tensor, af::AscTensorAttr &tensor_attr) const;
  ...
  af::Status CalculateReservedUbSize(const af::AscGraph &graph);

  // 检测Reduce/Broadcast分核Store冲突场景
  af::Status CheckReduceBroadcastSplitStoreConflict();
