...
inline std::string GenL0VarCmpAnnotation() {
  ...
  strs +=
      ...
      "类型变量的大小。它遵循特定的比较逻辑：\n";
  strs +=
      " * 1. 如果 a 变量绑定到多核且 b 变量没有绑定多核，则 a 被认为大于 "
      "b，函数返回\n";
  strs += " * true。\n";
  strs +=
      " * 2. 如果 a 变量没有绑定多核且 b 变量绑定多核，则 a 被认为小于 "
      "b，函数返回\n";
  strs += " * false。\n";
  strs +=
      " * 3. 如果 a 和 b 变量都绑定多核或都未绑定多核，则比较它们的 "
      "prompt_align\n";
...
inline std::string GenGetMacUseAnnotaion() {
  ...
  strs +=
      "   * 通过不断更新 usage 变量，乘法操作确保了所有 L0 "
      "变量的影响都被计入。\n";
  strs +=
      "   * 最终 usage 变量中的值就是所有 L0 "
      "变量值的乘积，代表了整体的数据处理量。\n";
  strs +=
      "   * "
...
inline std::string GenL0TileSolver() {
  ...
  strs +=
      "  int32_t MaxCoreNum(const L0Var *l0_vars, const uint32_t "
      "&core_num);\n";
  strs += "\n";
...
inline std::string GenMaxCoreNumFuncAnnotation() {
  ...
  strs += " *\n";
  strs +=
      " * 这个函数遍历 L0Var 结构数组，根据每个变量的 bind_multicore "
      "属性以及\n";
...
inline std::string GenMaxCoreNumFunc() {
  ...
  strs +=
      "            ? ((var.max_value + std::max(var.value, var.prompt_align) "
      "- 1)) /\n";
...
inline std::string GenGetMacUseFuncAnnotation() {
  ...
  strs += " *\n";
  strs +=
      " * 这个函数遍历 L0TileInput 结构体中的所有 L0Var 对象，计算它们的 "
      "value\n";
  strs += " * 属性的乘积。这个乘积代表了所有 L0\n";
  strs +=
      " * 变量值的联合效应，或者说数据处理量的一个度量。 通过不断更新 "
      "usage\n";
...
inline std::string GenIterativeRunFuncAnnotation() {
  ...
  strs +=
      ...
      "对齐要求。\n";
  strs +=
      " * 如果满足这些条件，它将继续下一个循环或者递归调用自身来处理下一个L0 "
      "变量。\n";
  strs +=
      " * 如果是最后一个 L0 "
      "变量，它将检查当前组合是否满足优化条件，如核心数量和数据处理量。如果满"
...
inline std::string GenIterativeRunFunc() {
  ...
  strs +=
      "void L0TileSolver::IterativeRun(uint32_t loop_id, uint32_t "
      "*best_var_value) {\n";
  ...
  strs +=
      "      int32_t core_num = MaxCoreNum(input_.l0_vars, "
      "input_.core_num);\n";
  strs +=
      "      // "
      "最大核数如果满足核数*系数（默认0."
...
inline std::string GenUpdateAlignFuncAnnotation() {
  ...
  strs +=
      " * 值，以确保它们在内存中按照最优方式对齐。它遍历 input_ 对象中的 "
      "l0_vars\n";
...
inline std::string GenCheckInputFuncAnnotation() {
  ...
  strs +=
      " * - 对于输入数据中的每个 L0Var 对象（通过索引 i "
      "访问），它检查几个属性：\n";
  strs += " *   - max_value、align 和 prompt_align 是否都不等于0。\n";
  strs += " *   - align 是否不大于 prompt_align。\n";
  strs += " *\n";
  strs +=
      " * 如果以上任何一个条件不满足，函数将通过 OP_LOG "
      "宏记录一条错误消息，并返回\n";
  strs +=
      " * false，表示输入无效。如果所有条件都满足，函数返回 "
      "true，表示输入有效。\n";
  strs += " *\n";
...
inline std::string GenCheckInputFunc() {
  ...
  strs +=
      "    if ((var.max_value == 0) || (var.align == 0) || (var.prompt_align "
      "== 0)) {\n";
...
inline std::string GenInitInputFuncAnnotation() {
  ...
  strs += " *\n";
  strs +=
      " * 这个函数用于初始化 L0TileSolver 类的 input_ 对象中的 l0_vars "
      "数组。它遍历\n";
  strs += " * l0_vars 数组中的每一个元素，对于每个元素，执行以下操作：\n";
  strs += " * 1. 通过访问索引 i 对应的 L0Var 对象的引用 var，重置其 max_value\n";
  strs +=
      " * 属性。具体重置方式是，先将 max_value 增加 align 属性值减 1，再除以 "
      "align\n";
  strs +=
      " * 属性值，最后乘以 align 属性值。这样做的目的可能是为了确保 "
      "max_value 是 align\n";
  strs += " * 的整数倍。\n";
  strs +=
      " * 2. 将当前循环的索引值 i 设置为 var 的 idx "
      "属性。这可能是为了标记每个 L0Var\n";
...
inline std::string GenInitInputFunc() {
  ...
  strs += "    auto &var = input_.l0_vars[i];\n";
  strs +=
      "    var.max_value = (var.max_value + var.align - 1) / var.align * "
      "var.align;\n";
  strs += "    var.idx = i;\n";
  strs += "  }\n";
  strs += "}\n";
...
inline std::string GenCheckOutputFuncAnnotation() {
  ...
  strs += " *\n";
  strs +=
      " * 这个函数用于检查 L0TileSolver 类的输出数据是否有效。它首先检查 "
      "output_\n";
  strs +=
      " * 指针是否为空。如果 output_ 指针为空，通过 OP_LOG "
      "宏记录一条错误消息，并返回\n";
  strs += " * false，表示输出无效。\n";
  strs += " *\n";
  strs +=
