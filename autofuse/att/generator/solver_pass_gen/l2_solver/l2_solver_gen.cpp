...
std::string L2TileSolverGen::GenClassDef() {
  ...
  ss << "    explicit " << tiling_case_id_
     << "L2TileSolver(L2TileInput &input, " + type_name_ + " &tiling_data) : L2TileSolver(input) {\n";
  if (l2_rel_input_arg != "") {
...
Expr L2TileSolverGen::GetRelateL0Arg(const Expr &l2_arg) {
  ...
  for (auto l0_arg : l0_args_) {
    GE_ASSERT_TRUE(arg_max_value_map_.find(l0_arg) != arg_max_value_map_.end(), "Ori arg map does not find l0 arg[%s]",
                   l0_arg.Str().get());
...
std::string L2TileSolverGen::GenSolverInvokeDoc() const {
  ...
  strs +=
      "@return "
      "执行L2TileSolver类的主要流程l2_solver.run()，如果l2_solver.run()的返回值不为true,"
      "则l2求解器执行失败，函数返回false\n";
