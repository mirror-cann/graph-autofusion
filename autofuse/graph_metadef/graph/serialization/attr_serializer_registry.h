...
#define REG_GEIR_SERIALIZER_BUILDER_UNIQ(name, ctr, cls, obj_type, bin_type)                    \
  static ::af::AttrSerializerRegistrar register_serialize_##name##ctr __attribute__((unused)) = \
      ::af::AttrSerializerRegistrar(                                                            \
          []() -> std::unique_ptr<af::GeIrAttrSerializer> {                                     \
            return std::unique_ptr<af::GeIrAttrSerializer>(new (std::nothrow) cls());           \
          },                                                                                    \
          obj_type, bin_type)
...
class AttrSerializerRegistry {
 public:
   ...
   */
  void RegisterGeIrAttrSerializer(const GeIrAttrSerializerBuilder &builder, const TypeId obj_type,
                                  const af::GeIrAttrDef::ValueCase proto_type);

  GeIrAttrSerializer *GetSerializer(const TypeId obj_type);
  GeIrAttrSerializer *GetDeserializer(const af::GeIrAttrDef::ValueCase proto_type);
 ...
 private:
  ...
  std::map<af::GeIrAttrDef::ValueCase, GeIrAttrSerializer *> deserializer_map_;
};

class AttrSerializerRegistrar {
 public:
  AttrSerializerRegistrar(const GeIrAttrSerializerBuilder builder, const TypeId obj_type,
