...
std::vector<NodeToOutAnchor> OpDescUtils::GetConstInputNodeAndAnchor(const Node &node) {
...
size_t OpDescUtils::GetNonConstInputsSize(const Node &node) {
  if (NodeUtils::IsAnchorStatusSet(node)) {
    size_t input_num = 0UL;
    for (const auto &anchor : node.GetAllInDataAnchors()) {
      if (AnchorUtils::GetStatus(anchor) == ANCHOR_DATA) {
        input_num++;
        continue;
      }
    }
...
GE_FUNC_DEV_VISIBILITY GE_FUNC_HOST_VISIBILITY size_t OpDescUtils::GetNonConstInputsSize(const ConstNodePtr node) {
  if (node == nullptr) {
...
GeTensorDesc OpDescUtils::GetNonConstInputTensorDesc(const Node &node, const size_t index_non_const) {
...
OpDescUtils::GetNonConstInputTensorDesc(const ConstNodePtr &node, const size_t index_non_const) {
  CHECK_FALSE_EXEC(node != nullptr, return GeTensorDesc());
  return GetNonConstInputTensorDesc(*node, index_non_const);
}

bool OpDescUtils::GetNonConstInputIndex(const Node &node, const size_t index_non_const, size_t &index) {
  bool ret = false;
  size_t i = 0UL;
  if (NodeUtils::IsAnchorStatusSet(node)) {
    for (const auto &anchor : node.GetAllInDataAnchors()) {
...
GE_FUNC_DEV_VISIBILITY GE_FUNC_HOST_VISIBILITY bool OpDescUtils::GetNonConstInputIndex(const ConstNodePtr &node,
...
bool OpDescUtils::IsNonConstInput(const Node &node, const size_t index) {
  bool ret = false;
...
GE_FUNC_DEV_VISIBILITY GE_FUNC_HOST_VISIBILITY bool OpDescUtils::IsNonConstInput(const ConstNodePtr &node,
                                                                                 const size_t index) {
  CHECK_FALSE_EXEC(node != nullptr, return false);
...
GE_FUNC_DEV_VISIBILITY GE_FUNC_HOST_VISIBILITY std::vector<GeTensorDesc> OpDescUtils::GetNonConstTensorDesc(
    const ConstNodePtr &node) {
  if ((node == nullptr) || (node->GetOpDesc() == nullptr)) {
    return std::vector<GeTensorDesc>();
  }
  std::vector<GeTensorDesc> ret;
  if (NodeUtils::IsAnchorStatusSet(*node)) {
    for (const auto &in_anchor : node->GetAllInDataAnchors()) {
      if (AnchorUtils::GetStatus(in_anchor) == ANCHOR_DATA) {
...
GE_FUNC_DEV_VISIBILITY GE_FUNC_HOST_VISIBILITY graphStatus
OpDescUtils::SetWeights(Node &node, const std::map<int, GeTensorPtr> &weights_map) {
...
GE_FUNC_DEV_VISIBILITY GE_FUNC_HOST_VISIBILITY graphStatus OpDescUtils::ClearWeights(const NodePtr node) {
  GE_CHECK_NOTNULL(node);
...
graphStatus GetOutputInstanceNum(const OpDescPtr &op_desc, size_t ir_index, size_t start_index,
  ...
  GELOGE(FAILED, "Failed to get instance num for node %s, unknown ir output type %d, ir name %s",
...
graphStatus OpDescUtils::GetInstanceNum(const OpDescPtr &op_desc, size_t ir_index, size_t start_index,
  ...
  GELOGE(FAILED, "Failed to get instance num for node %s, unknown ir input type %d, ir name %s",
         op_desc->GetName().c_str(), ir_type, ir_name.c_str());
  return FAILED;
}
...
graphStatus OpDescUtils::GetPromoteIrInputList(const OpDescPtr &op_desc,
  ...
  const Operator operator_ir = OperatorFactory::CreateOperator("temp_operator", op_desc->GetType().c_str());
  const auto opdesc_ir = OpDescUtils::GetOpDescFromOperator(operator_ir);
...
graphStatus OpDescUtils::GetPromoteInstanceInputList(const OpDescPtr &op_desc,
                                                     std::vector<std::vector<size_t>> &promote_index_list) {
  GE_ASSERT_NOTNULL(op_desc);
  GE_ASSERT_SUCCESS(RecoverIrUtils::RecoverOpDescIrDefinition(op_desc, op_desc->GetTypePtr()));
