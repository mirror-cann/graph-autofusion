...
graphStatus UpdateParentNodeForBranch(const ConstNodePtr &node,
  ...
  for (size_t i = 0UL; i < ref_out_tensors.size(); i++) {
    ...
    auto &ref_out_tensor = ref_out_tensors[i].at(0U);
    af::GeShape &ref_out_tensor_shape = ref_out_tensor.MutableShape();
...
graphStatus UpdateOpInputDesc(const ConstNodePtr &node_ptr) {
  ...
  for (const auto &in_anchor : node_ptr->GetAllInDataAnchors()) {
    ...
    if (af::AttrUtils::GetListInt(*peer_out_desc, kPreOpInputShapeRange, pre_op_in_range)) {
      (void)af::AttrUtils::SetListInt(*in_desc, kPreOpInputShapeRange, pre_op_in_range);
    }
    af::TensorUtils::SetRealDimCnt(*in_desc, static_cast<uint32_t>(peer_out_desc->MutableShape().GetDims().size()));
  }
  return GRAPH_SUCCESS;
}
}  // namespace
void ShapeRefiner::PrintInOutTensorShape(const af::NodePtr &node, const std::string &phase) {
  if (!IsLogEnable(GE, DLOG_DEBUG)) {
    return;
  }
  const af::OpDescPtr op_desc = node->GetOpDesc();
...
graphStatus ShapeRefiner::InferShapeAndType(const ConstNodePtr &node, Operator &op, const bool before_subgraph) {
  ...
  if (ret == GRAPH_PARAM_INVALID) {
    ...
    const auto node_op = af::OperatorFactory::CreateOperator("node_op", op_desc->GetType().c_str());
    if (node_op.IsEmpty()) {
