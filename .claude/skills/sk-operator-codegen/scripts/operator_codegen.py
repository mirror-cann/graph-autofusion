...
def _read_pybind_io_contract(path_text: str) -> dict[str, dict[str, Any]]:
    ...
    def _param_map(value: object, field_name: str, entry_name: str) -> dict[str, dict[str, Any]]:
        ...
        for raw_name, raw_spec in value.items():
            ...
            if not param_name:
                raise CliUsageError(f"IO contract entry {entry_name!r} field {field_name!r} contains an empty parameter name")
    ...
    def _launch_contract(value: object, entry_name: str) -> dict[str, Any]:
        ...
        if "block_dim" in value:
            try:
                ...
                raise CliUsageError(f"IO contract entry {entry_name!r} launch.block_dim must be a positive integer") from exc
            if block_dim <= 0:
    ...
    def _runtime_wrapper_contract(value: object, entry_name: str) -> dict[str, Any]:
        ...
        normalized: dict[str, Any] = {"source": source_path.as_posix(), "entry": wrapper_entry}
        ...
        if strategy:
            ...
            normalized["descriptor_order"] = _name_list(value.get("descriptor_order"), "runtime_wrapper.descriptor_order", entry_name)
        return normalized
...
def _resolve_runtime_wrapper_source(asset_path: Path, runtime_wrapper: dict[str, Any], extra_roots: list[Path] | None = None) -> tuple[Path, Path]:
...
def _render_checklist(summary: dict[str, Any]) -> str:
    ...
    checks = [
        ("kernel entry contract", not any(item == "operator_entry_contract" for item in manifest["missing_contracts"])),
        ...
        ("SK binding contract", not any(item == "sk_binding_contract" for item in manifest["missing_contracts"])),
    ]
...
def _write_delivery_artifacts(output_dir: Path, summary: dict[str, Any]) -> None:
    ...
    _write_text(output_dir / "operator-delivery-execution-plan.md", _render_execution_plan(summary))
...
def _write_sk_binding_artifacts(output_dir: Path, manifest: dict[str, Any]) -> None:
    ...
    _write_text(output_dir / "operator-sk-binding-scaffold.md", _render_sk_binding_scaffold_markdown(manifest))
...
def _render_sk_binding_source(entry: dict[str, str], parsed: dict[str, Any], params: list[dict[str, str]]) -> tuple[str, dict[str, Any]]:
...
_SK_NON_GLOBAL_FORM_SIGNAL_RE = re.compile(r"__spk__\b|__sk__\b|\bSK_BIND\s*\(|\.ascend\.meta|FunLevel(?:MixCoreType|KType)")
...
def _collect_sk_form_sources(asset_path: Path) -> tuple[list[tuple[Path, Path]], list[dict[str, str]]]:
    if asset_path.is_file():
...
def _unique_aclgraph_csrc_name(name: str, used_names: set[str]) -> str:
    def _is_pybind_source(candidate_name: str) -> bool:
        return candidate_name == "pybind11.asc" or (candidate_name.startswith("pybind11_") and candidate_name.endswith(".asc"))
...
def cmd_normalize_operator_asset(args: argparse.Namespace) -> int:
    from operator_asset_analyzer import analyze_asset, analyze_asset_from_layout_file, materialize_operator_units
...
def cmd_adapt_sk_from_global(args: argparse.Namespace) -> int:
    ...
    if name_resolution_path:
        from operator_name_resolution import public_name_resolution_payload, read_name_resolution, resolution_by_asset_and_entry
    ...
    public_name_resolution = public_name_resolution_payload(name_resolution_payload) if name_resolution_payload is not None else None
    ...
    def apply_name_resolution(entry: dict[str, Any], source_asset: object | None) -> None:
        ...
        if resolution:
            ...
            entry.setdefault("name_resolution", {"renamed": False, "rename_reason": "", "asset_namespace": "", "collision_group": ""})
    ...
    def contract_bind_targets_for_global_entry(global_entry_names: list[str]) -> dict[str, str]:
    ...
    def apply_io_contract(entry: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if io_contract is None:
            entry["pybind_return_source"] = "single-tensor-default" if len(tensor_names) == 1 else "no-tensor-default" if not tensor_names else "unresolved"
        ...
        if runtime_wrapper:
            ...
            if source_asset_override is not None:
                source_asset_root = source_asset_override if source_asset_override.is_dir() else source_asset_override.parent
                extra_wrapper_roots.append(source_asset_root)
                extra_wrapper_roots.append(source_asset_root.parent)
            wrapper_source, wrapper_root = _resolve_runtime_wrapper_source(asset_path, runtime_wrapper, extra_wrapper_roots)
    ...
    if not sources:
        raise CliUsageError(f"no kernel .asc or .cpp source files found under {asset_path}")
    support_directories = _normalize_support_directories(getattr(args, "support_dir", None), asset_path if asset_path.is_dir() else asset_path.parent)
    ...
    if io_contract_by_entry and unresolved_entries and not used_io_contract_entries:
        escalation = _codegen_human_finding(
            ...
            [f"unresolved_entries={unresolved_entries}", f"contract_entries={sorted(io_contract_by_entry)}"],
        )
        all_escalations.append(escalation)
    ...
    for export_name, entries in sorted(entries_by_export_name.items()):
        ...
        if len(entries) <= 1 or len(bind_targets) <= 1:
            continue
        selected_targets = sorted({_entry_contract_bind_target(entry) for entry in entries if _entry_contract_bind_target(entry)})
    ...
    if not all_escalations and canonical_entries:
        ...
        (op_extension / "__init__.py").write_text(render_op_extension_init_py(module_name, pybind_entries), encoding="utf-8")
        (op_extension / "_arch_selector.py").write_text(render_arch_selector_py(module_name, pybind_entries), encoding="utf-8")
        (op_extension / "_torch_library.py").write_text(render_torch_library_py(module_name, pybind_entries), encoding="utf-8")
        if public_name_resolution is not None:
            (op_extension / "_name_resolution.json").write_text(json.dumps(public_name_resolution, indent=2), encoding="utf-8")
        ...
        canonical_written.extend(
            [
                ...
                *([f"operator-sk-adapted/{package_dir_name}/_name_resolution.json"] if public_name_resolution is not None else []),
    ...
    manifest = {
        ...
        "legacy_migration_body_sources": [
            {"entry_name": entry.get("entry_name"), "body_source": entry.get("body_source")}
            for entry in canonical_entries
            if entry.get("body_source")
        ],
        "legacy_helper_evidence": [
            {"entry_name": entry.get("entry_name"), "mode": entry.get("legacy_helper_evidence")}
            for entry in canonical_entries
...
def cmd_apply_remediation(args: argparse.Namespace) -> int:
    ...
    manifest = {
        ...
        "summary": {"applied": applied, "escalated": escalated, "failed": failed, "skipped": skipped},
        "results": results,
    }
...
def cmd_generate_from_template(args: argparse.Namespace) -> int:
    ...
    for rel, body in files:
        ...
        try:
            ...
            raise CliUsageError(f"template attempted to write outside output dir: {rel}")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    sk_conversion.add_argument("--support-dir", action="append", default=[], help="Support source directory to copy into SK source, optionally name=/path")
    sk_conversion.add_argument("--include-dir", action="append", default=[], help="External include directory to use during generated CMake builds")
    sk_conversion.add_argument("--ascend-cann-path", help="CANN root containing tools/bisheng_compiler/bin/bisheng for Ascend build validation")
    sk_conversion.add_argument("--ascend-arch", help="Ascend NPU arch passed as --npu-arch=<value> when Ascend build validation is enabled")
    sk_conversion.add_argument("--ascend-compile-option", action="append", default=[], help="Extra Bisheng compile option for generated Ascend CMake builds")
    sk_conversion.add_argument("--ascend-force-include", action="append", default=[], help="Header file to force include before generated Ascend source compilation")
    ...
    analyze_asset.add_argument("--target-chip", default="", help="Comma/semicolon separated target chips used to resolve per-op arches")
    ...
    normalize_asset.add_argument("--asset-layout", help="Explicit operator-asset-layout.json to drive normalization")
    normalize_asset.add_argument("--output-dir", required=True, help="Output directory")
    normalize_asset.add_argument("--target-chip", default="", help="Comma/semicolon separated target chips used to resolve per-op arches")
    ...
    adapt_sk.add_argument("--output-dir", required=True, help="Output directory; operator-sk-adapted/ will be created here")
    adapt_sk.add_argument("--understanding", help="operator-asset-understanding.json used to carry per-entry support metadata")
    adapt_sk.add_argument("--target-chip", default="", help="Comma/semicolon separated target chips used to resolve sparse arch builds")
    adapt_sk.add_argument("--package-name", default="op_extension", help="Distribution/Python package name for generated pybind project")
    adapt_sk.add_argument("--package-version", default="0.1.0", help="Distribution package version")
    adapt_sk.add_argument("--name-resolution", help="name-resolution-report.json used to namespace duplicate public entry names")
    adapt_sk.add_argument("--source-asset", help="Original source asset path used to match name-resolution entries")
    ...
    adapt_sk.add_argument("--mask", default="4", help="SK_BIND mask: 4=DCCI default; accepts bit combinations 0..7")
    adapt_sk.add_argument("--num-splits", default="4", help="Number of SK split symbols (1..4)")
    adapt_sk.add_argument("--support-dir", action="append", default=[], help="Support source directory to copy into generated csrc, optionally name=/path")
    adapt_sk.add_argument(
        "--with-sys-args",
    ...
    aggregate_sk.add_argument("--output-dir", required=True, help="Fresh directory for aggregate operator-sk-adapted/ output")
    aggregate_sk.add_argument("--aggregate-wheel-name", default="op_extension", help="Distribution package name for the aggregate wheel")
    aggregate_sk.add_argument("--package-version", default="0.1.0", help="Distribution package version for the aggregate wheel")
    ...
    standalone_compare.add_argument("--output-dir", required=True, help="Fresh directory for operator-sk-standalone-verify output")
    standalone_compare.add_argument("--runtime-fixture-dir", help="Directory containing per-op runtime fixtures")
    standalone_compare.add_argument("--target-chip", default="", help="Target chip recorded in the standalone manifest")
    standalone_compare.add_argument(
        "--npu-arch",
        default="",
    ...
    apply_rem.add_argument("asset_dir", help="Directory whose files the findings reference (target_file paths are relative to this)")
    apply_rem.add_argument("findings_json", help="Path to a findings envelope JSON")
    apply_rem.add_argument("--output-dir", help="Where to write operator-sk-remediation.json (defaults to asset_dir)")
    ...
    gen_tmpl.add_argument("--param", action="append", help="Template parameter override: key=value (repeatable)")
