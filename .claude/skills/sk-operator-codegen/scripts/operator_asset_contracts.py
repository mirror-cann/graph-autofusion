...
SCHEMA_VERSION = 1
SUPPORTED_BUILD_KINDS = {"sk-native-wheel", "sk-standalone", "cmake-project", "python-wheel", "external-command"}
SUPPORTED_VERIFY_KINDS = {"command", "bind-target-on-wheel", "reference-impls-numpy", "standalone"}
...
def validate_asset_layout_contract(layout: dict[str, Any], base_dir: Path | None = None) -> None:
    ...
    for index, unit in enumerate(layout.get("operator_units", [])):
        ...
        _check_existing_file(root, unit.get("kernel_source"), field=f"operator unit {unit_id} kernel_source")
        entry_names = _global_entry_names(root, unit.get("kernel_source"), field=f"operator unit {unit_id} kernel_source")
        if entry_name not in entry_names:
            raise ContractError(
                f"operator unit {unit_id} entry_name {entry_name} is not defined in kernel_source"
            )
        _check_existing_file(root, unit.get("host_source"), field=f"operator unit {unit_id} host_source", required=False)
        _check_existing_file(root, unit.get("json_spec"), field=f"operator unit {unit_id} json_spec", required=False)
...
def _root_for_layout(layout: dict[str, Any]) -> Path:
    asset_root = Path(str(layout["asset_root"])).resolve()
    return asset_root if asset_root.is_dir() else asset_root.parent


def _inventory_unresolved_includes(inventory: dict[str, Any] | None) -> list[dict[str, str]]:
...
def build_default_build_context(
    ...
    *,
    ...
    if layout.get("status") != "ready":
        questions.append({"id": "layout-required", "message": "operator asset layout must be ready before build context can be ready"})
    if unresolved_includes:
        questions.append({"id": "unresolved-include", "message": "some quoted includes could not be resolved inside the asset root"})
