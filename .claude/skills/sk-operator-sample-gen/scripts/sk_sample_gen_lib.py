                ...
                parameter_values.append(
                    {
                        ...
                        "dtype": dtype,
                        "layout": "ND",
                        "value_source": {"kind": "inline_json", "value": _zero_nested(list(shape))},
                    }
                )
            else:
                parameter_values.append(
...
def build_oracle_spec(
    ...
    for input_set in input_values_artifact["input_values"]:
        ...
        if ref is None:
            ...
            continue
        inputs = {
            pv["name"]: {"shape": pv["shape"], "dtype": pv["dtype"], "value": pv["value_source"]["value"]}
        ...
        oracle_specs.append(
            {
                ...
                "expected_output": {
                    "kind": "inline_json",
                    "value": {"source": "bind-target-on-wheel", "entry_name": entry_name},
                },
                "comparator": "allclose",
...
def build_standalone_insufficient_fixture_verdict(entry_name: str, reason: str = "runtime fixture not available") -> dict[str, Any]:
    return {
        "backend": "standalone",
        "entry_name": entry_name,
...
_PARAMETER_ROLES = {"input", "output", "workspace", "tiling", "scalar", "descriptor", "unknown"}
...
def build_single_op_verification_contract(runtime_contract: dict[str, Any]) -> dict[str, Any]:
    ...
    declared_outputs = [
        ...
        for item in _as_list(_as_object(payload.get("comparison", {}), "runtime contract comparison").get("outputs", []), "runtime contract comparison outputs")
    ]
    if not declared_outputs:
        declared_outputs = [parameter["name"] for parameter in parameters if parameter["compare"]]
...
def normalize_network_sample_contract(payload: dict[str, Any]) -> dict[str, Any]:
        ...
        inputs.append(
            {
                ...
                "shape": [int(dim) for dim in _as_list(item.get("shape"), "network sample contract input.shape")],
            }
        )
    ...
    require_all_fused = _as_bool(expected_fusion.get("require_all_fused", True), "network sample contract expected_fusion.require_all_fused")
    ...
    return {
        ...
        "network_kind": _as_string(raw.get("network_kind", network_name), "network sample contract network_kind"),
        "package": {
            "name": package_name,
            "wheel": _as_string(package.get("wheel", ""), "network sample contract package.wheel", allow_empty=True),
            "arch": _as_string(package.get("arch", ""), "network sample contract package.arch", allow_empty=True),
        },
        ...
        "expected_fusion": {
            "scope": _as_string(expected_fusion.get("scope", ""), "network sample contract expected_fusion.scope", allow_empty=True),
            "ops": expected_ops,
...
def build_network_fusion_expectation(network_contract: dict[str, Any]) -> dict[str, Any]:
