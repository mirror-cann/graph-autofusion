...
def run_compat_rules(
    ...
    *,
    ...
    if not chip:
        ...
        verification_note = (
            "target chip is recorded, but no official-source-backed capability "
            "checks were declared"
        )
    elif not target_cann and not has_applicable_source_backed_checks:
        verification_note = (
            "target CANN was not provided, so CANN-scoped official-source-backed "
            "compatibility checks were not applied"
        )
