        ...
        findings.append(
            {
                ...
                "remediation_hint": {"kind": "rename-symbol", "old_value": legacy, "new_value": "skNumBlocks"},
                "evidence": [m.group(0)],
...
class PipelineOrchestrator:
    ...
    def _shared_support_dirs_for_asset(self, asset: Path) -> list[tuple[str, Path]]:
        ...
        for source in sorted(item for item in asset_root.rglob("*") if item.is_file() and item.suffix in source_suffixes):
    ...
    def _shared_support_dirs_for_stage_asset(self, stage_asset: Path, source_asset: Path | None = None) -> list[tuple[str, Path]]:
        support_dirs: dict[str, Path] = {}
    ...
    def _understand_and_expand_assets(
        ...
        for raw in raw_assets:
            ...
            scan_argv = ["adapt-asset", str(inputs / "asset"), "--output-dir", str(outputs)]
    ...
    def _parse_stages(
        ...
        *,
        ...
        if stages is None:
            if not do_package:
                selected = ["01", "02", "03"]
            elif profile == "fast" or not PipelineOrchestrator._needs_wheel_stage(wheel_mode, verify_backend, reuse_wheel):
    ...
    def _aggregate(self, adapted_outputs: list[Path], output_dir: Path, package_name: str, package_version: str) -> dict[str, Any]:
        argv: list[str] = ["aggregate-sk-adapted"]
        for adapted_output in adapted_outputs:
            argv.extend(["--adapted-output-dir", str(adapted_output)])
        argv.extend(["--output-dir", str(output_dir), "--aggregate-wheel-name", package_name, "--package-version", package_version])
        result = self._run("codegen", *argv)
        if result.returncode != 0:
    ...
    def _build_native_wheel(
        ...
        *,
        ...
        for stale in ("operator-sk-native-wheel-build", "operator-sk-native-wheel.json"):
    ...
    def _build_differential_verdicts(
        ...
        *,
        ...
        per_op: dict[str, Any] = {}
        summary_rows = ["# Operator Differential Verification", "", "| op | status | baseline | sk | wheel |", "|---|---|---|---|---|"]
    ...
    def _build_runtime_contracts(
        ...
        *,
        ...
        self._write_json(output_dir / "operator-runtime-contract-summary.json", {"schema_version": 1, "contracts": contracts})
        return contracts
    ...
    def _generate_standalone_compare(self, aggregate_output_dir: Path, output_dir: Path, fixture_dir: Path, target_chip: str) -> dict[str, Any]:
        result = self._run(
            "codegen",
            "generate-standalone-compare",
            str(aggregate_output_dir),
            "--output-dir", str(output_dir),
            "--runtime-fixture-dir", str(fixture_dir),
    ...
    def _build_standalone_executable(
        ...
        *,
        ...
        self._ensure_structural_toolchain_env(output_dir, allow_structural_toolchain=allow_structural_toolchain)
        for stale in ("operator-sk-standalone-build.json", "build-log.txt", "executable-path.txt"):
            self._remove_existing(output_dir / stale)
        argv = ["build-standalone-executable", str(output_dir), "--target-chip", target_chip]
        if target_cann:
            argv.extend(["--target-cann", target_cann])
        result = self._run("build_package", *argv)
        manifest_path = output_dir / "operator-sk-standalone-build.json"
        manifest = self._read_json(manifest_path) if manifest_path.exists() else {}
        log = (output_dir / "build-log.txt").read_text(encoding="utf-8", errors="replace") if (output_dir / "build-log.txt").exists() else ""
        ...
        return lib.cache_key(
            {
                ...
                "ascend_home_path": str(Path(env.get("ASCEND_HOME_PATH", "")).resolve()) if env.get("ASCEND_HOME_PATH") else "",
    ...
    def _relocate_standalone_build_manifest(self, output_dir: Path) -> dict[str, Any]:
        ...
        executable_targets = manifest.get("executable_targets") if isinstance(manifest.get("executable_targets"), dict) else {}
        if executable_targets:
            executable_targets = {target: str(build_dir / Path(path).name) for target, path in executable_targets.items()}
        else:
        ...
        manifest.update(
            {
                ...
                "configure_command": ["cmake", "-S", str(source_root), "-B", str(build_dir)],
        ...
        manifest.update(
            {
                ...
                "command": [
                    ...
                    "--wheel-dir", str(output_dir / "operator-sk-native-wheel-build" / "wheels"),
    ...
    def _runtime_input_spec_for_entry(self, entry: dict[str, Any]) -> dict[str, Any]:
        ...
        for index, param in enumerate(entry.get("parameters", [])):
            ...
            name = param["name"]
            parameters.append({"index": index, "name": name, "type": c_type, "source": f"{c_type} {name}"})
        return {
            "input_specs": [
                {
    ...
    def _copy_runtime_fixtures(self, ops: list[dict[str, Any]], fixture_root: Path) -> dict[str, dict[str, Any]]:
        ...
        for op in ops:
            ...
            runtime_spec = asset / "operator-sk-runtime-fixture.json" if asset.is_dir() else asset.parent / "operator-sk-runtime-fixture.json"
            ...
            for candidate in (asset / "interface" if asset.is_dir() else asset.parent / "interface", asset.parent / "interface"):
                if candidate.is_dir():
    ...
    def _standalone_runner_stdout(self, op_name: str, *, status: str) -> dict[str, Any]:
        return {"backend": "standalone", "status": status, "outputs": {}, "calls": {op_name: []}}
        ...
        *,
        ...
        def worker(entry: dict[str, Any]) -> dict[str, Any]:
            ...
            if precheck_status == "skipped-target-arch":
                ...
                verdict = {"status": "skipped-target-arch", "reason": "standalone target arch is not resolved"}
                ...
                verdict = {"status": "mock-passed", "reason": "mock NPU verification explicitly enabled"}
                ...
                verdict = {"status": "skipped-by-user", "reason": "verification disabled by --no-verify"}
                ...
                comparison = {"status": "skipped-insufficient-runtime-spec", "comparisons": []}
                verdict = {"status": "skipped-insufficient-runtime-spec", "reason": reason}
            else:
                ...
                if completed.returncode != 0:
                    runner_stdout = {"backend": "standalone", "status": "failed", "stdout": completed.stdout, "outputs": {}, "calls": {op_name: []}}
                ...
                try:
                    ...
                    raise OrchestratorError(f"standalone runner emitted invalid JSON for {op_name}: {completed.stdout}") from exc
                ...
                if entry_status_value != "passed":
                    ...
                    comparison = {"status": "failed", "comparisons": []}
                    verdict = {"status": "failed", "reason": "standalone_runner_missing_outputs"}
                else:
                    compare = self._run("sample_gen", "compare-sk-runtime-outputs", str(op_dir), str(op_dir / "runner-stdout.json"))
                    if compare.returncode != 0:
                        raise OrchestratorError(f"standalone compare-sk-runtime-outputs failed for {op_name}: {compare.stdout}")
                    ...
                    verdict = {
                        ...
                        "reason": "differential_outputs_matched" if comparison.get("status") == "matched" else "differential_outputs_mismatched",
                    }
            self._write_json(op_dir / "runner-stdout.json", runner_stdout)
            self._write_json(op_dir / "comparison.json", comparison)
            self._write_json(op_dir / "verdict.json", verdict)
        ...
        if "failed" in status_values:
            ...
            status = "mixed"
        rows = ["# Standalone Differential Verification", "", "| op | status |", "|---|---|"]
        for op_name in sorted(per_op):
            rows.append(f"| `{op_name}` | `{per_op[op_name]['status']}` |")
        (verify_dir / "summary.md").write_text("\n".join(rows) + "\n", encoding="utf-8")
        return {"status": status, "per_op": per_op, "summary": str(verify_dir / "summary.md"), "parallel": parallel}
    ...
    def _run_sample_gen_verification(
        ...
        *,
        ...
        verify_dir.mkdir(parents=True, exist_ok=True)
        summary_rows = ["# Differential Verification", "", "| op | status |", "|---|---|"]
        per_op: dict[str, Any] = {}
        for entry in entries:
            op_name = entry["entry_name"]
            op_dir = verify_dir / op_name
            op_dir.mkdir(parents=True, exist_ok=True)
            self._write_json(op_dir / "operator-sk-runtime-input-spec.json", self._runtime_input_spec_for_entry(entry))
            ...
            shutil.copy2(op_dir / "operator-sk-auto-input-values-spec.json", op_dir / "auto-inputs.json")
            ...
            runner_spec = {
                ...
                "entry_name": op_name,
                "functions": {"baseline": f"run_{op_name}", "sk": f"torch.ops.ascendc_ops.{op_name}"},
                "sample_gen_command_spec": self._read_json(op_dir / "operator-sk-target-runtime-command-spec.json")
            ...
            if status == "skipped-by-user":
                ...
                runner_stdout = {"status": "skipped-insufficient-runtime-spec", "outputs": {}}
                comparison = {"status": "skipped-insufficient-runtime-spec", "comparisons": []}
                verdict = {
                    "status": "skipped-insufficient-runtime-spec",
                ...
                env = (self.env or os.environ).copy()
                completed = subprocess.run(
                    [self.python, str(op_dir / "operator-sample-runner.py"), str(op_dir / "operator-sk-runtime-input-values.json")],
                    cwd=str(op_dir),
                    stdout=subprocess.PIPE,
                    stderr=subprocess.STDOUT,
                    text=True,
                ...
                if completed.returncode != 0:
                    runner_stdout = {"status": "failed", "stdout": completed.stdout, "outputs": {}}
                    comparison = {"status": "failed", "comparisons": []}
        ...
        if status == "skipped-no-npu":
            ...
            summary_rows.extend(["", "Mock NPU validation was explicitly requested; numerical correctness was not executed."])
        (verify_dir / "summary.md").write_text("\n".join(summary_rows) + "\n", encoding="utf-8")
        ...
        if status_values == {"skipped-insufficient-runtime-spec"}:
            ...
            final_status = status
        return {"status": final_status, "per_op": per_op, "summary": str(verify_dir / "summary.md")}
        ...
        *,
        ...
        if "00" in selected_stages:
            state["stages"].append(
                self._stage_record(
                    ...
                    {"asset_count": len(asset_understanding["assets"]), "operator_unit_count": len(ops)},
                )
            )
        ...
        if "01" in selected_stages:
            ...
            for item in stage_results:
                ...
                state["ops"][item["op_name"]]["stage01"] = {"outputs": item["outputs"], "input_mode": item["input_mode"]}
            state["stages"].append(self._stage_record("01", "passed", stage_root, {"parallel": parallel}, started_at=stage_started_at))
        ...
        if "02" in selected_stages:
            ...
            def worker(op: dict[str, Any]) -> dict[str, Any]:
                ...
                inputs = op_root / "inputs"
                outputs = op_root / "outputs"
                self._materialize_input(self._stage_dir(output_dir, "01") / op["name"] / "outputs", inputs / "detect-form")
                ...
                for support_name, support_source in self._shared_support_dirs_for_stage_asset(stage_asset, source_asset):
            ...
            for item in stage_results:
                ...
                if manifest_entries:
                    ...
                    state["ops"][op_name]["entry_name"] = entry_meta.get("entry_name") or state["ops"][op_name].get("entry_name")
                    state["ops"][op_name]["source_entry_name"] = entry_meta.get("source_entry_name") or state["ops"][op_name].get("source_entry_name")
                    state["ops"][op_name]["public_entry_name"] = entry_meta.get("public_entry_name") or state["ops"][op_name].get("entry_name")
                    state["ops"][op_name]["internal_symbol_name"] = entry_meta.get("internal_symbol_name") or state["ops"][op_name].get("entry_name")
                    state["ops"][op_name]["bind_target"] = entry_meta.get("bind_target") or state["ops"][op_name].get("source_entry_name")
            ...
            aggregate_manifest = self._aggregate(adapted_outputs, aggregate_outputs, aggregate_wheel_name, package_version)
            state["aggregate"] = {
        ...
        if "03" in selected_stages:
            ...
            def worker(op: dict[str, Any]) -> dict[str, Any]:
                ...
                inputs = op_root / "inputs"
                outputs = op_root / "outputs"
                self._materialize_input(self._stage_dir(output_dir, "02") / op["name"] / "outputs", inputs / "adapted-output")
                result = self._scan_spec(inputs / "adapted-output" / "operator-sk-adapted", outputs)
                op_findings = result.get("findings", [])
                return {"op_name": op["name"], "status": "passed", "outputs": str(outputs), "findings": op_findings}
            ...
            for item in stage_results:
                ...
                state["ops"][item["op_name"]]["stage03"] = {"outputs": item["outputs"], "findings": len(op_findings)}
            state["stages"].append(
                self._stage_record("03", "passed", stage_root, {"findings": len(findings), "parallel": parallel}, started_at=stage_started_at)
            )
        ...
        if "04" in selected_stages:
            ...
            def worker(op: dict[str, Any]) -> dict[str, Any]:
                ...
                outputs = op_root / "outputs"
                self._materialize_input(self._stage_dir(output_dir, "02") / op["name"] / "outputs", inputs / "adapted-output")
                result = self._scan_compat(inputs / "adapted-output" / "operator-sk-adapted", outputs, target_chip, target_cann)
                op_findings = result.get("findings", [])
                return {"op_name": op["name"], "status": "passed", "outputs": str(outputs), "findings": op_findings}
            ...
            for item in stage_results:
                op_findings = item["findings"]
                findings.extend(op_findings)
                state["ops"][item["op_name"]]["stage04"] = {"outputs": item["outputs"], "findings": len(op_findings)}
            state["stages"].append(
                self._stage_record(
                    "04",
                    "passed",
        ...
        if "05" in selected_stages:
            ...
            state["stages"].append(
                self._stage_record("05", "passed", stage_root, {"outputs": str(outputs)}, started_at=stage_started_at)
            )
            state["iterations"][0]["stages"].append("generate-pybind-binding")
            state["binding"] = {"package_name": binding.get("package_name"), "kernel_entries": binding.get("kernel_entries", [])}
            self._write_state(output_dir, state)
        ...
        if "06" in selected_stages:
            ...
            for op in ops:
                for key in (op.get("entry_name"), op.get("public_entry_name"), op.get("source_entry_name"), op.get("bind_target"), op["name"]):
                    if key:
            ...
            def baseline_worker(entry: dict[str, Any]) -> dict[str, Any]:
                ...
                return {
                    "op_name": entry_name,
                    "status": "passed" if manifest.get("status") in {"passed", "structural-passed", "skipped-real-backend-not-enabled"} else "failed",
                    "manifest": manifest,
            ...
            if len(ops) == 1:
                ...
                for entry in entries:
                    if entry["entry_name"] not in fixture_statuses and source_fixture_dir.exists():
                        ...
                        fixture_statuses[entry["entry_name"]] = {**source_status, "path": str(entry_fixture_dir)}
            ...
            if standalone_manifest.get("status") == "needs-target-arch":
                ...
                self._ensure_structural_toolchain_env(standalone_outputs, allow_structural_toolchain=allow_structural_toolchain)
                ...
                cached_standalone = self._copy_cached_namespace(cache_dir, "standalone", standalone_cache_key, standalone_outputs)
                    ...
                    if standalone_build_status not in {"passed", "structural-passed"}:
                        ...
                        state["stages"].append(
                            self._stage_record(
                                ...
                                "failed",
                                stage_root,
                                {"standalone": {"status": "failed", "cache_key": standalone_cache_key, "cache_hit": False}},
                                started_at=stage_started_at,
                            )
                        )
                        self._write_state(output_dir, state)
                        ...
                        return state
                    cached_standalone = self._store_cached_namespace(cache_dir, "standalone", standalone_cache_key, standalone_outputs)
                    standalone_cache_hit = False
            ...
            standalone_verification = self._run_standalone_verification(
                ...
                jobs=resolved_jobs,
                executable_path=Path(standalone_build.get("executable", "")) if standalone_build.get("executable") else None,
                executable_paths={
                    entry: Path(path)
                    for entry, path in standalone_build.get("executables", {}).items()
                } if isinstance(standalone_build.get("executables"), dict) else None,
            ...
            if self._needs_wheel_stage(resolved_wheel_mode, resolved_verify_backend, reuse_wheel):
                ...
                if reuse_wheel:
                    ...
                    if not reuse_path.name.startswith(expected_prefix) or reuse_path.suffix != ".whl":
                        raise OrchestratorError(f"reuse wheel does not match {aggregate_wheel_name} {package_version}: {reuse_path.name}")
                    dest = wheels_dest / reuse_path.name
                    shutil.copy2(reuse_path, dest)
                    wheel_paths = [str(dest)]
                    wheel_summary.update({"status": "reused", "reused_from": str(reuse_path), "wheel_paths": wheel_paths, "wheels": [dest.name]})
                else:
                    self._ensure_structural_toolchain_env(wheel_outputs, allow_structural_toolchain=allow_structural_toolchain)
                    ...
                    if cached_wheel:
                        ...
                        if build_status not in {"passed", "structural-passed"}:
                            ...
                            state["stages"].append(
                                self._stage_record("06", "failed", stage_root, {"wheel": wheel_summary}, started_at=stage_started_at)
                            )
                            self._write_state(output_dir, state)
                            if emit_layout:
                                self._emit_layout(
            ...
            legacy_build_status = (
                ...
                else "passed" if wheel_summary.get("status") in {"built", "reused"} else wheel_summary.get("status", "skipped")
            )
            state["iterations"][0]["build"] = {**wheel_summary, "status": legacy_build_status}

            legacy_verify = stage_root / "verify"
            self._remove_existing(legacy_verify)
            source_verify = (stage_root / "wheel" / "outputs" / "verify") if wheel_verification else (standalone_outputs / "verify")
            ...
            state["stages"].append(
                self._stage_record(
                    ...
                    stage_root,
                    {
                        ...
                        "wheel": wheel_summary,
                        "differential": {"status": differential["status"], "summary": differential["summary"]},
        ...
        if "06" in selected_stages:
            ...
            if wheel_status in {"structural-built", "structural-reused"} or standalone_build_status == "structural-passed" or allow_structural_toolchain:
                ...
                elif standalone_status in {"skipped-no-npu", "skipped-insufficient-runtime-spec", "skipped-by-user", "skipped-target-arch"}:
                    state["status"] = standalone_status
                else:
                    state["status"] = "packaged"
            else:
                if standalone_status in {"skipped-no-npu", "skipped-insufficient-runtime-spec", "skipped-by-user", "skipped-target-arch"}:
                    state["status"] = standalone_status
