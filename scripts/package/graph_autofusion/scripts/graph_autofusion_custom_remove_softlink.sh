...
recreate_common_stub_softlink() {
    ...
    local pwdbak="$(pwd)"
    cd $stub_dir && [ -L "$arch_name/libgraph_af.so" ] && chmod u+w . && ln -s "$arch_name/libgraph_af.so" libgraph_af.so
    cd $pwdbak
}
...
remove_softlink "superkernel"
remove_softlink "superkernel-*.dist-info"
