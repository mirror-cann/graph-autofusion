...
install_run() {
    ...
    if [ "$is_docker_install" = "y" ]; then
        graph_autofusion_install_path_param=$(concat_docker_install_path "${docker_root}" "${graph_autofusion_input_install_path}")
    else
        graph_autofusion_install_path_param="${graph_autofusion_input_install_path}"
    fi
    if [ "$1" = "install" ]; then
        unchattr_files
        chmod_start
        ...
        if [ $? -eq 0 ]; then
            ...
            chmod_end
            log "ERROR" "graph_autofusion package install failed, please retry after uninstall!"
...
upgrade_run() {
    ...
    if [ -f "$install_info" ]; then
        local graph_autofusion_input_install_path=$(get_install_param "GRAPH_AUTOFUSION_Install_Path_Param" "${install_info}")
        local graph_autofusion_install_type=$(get_install_param "GRAPH_AUTOFUSION_Install_Type" "${install_info}")
    elif [ -f "$install_info_old" ] && [ $(grep -c -i graph_autofusion_install_path_param "$install_info_old") -ne 0 ]; then
        local graph_autofusion_input_install_path="$(grep -iw graph_autofusion_install_path_param "$install_info_old" | cut -d"=" -f2-)"
        local graph_autofusion_install_type="$(grep -iw graph_autofusion_install_type "$install_info_old" | cut -d"=" -f2-)"
    else
    ...
    if [ "$1" = "upgrade" ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            chmod_end
            log "ERROR" "graph_autofusion package upgrade failed, please retry after uninstall!"
            log_operation "${operation}" "failed!"
            exit_install_log 1
        fi
    fi
...
uninstall_run() {
    ...
    if [ -f "$upgrade_install_info" ]; then
        local graph_autofusion_input_install_path=$(get_install_param "GRAPH_AUTOFUSION_Install_Path_Param" "${upgrade_install_info}")
        local graph_autofusion_install_type=$(get_install_param "GRAPH_AUTOFUSION_Install_Type" "${upgrade_install_info}")
    elif [ -f "$install_info_old" ] && [ $(grep -c -i graph_autofusion_install_path_param "$install_info_old") -ne 0 ]; then
        local graph_autofusion_input_install_path="$(grep -iw graph_autofusion_install_path_param "$install_info_old" | cut -d"=" -f2-)"
        local graph_autofusion_install_type="$(grep -iw graph_autofusion_install_type "$install_info_old" | cut -d"=" -f2-)"
    else
    ...
    if [ ! -f "$upgrade_default_dir/script/run_graph_autofusion_uninstall.sh" ]; then
        log "WARNING" "run_graph_autofusion_uninstall.sh not found."
        return $?
    fi
    if [ "$1" = "uninstall" ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            if [ $(id -u) -eq 0 ]; then
                if [ "$uninstall" = "y" ] && [ -f "$install_info_old" ] && [ $(grep -c -i graph_autofusion_install_path_param "$install_info_old") -ne 0 ]; then
                    sed -i '/graph_autofusion_install_path_param=/Id' "$install_info_old" 2> /dev/null
                    sed -i '/graph_autofusion_install_type=/Id' "$install_info_old" 2> /dev/null
                fi
            fi
            new_echo "INFO" "graph_autofusion package uninstalled successfully! Uninstallation takes effect immediately."
            log "INFO" "graph_autofusion package uninstalled successfully! Uninstallation takes effect immediately."
            log_operation "${operation}" "succeeded"
        else
            log "ERROR" "graph_autofusion package uninstall failed!"
            log_operation "${operation}" "failed!"
            exit_uninstall_log 1
        fi
    fi
...
judgmentpath() {
    ...
    if [ $? -ne 0 ]; then
        log "ERROR" "The graph_autofusion install_path ${1} is invalid, only characters in [a-z,A-Z,0-9,-,_] are supported!"
        exit 1
    fi
}
...
migrate_user_assets_v2() {
    if [ "$pkg_is_multi_version" = "true" ]; then
        ...
        if [ -n "$last_installed_dir" ]; then
            ...
            local data_dir="graph_autofusion/data/fusion_strategy"
...
if [ "$docker_install" = "y" ]; then
    log "ERROR" "ERR_NO:0x0004;ERR_DES:Unsupported parameters, operation failed."
    log "INFO" "--docker not uesd in graph_autofusion"
    exit 1
fi
...
if [ "x$version_installed" != "x" -a "$version_installed" != "none" ] || [ -f "${install_info}" ]; then
    ...
    # 安装场景
        ...
        if [ "$is_quiet" = "n" ]; then
            log "INFO" "graph_autofusion package has been installed on the path ${pkg_install_path}, the version is ${version_installed}, and the version of this package is ${version_of_package}, do you want to continue? [y/n]"
            while true
            do
                read yn
                if test "$yn" = n; then
