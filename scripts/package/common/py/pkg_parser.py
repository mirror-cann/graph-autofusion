...
def evaluate_info(info: Dict[str, str],
    ...
    return {key: eval_value_func(key, value)
        for key, value in itertools.chain(default_cfg, info.items())}
...
def load_block_element(package_attr: PackageAttr,
    ...
    def with_filepath(block_xml: str):
        ...
        try:
            ...
            raise BlockConfigError(f"dependent block configuration {block_xml} parse failed!") from e
...
def parse_blocks(root_ele: ET.Element,
    ...
    """解析块列表。"""
    loaded_block_list = itertools.chain(
    ...
    return [parse_block_config(loaded_block, package_attr, parse_env)
            for loaded_block in loaded_block_list]
