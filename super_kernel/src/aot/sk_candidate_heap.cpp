...
void SkCandidateHeap::push(uint64_t nodeId) {
  ...
  if (nodeType == SkNodeType::NODE_DEFAULT) {
    ...
    SK_LOGD("SkCandidateHeap::push: added default node %s, defaultCount=%zu", node->Format().c_str(),
            defaultNodes_.size());
  } else if (nodeType == SkNodeType::NODE_KERNEL) {
    if (node->IsScopeNode()) {
      nonKernelNodes_.insert(node);
      SK_LOGD("SkCandidateHeap::push: added non-kernel node %s (type=%d), nonKernelCount=%zu", node->Format().c_str(),
              static_cast<int>(nodeType), nonKernelNodes_.size());
    } else {
      kernelNodes_.insert(node);
      SK_LOGD("SkCandidateHeap::push: added kernel node %s, kernelCount=%zu", node->Format().c_str(),
              kernelNodes_.size());
    }
  } else if (nodeType == SkNodeType::NODE_NOTIFY || nodeType == SkNodeType::NODE_WAIT ||
             nodeType == SkNodeType::NODE_RESET) {
    nonKernelNodes_.insert(node);
    SK_LOGD("SkCandidateHeap::push: added non-kernel node %s (type=%d), nonKernelCount=%zu", node->Format().c_str(),
            static_cast<int>(nodeType), nonKernelNodes_.size());
  } else {
    SK_LOGW("SkCandidateHeap::push: unsupported node type %d for node %s, ignored", static_cast<int>(nodeType),
            node->Format().c_str());
  }
}
...
SuperKernelBaseNode *SkCandidateHeap::SelectNextKernelNode() {
  ...
  if (prevKernelTypeClass_ == KernelTypeClass::MIX) {
    ...
    for (auto *node : kernelNodes_) {
      if (IsMixKernelType(node->GetKernelType()) && node->GetStreamIdxInGraph() != prevKernelStreamIdx_) {
        selectedNode = node;
        foundByRule = true;
        break;
      }
    ...
    for (auto *node : kernelNodes_) {
      if (IsCubeKernelType(node->GetKernelType()) && node->GetStreamIdxInGraph() != prevKernelStreamIdx_) {
        selectedNode = node;
        foundByRule = true;
        break;
      }
    ...
    for (auto *node : kernelNodes_) {
      if (IsVecKernelType(node->GetKernelType()) && node->GetStreamIdxInGraph() != prevKernelStreamIdx_) {
        selectedNode = node;
        foundByRule = true;
        break;
      }
