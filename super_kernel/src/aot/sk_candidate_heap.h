...
enum class SkHeapType : uint8_t { PRIORITY_QUEUE = 0, CUSTOMIZE_QUEUE = 1, DEFAULT_QUEUE = 0xff };
...
class SkCandidateHeap {
 public:
   ...
   */
  KernelTypeClass GetPrevKernelTypeClass() const {
    return prevKernelTypeClass_;
  }
   ...
   */
  uint32_t GetPrevStreamIdx() const {
    return prevStreamIdx_;
  }
   ...
   */
  void SetPrevKernelTypeClass(KernelTypeClass typeClass) {
    prevKernelTypeClass_ = typeClass;
  }
   ...
   */
  void SetPrevStreamIdx(uint32_t streamIdx) {
    prevStreamIdx_ = streamIdx;
  }
