...
SuperKernelBaseNode *AdvanceNodeWithinScope(SuperKernelGraph &graph,
...
uint64_t FindKernelNodeWithFrontReserve(SuperKernelGraph &graph,
                                        SuperKernelBaseNode *headNode,
                                        uint64_t tailNodeId,
...
bool EnsureStreamCapacity(const SuperKernelProcessedScopeInfo &processedScopeInfo,
                         uint32_t checkStreamIdx)
{
    ...
    if (streamInfo.customParams.size() > streamInfo.nodeSize) {
        SK_LOGE("scope post-process overflow: streamId=%u, nodeSize=%lu, customParamSize=%zu",
...
bool ProcessFrontWaitForStream(SuperKernelGraph &graph,
                               const SuperKernelScopeInfo &scopeInfo,
                               std::vector<StreamPostPlan> &plans,
                               SuperKernelProcessedScopeInfo &processedScopeInfo,
                   uint32_t curStreamIdx,
                               uint64_t lastNodeId,
                   ...
                   uint32_t &prevWaitStreamIdx)
{
    ...
    auto resetNode = SuperKernelNodeFactory::CreateNode(
        std::make_unique<aclrtTask>(nullptr), ACL_RT_TASK_EVENT_RESET,
    ...
    processedScopeInfo.updateStreamInfos[prevWaitStreamIdx]
        .customParams.emplace(
            processedScopeInfo.updateStreamInfos[prevWaitStreamIdx]
                .customParams.begin(),
            waitParams);
...
bool ProcessBackBlockForStream(const SuperKernelScopeInfo &scopeInfo,
                               std::vector<StreamPostPlan> &plans,
                               SuperKernelProcessedScopeInfo &processedScopeInfo,
                   uint32_t curStreamIdx,
                               uint64_t lastNodeId)
{
    auto &scopeStreamInfo = scopeInfo.scopeStreamInfos[curStreamIdx];
    SK_LOGI("back-block process begin: streamIdx=%u, streamId=%u",
        curStreamIdx, scopeStreamInfo.streamIdx);
    ...
    auto notifyNode = SuperKernelNodeFactory::CreateNode(
        std::make_unique<aclrtTask>(nullptr), ACL_RT_TASK_EVENT_RECORD,
        INVALID_TASK_ID, scopeStreamInfo.streamIdx, lastNodeId);
...
SuperKernelProcessedScopeInfo SuperKernelScopePostProcessor::PostProcess(SuperKernelScopeInfo &scopeInfo) {
    ...
    for (uint32_t curStreamIdx = 0; curStreamIdx < streamCount; ++curStreamIdx) {
        ...
        SK_LOGI("stream plan collected: streamIdx=%u, streamId=%u, head=%lu, tail=%lu, nodeSize=%lu, needFrontWait=%d, needBackBlock=%d",
                curStreamIdx,
                scopeStreamInfo.streamIdx,
                scopeStreamInfo.headNodeIdx,
                scopeStreamInfo.tailNodeIdx,
    ...
    for (uint32_t curStreamIdx = 0; curStreamIdx < streamCount; ++curStreamIdx) {
        ...
        if (processedScopeInfo.skMainNodeId == INVALID_TASK_ID) {
            ...
            uint64_t candidateNodeId = FindKernelNodeWithFrontReserve(
                graph, plans[curStreamIdx].workNode,
                processedScopeInfo.updateStreamInfos[curStreamIdx].tailNodeIdx,
            ...
            if (candidateNodeId != INVALID_TASK_ID) {
                ...
                SK_LOGI("main stream selected: streamIdx=%u, streamId=%u, skMainNodeId=%lu",
                        mainStreamIdx,
                        processedScopeInfo.updateStreamInfos[mainStreamIdx].streamIdx,
                        processedScopeInfo.skMainNodeId);
            }
        }
    ...
    for (uint32_t curStreamIdx : streamOrder) {
        SK_LOGI("scope pass3 stream begin: streamIdx=%u, streamId=%u, needFrontWait=%d, needBackBlock=%d",
                curStreamIdx,
                processedScopeInfo.updateStreamInfos[curStreamIdx].streamIdx,
                plans[curStreamIdx].needFrontWait,
                plans[curStreamIdx].needBackBlock);
        ...
        if (plans[curStreamIdx].needFrontWait &&
            !ProcessFrontWaitForStream(graph, scopeInfo, plans, processedScopeInfo,
                                       curStreamIdx, lastNodeId, needFrontWaitCount,
                                       prevWaitStreamIdx)) {
            return {};
        }

        if (plans[curStreamIdx].needBackBlock &&
            !ProcessBackBlockForStream(scopeInfo, plans, processedScopeInfo,
                                       curStreamIdx, lastNodeId)) {
            return {};
        }
    }
    ...
    SK_LOGI("scope post-process done: streamCount=%u, streamOrderSize=%zu, skMainNodeId=%lu, eventNodeCount=%zu, totalCustomParamSize=%zu",
            streamCount,
            streamOrder.size(),
            processedScopeInfo.skMainNodeId,
            processedScopeInfo.eventNodes.size(),
