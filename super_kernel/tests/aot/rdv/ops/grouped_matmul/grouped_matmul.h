...
*/
template <typename ComputeType, typename GMMBaseParamType, typename GMMArrType>
class GMMProcess {
 protected:
    using B = typename ComputeType::B;
    ComputeType& computeOp;   // inernal computation operator
    const GMMBaseParamType * __restrict gmmBaseParams;
    ...
    const GMMArrType * mListGm;
    const GMMArrType * kListGm;
    const GMMArrType * nListGm;

 public:
    /** @brief constructor */
    __aicore__ inline GMMProcess(ComputeType& computeOp_) : computeOp(computeOp_) {}

    __aicore__ inline void Init(const GMMBaseParamType * __restrict gmmBaseParamsIn,
                                const TCubeTiling* __restrict mmTilingDataIn, const GMMArrType * gmmArrayAddrIn,
                                GM_ADDR groupList);
...
template <typename ComputeType, typename GMMBaseParamType, typename GMMArrType>
 __aicore__ inline void GMMProcess<ComputeType, GMMBaseParamType, GMMArrType>::Init(const GMMBaseParamType * __restrict gmmBaseParamsIn,
    const TCubeTiling* __restrict mmTilingDataIn, const GMMArrType * gmmArrayAddrIn, GM_ADDR groupList) {
...
template <typename ComputeType, typename GMMBaseParamType, typename GMMArrType>
__aicore__ inline void GMMProcess<ComputeType, GMMBaseParamType, GMMArrType>::SetMNConfig(const int32_t splitValue, const uint32_t groupIdx, MNConfig &mnConfig) {
...
template <typename ComputeType, typename GMMBaseParamType, typename GMMArrType>
__aicore__ inline void GMMProcess<ComputeType, GMMBaseParamType, GMMArrType>::SetMKN(const int32_t splitValue, const uint32_t groupIdx,
                                                       MNConfig &mnConfig) {
...
template <typename ComputeType, typename GMMBaseParamType, typename GMMArrType>
__aicore__ inline void GMMProcess<ComputeType, GMMBaseParamType, GMMArrType>::UpdateMnConfig(MNConfig &mnConfig) {
...
template <typename ComputeType, typename GMMBaseParamType, typename GMMArrType>
__aicore__ inline void GMMProcess<ComputeType, GMMBaseParamType, GMMArrType>::MNBlockIdxCompute(MNConfig &mnConfig, const uint32_t curBlock,
...
template <typename ComputeType, typename GMMBaseParamType, typename GMMArrType>
__aicore__ inline void GMMProcess<ComputeType, GMMBaseParamType, GMMArrType>::Process() {
    MNConfig mnConfig;
    ...
    for (uint32_t groupIdx = 0, count = 0; groupIdx < groupNum; ++groupIdx) {
        ...
        int32_t splitValue = GetSplitValueFromGroupList<GMMBaseParamType>(groupIdx, preOffset, gmmBaseParams, groupListGm);
...
*/
template <typename ComputeType, typename GMMBaseParamType, typename GMMArrType>
class GMMGroupMSparseProcess : public GMMProcess<ComputeType, GMMBaseParamType, GMMArrType> {
public:
    /** @brief constructor */
    __aicore__ inline GMMGroupMSparseProcess(ComputeType& computeOp_) : GMMProcess<ComputeType, GMMBaseParamType, GMMArrType>(computeOp_) {}
    ...
    __aicore__ inline void Process()
    {
        ...
        for (uint32_t loop = 0, count = 0; loop < groupListShapeSize; loop += groupListInnerShape) {
            ...
            if constexpr (GMMProcess<ComputeType, GMMBaseParamType, GMMArrType>::B::format == CubeFormat::NZ) {
...
*/
template <class mmType, typename GMMBaseParamType, typename DTYPE_WEIGHT, typename DTYPE_Y, typename DTYPE_SCALE, bool sync = false>
class GMMCompute {
 public:
...
template <typename mmType, typename GMMBaseParamType, typename DTYPE_WEIGHT, typename DTYPE_Y, typename DTYPE_SCALE, bool sync>
__aicore__ inline void GMMCompute<mmType, GMMBaseParamType, DTYPE_WEIGHT, DTYPE_Y, DTYPE_SCALE, sync>::Init(GM_ADDR x, GM_ADDR weight, GM_ADDR bias, GM_ADDR scale,
                                                      GM_ADDR offset, GM_ADDR antiquantScale, GM_ADDR antiquantOffset,
                                                      GM_ADDR groupList, GM_ADDR perTokenScale, GM_ADDR y,
                                                      GM_ADDR workspace, const GMMBaseParamType * __restrict gmmBaseParams,
                                                      const TCubeTiling* __restrict mmTilingData,
                                                      TPipe* tPipe) {
    xTensorPtr = x;
...
template <typename mmType, typename GMMBaseParamType, typename DTYPE_WEIGHT, typename DTYPE_Y, typename DTYPE_SCALE, bool sync>
__aicore__ inline void GMMCompute<mmType, GMMBaseParamType, DTYPE_WEIGHT, DTYPE_Y, DTYPE_SCALE, sync>::SetGlobalBufferBias(uint32_t groupIdx,
...
template <typename mmType, typename GMMBaseParamType, typename DTYPE_WEIGHT, typename DTYPE_Y, typename DTYPE_SCALE, bool sync>
__aicore__ inline uint64_t GMMCompute<mmType, GMMBaseParamType, DTYPE_WEIGHT, DTYPE_Y, DTYPE_SCALE, sync>::SetWOffset(uint32_t tailN, uint32_t k) {
    uint64_t wOffset = 0;
...
template <typename mmType, typename GMMBaseParamType, typename DTYPE_WEIGHT, typename DTYPE_Y, typename DTYPE_SCALE, bool sync>
__aicore__ inline GlobalTensor<typename mmType::BT::T> GMMCompute<mmType, GMMBaseParamType, DTYPE_WEIGHT, DTYPE_Y, DTYPE_SCALE, sync>::SetGlobalBufferW(
...
template <typename mmType, typename GMMBaseParamType, typename DTYPE_WEIGHT, typename DTYPE_Y, typename DTYPE_SCALE, bool sync>
__aicore__ inline void GMMCompute<mmType, GMMBaseParamType, DTYPE_WEIGHT, DTYPE_Y, DTYPE_SCALE, sync>::MMCompute(uint32_t groupIdx, MNConfig& mnConfig, uint32_t coreIdx) {
    if (subBlockIdx != 0) {
        return;
    }
