...
private:
  ...
  __aicore__ inline void ComputeFormerPrecision(uint32_t iOuter, uint32_t calcRowNum,
                                                uint32_t jTail, uint32_t jOuterMax, uint32_t jInnerTail,
  ...
  __aicore__ inline void ComputeFormer(uint32_t i_o_idx, uint32_t calc_row_num, uint32_t j_idx, LocalTensor<float>& rstdLocal, LocalTensor<float>& sumLocal, uint32_t num,
  ...
  __aicore__ inline void ComputeSum(uint32_t i_i_idx, LocalTensor<float>& sumLocal,
                                    uint32_t num, uint32_t left_num = 0, uint32_t reduce_mask = 0)
  {
    LocalTensor<T> xLocal = inQueueX.DeQue<T>();
    LocalTensor<float> sqx = sqx_buf.Get<float>();
    LocalTensor<float> reduce_buf_local = reduce_fp32_buf.Get<float>();
    ...
    PipeBarrier<PIPE_V>();
    #if defined(__CCE_AICORE__) && __CCE_AICORE__  == 100
      ReduceSumHalfIntervalToRepeat(sumLocal[i_i_idx * NUM_PER_BLK_FP32],
                                    sqx, num, left_num);
    #else
  ...
  __aicore__ inline void ComputeLatter(uint32_t i_o_idx, uint32_t calc_row_num, uint32_t j_idx, LocalTensor<float>& rstdLocal, uint32_t num)
  {
  ...
  __aicore__ inline void ComputeY(uint32_t rstdIdx, LocalTensor<T_GAMMA>& gammaLocal, LocalTensor<float>& rstdLocal, uint32_t num)
  {
    event_t eventVS = static_cast<event_t>(GetTPipePtr()->FetchEventID(HardEvent::V_S));
    event_t eventSV = static_cast<event_t>(GetTPipePtr()->FetchEventID(HardEvent::S_V));
    LocalTensor<T> xLocal = inQueueX.DeQue<T>();
