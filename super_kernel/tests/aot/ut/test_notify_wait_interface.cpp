...
std::unique_ptr<SuperKernelKernelNode> CreateKernelNode(uint64_t nodeId, uint32_t streamIdx, uint64_t preNodeId,
                                                        uint64_t nextNodeId, uint32_t numBlocks = 1,
                                                        SkKernelType kernelType = SkKernelType::AIC_ONLY) {
  auto node = std::make_unique<SuperKernelKernelNode>(nullptr, ACL_MODEL_RI_TASK_KERNEL, nodeId, streamIdx,
...
std::unique_ptr<SuperKernelMemoryNode> CreateWaitNode(uint64_t nodeId, uint32_t streamIdx, uint64_t preNodeId,
                                                      uint64_t nextNodeId, uint64_t notifyId) {
  auto node = std::make_unique<SuperKernelMemoryNode>(nullptr, ACL_MODEL_RI_TASK_VALUE_WAIT, nodeId, streamIdx,
                                                      INVALID_STREAM_ID, preNodeId);
  node->SetNodeId(nodeId);
  node->SetNextNodeId(nextNodeId);
...
std::unique_ptr<SuperKernelMemoryNode> CreateNotifyNode(
    uint64_t nodeId, uint32_t streamIdx, uint64_t preNodeId, uint64_t nextNodeId, uint64_t eventId,
    const std::vector<uint64_t> &waitNodeIds = std::vector<uint64_t>()) {
  auto node = std::make_unique<SuperKernelMemoryNode>(nullptr, ACL_MODEL_RI_TASK_VALUE_WRITE, nodeId, streamIdx,
                                                      INVALID_STREAM_ID, preNodeId);
  node->SetNodeId(nodeId);
  node->SetNextNodeId(nextNodeId);
...
std::unique_ptr<SuperKernelMemoryNode> CreateResetNode(uint64_t nodeId, uint32_t streamIdx, uint64_t preNodeId,
...
TEST_F(NotifyWaitInterfaceTest, TestCase3_MemoryNotifyWaitInterface) {
  ...
  auto memoryNotifyNode = std::make_unique<SuperKernelMemoryNode>(nullptr, ACL_MODEL_RI_TASK_VALUE_WRITE, 1, 0,
  ...
  auto memoryWaitNode =
      std::make_unique<SuperKernelMemoryNode>(nullptr, ACL_MODEL_RI_TASK_VALUE_WAIT, 4, 1, INVALID_STREAM_ID, 3);
