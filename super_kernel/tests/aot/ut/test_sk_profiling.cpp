...
static std::unique_ptr<SuperKernelKernelNode> CreateKernelNodeWithCacheInfo(uint64_t nodeId, uint32_t tensorNum = 0,
                                                                            uint64_t attrId = 0, uint32_t opFlag = 0) {
  // Create node using factory method with correct constructor parameters
  auto node = static_cast<SuperKernelKernelNode *>(
      SuperKernelNodeFactory::CreateNode(std::make_unique<aclmdlRITask>(nullptr), ACL_MODEL_RI_TASK_KERNEL, nodeId, 0,
