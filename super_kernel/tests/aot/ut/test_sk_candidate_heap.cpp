...
class SkCandidateHeapTest : public testing::Test {
 protected:
  ...
  uint64_t CreateKernelNode(uint64_t nodeId, uint32_t streamIdx = 0, SkKernelType kernelType = SkKernelType::AIC_ONLY) {
  ...
  uint64_t CreateDefaultNode(uint64_t nodeId, uint32_t streamIdx = 0) {
    auto node = std::make_unique<SuperKernelDefaultNode>(nullptr, ACL_MODEL_RI_TASK_KERNEL, 0, streamIdx,
                                                         INVALID_STREAM_ID, INVALID_TASK_ID);
